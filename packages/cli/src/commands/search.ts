 ...
 * ko면 명소도 함께. 전 섹션 0건일 때만 웹 검색(Perplexity)으로 폴백한다.
 * 섹션 순서: 명소(있으면 최상단, 랜드마크 신호가 가장 강함) → 건수 내림차순.
 */
export const searchCommand = defineCommand({
  ...
  async run({ args }) {
    ...
    const attractionsPromise: Promise<{ places: unknown[] }> =
      lang === "ko"
    ...
    // 명소는 보조 신호라 실패 허용 — 장소·주소가 둘 다 실패했을 때만 명령 자체를 실패 처리한다.
    ...
    const data = web !== undefined ? { places, addresses, attractions, web } : { places, addresses, attractions };
