 ...
 * 한 곳에 묶는다. envelope 확정값(카탈로그 정본): meta는 `{ meta }`, 나머지 둘은 `{ facilities }`.
 */
const INFO_SECTIONS = [
...
/**
 * 역명 메타 + 코레일 시설 + 서울지하철 시설 3종을 병렬 조회한다(allSettled, 부분 실패 보존).
...
export const stationCommand = defineCommand({
  ...
  subCommands: { info: infoCommand, arrivals: arrivalsCommand },
});
