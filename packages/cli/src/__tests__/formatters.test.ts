...
import { FORMATTERS, GUIDANCE_LANG_FALLBACK_LINE } from "../lib/formatters.js";
...
describe("station-facilities unknown 생략", () => {
  ...
  it("역 자체가 미커버(null)면 '시설 정보 없음'", () => {
    expect(FORMATTERS["station-facilities"]({ facilities: null } as never, NO_LANG)).toEqual(["시설 정보 없음."]);
  });
...
describe("빈 목록", () => {
  it("0건은 '없습니다' 한 줄", () => {
    expect(FORMATTERS["nearby-bike"]({ stations: [] } as never, NO_LANG)).toEqual(["주변에 따릉이 대여소가 없습니다."]);
  });
});
...
describe("places-search", () => {
  ...
  it("0건이면 검색 결과 없음 한 줄", () => {
    expect(FORMATTERS["places-search"]({ places: [] } as never, NO_LANG)).toEqual(["검색 결과가 없습니다."]);
  });
});
...
describe("address-search", () => {
  ...
  it("0건이면 검색 결과 없음", () => {
    expect(FORMATTERS["address-search"]({ addresses: [] } as never, NO_LANG)).toEqual(["검색 결과가 없습니다."]);
  });
});
...
describe("nearby-kids", () => {
  ...
  it("0건이면 없음 한 줄", () => {
    expect(FORMATTERS["nearby-kids"]({ kids: [] } as never, NO_LANG)).toEqual(["주변에 아이 놀 곳이 없습니다."]);
  });
});
...
describe("nearby-bus", () => {
  ...
  it("0건이면 없음 한 줄", () => {
    expect(FORMATTERS["nearby-bus"]({ stops: [] } as never, NO_LANG)).toEqual(["주변에 버스 정류소가 없습니다."]);
  });
});
...
describe("nearby-clinic", () => {
  ...
  it("0건이면 없음 한 줄", () => {
    expect(FORMATTERS["nearby-clinic"]({ clinics: [] } as never, NO_LANG)).toEqual(["주변에 소아 야간·휴일 진료 기관이 없습니다."]);
  });
});
...
describe("nearby-around", () => {
  ...
  it("0건이면 없음 한 줄", () => {
    expect(FORMATTERS["nearby-around"]({ places: [] } as never, NO_LANG)).toEqual(["주변 정보가 없습니다."]);
  });
});
...
describe("nearby-barrier-free", () => {
  ...
  it("0건이면 없음 한 줄", () => {
    expect(FORMATTERS["nearby-barrier-free"]({ places: [] } as never, NO_LANG)).toEqual(["주변에 무장애 관광지가 없습니다."]);
  });
});
...
describe("station-meta", () => {
  ...
  it("미커버 역이면 못 찾음 한 줄", () => {
    expect(FORMATTERS["station-meta"]({ meta: null } as never, NO_LANG)).toEqual(["역 정보를 찾을 수 없습니다."]);
  });
...
describe("station-metro-facilities", () => {
  ...
  it("null이면 정보 없음", () => {
    expect(FORMATTERS["station-metro-facilities"]({ facilities: null } as never, NO_LANG)).toEqual(["교통약자 시설 정보가 없습니다."]);
  });
  ...
  it("groups 전멸 + supplementFailed면 실패 문장만(은폐 금지)", () => {
    ...
    } as never, NO_LANG)).toEqual(["일부 시설 정보를 불러오지 못했습니다."]);
  });
...
describe("station-timetable", () => {
  ...
  it("null(미커버 역·키 없음)은 미제공 문장(3-state)", () => {
    expect(FORMATTERS["station-timetable"]({ timetable: null } as never, NO_LANG)).toEqual([
...
describe("subway-arrival (역명 단건)", () => {
  it("미커버 역(null)과 0건(빈 배열)을 구분", () => {
    expect(FORMATTERS["subway-arrival"]({ arrivals: null } as never, NO_LANG)).toEqual([
      "이 역은 실시간 도착 정보 제공 대상이 아닙니다.",
    ]);
    expect(FORMATTERS["subway-arrival"]({ arrivals: { stationName: "서울역", arrivals: [] } } as never, NO_LANG)).toEqual([
      "도착 예정 열차 없음.",
    ]);
  });
...
describe("bus-route-stops", () => {
  ...
  it("0건이면 없음 한 줄", () => {
    expect(FORMATTERS["bus-route-stops"]({ stops: [] } as never, NO_LANG)).toEqual(["경유 정류소 정보가 없습니다."]);
  });
});
...
describe("route-car", () => {
  ...
  // E29: 폴백 표기는 요청 lang과 응답 guidanceLang 두 축의 교집합에서만 선다.
  const fallbackBody = {
  ...
  it("en 요청 + guidanceLang ko → 요약 줄 다음에 '한국어 안내(영문 미제공)' 한 줄", () => {
    const lines = FORMATTERS["route-car"]({ ...fallbackBody, guidanceLang: "ko" } as never, { lang: "en" });
    expect(lines).toEqual(["1km, 약 1분, 택시 약 3,000원", GUIDANCE_LANG_FALLBACK_LINE, "1. 직진"]);
  });
  it("en 요청 + guidanceLang en(정상 영문)은 표기 없음", () => {
    const lines = FORMATTERS["route-car"]({ ...fallbackBody, guidanceLang: "en" } as never, { lang: "en" });
    expect(lines).not.toContain(GUIDANCE_LANG_FALLBACK_LINE);
  });
  it("ko 요청·미지정은 guidanceLang ko라도 표기 없음(정상 한국어 응답)", () => {
    for (const lang of ["ko", undefined]) {
      const lines = FORMATTERS["route-car"]({ ...fallbackBody, guidanceLang: "ko" } as never, { lang });
      expect(lines).not.toContain(GUIDANCE_LANG_FALLBACK_LINE);
    }
  });
  it("guidanceLang 부재(구버전 응답)는 en 요청이어도 침묵 — 부재를 ko로 읽지 않는다", () => {
    const lines = FORMATTERS["route-car"](fallbackBody as never, { lang: "en" });
    expect(lines).not.toContain(GUIDANCE_LANG_FALLBACK_LINE);
  });
...
describe("route-transit", () => {
  it("경로 없으면 못 찾음 한 줄", () => {
    expect(FORMATTERS["route-transit"]({ result: null } as never, NO_LANG)).toEqual(["대중교통 경로를 찾을 수 없습니다."]);
  });
...
describe("route-walk", () => {
  ...
  it("result null(3102 경로 없음)은 미발견 문장(크래시 금지)", () => {
    expect(FORMATTERS["route-walk"]({ result: null } as never, NO_LANG)).toEqual(["도보 경로를 찾을 수 없습니다."]);
  });
});

describe("weather", () => {
  it("null(키 부재)은 실패 문장과 구분해 '제공되지 않습니다'", () => {
    expect(FORMATTERS["weather"]({ weather: null } as never, NO_LANG)).toEqual(["날씨 정보가 제공되지 않습니다."]);
  });
...
describe("air-quality", () => {
  it("null(키 부재/측정소 없음)은 '제공되지 않습니다'", () => {
    expect(FORMATTERS["air-quality"]({ air: null } as never, NO_LANG)).toEqual(["공기질 정보가 제공되지 않습니다."]);
  });
...
describe("nearby-congestion", () => {
  it("area null(핫스팟 밖)은 오류가 아닌 대상 제외 안내 한 줄", () => {
    expect(FORMATTERS["nearby-congestion"]({ area: null } as never, NO_LANG)).toEqual([
...
describe("where-am-i", () => {
  it("null(키 부재)은 제공되지 않음", () => {
    expect(FORMATTERS["where-am-i"]({ data: null } as never, NO_LANG)).toEqual(["현재 위치 정보가 제공되지 않습니다."]);
  });
...
describe("web-search", () => {
  ...
  it("0건이면 없음 한 줄", () => {
    expect(FORMATTERS["web-search"]({ web: [] } as never, NO_LANG)).toEqual(["검색 결과가 없습니다."]);
  });
});
...
describe("barrier-free-detail", () => {
  ...
  it("null(키 부재)이면 제공되지 않음", () => {
    expect(FORMATTERS["barrier-free-detail"]({ detail: null } as never, NO_LANG)).toEqual([
...
describe("nearby-subway 0건 — 최근접 역 동봉", () => {
  ...
  it("nearest가 없으면 종전 문구 그대로(스키마 하위호환)", () => {
    const lines = FORMATTERS["nearby-subway"]({ stations: [] } as never, NO_LANG);
...
describe("route-transit 경유지 미지원(N4)", () => {
  it("unsupported:waypoint는 '경로 없음'과 다른 문장", () => {
    expect(FORMATTERS["route-transit"]({ result: null, unsupported: "waypoint" } as never, NO_LANG)).toEqual([
...
describe("nearby-overview", () => {
  it("data null은 부재 문장", () => {
    expect(FORMATTERS["nearby-overview"]({ data: null } as never, NO_LANG)).toEqual(["주변 정보가 제공되지 않습니다."]);
  });
  ...
  it("상태별 문장이 전부 다르다 — 0건·정보 없음·실패·키 없음을 뭉개지 않는다", () => {
    ...
    expect(lines).toEqual([
      ...
      "한눈에 보기 (1km 안)",
      "가장 가까운 지하철역은 북동쪽 262m 지점에 있는 5호선 길동입니다. 버스 정류소 정보는 이 지역에서 제공되지 않습니다.",
      "식당이 15곳 이상 있습니다. 가장 가까운 곳은 남쪽 40m 지점에 있는 봉래면옥이고, 동쪽 60m 지점에 김밥천국이 있습니다.",
      // 한 곳뿐이면 나열이 없어 조사 자리가 아예 없다.
      "카페가 1곳 있습니다. 가장 가까운 곳은 서쪽 90m 지점에 있는 GS25입니다.",
      "아이 놀 곳은 1km 안에 없습니다.",
      "문화 행사는 서울에서만 안내합니다.",
