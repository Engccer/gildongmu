...
export const ENDPOINT_CATALOG: EndpointSpec[] = [
  ...
  { name: "nearby-subway", description: "내 주변 지하철역 실시간 도착", path: "/api/station/subway-arrival/nearby", method: "GET",
    params: [...LATLNG, { key: "lang", type: "string", required: false, description: "en이면 노선명·역명·도착 문장의 영문 필드(*En) 병기 — 한국어 필드는 그대로(조인 키)" }],
    envelope: "stations", locationParam: true, mcp: true },
  ...
  { name: "station-meta", description: "도시철도역 메타(영문명·노선·환승)", path: "/api/station/meta", method: "GET",
    params: [{ key: "station", type: "string", required: true, description: "역명" },
             { key: "lang", type: "string", required: false, description: "en이면 노선명·역명·도착 문장의 영문 필드(*En) 병기 — 한국어 필드는 그대로(조인 키)" }], envelope: "meta", locationParam: false, mcp: true },
  ...
  { name: "station-metro-facilities", description: "서울 지하철역 교통약자 시설", path: "/api/station/metro-facilities", method: "GET",
    params: [{ key: "station", type: "string", required: true, description: "역명" },
             { key: "lang", type: "string", required: false, description: "en이면 음성유도기 노선의 영문 노선명(parts.lineEn) 병기 — 한국어 필드는 그대로" }], envelope: "facilities", locationParam: false, mcp: true },
  { name: "station-timetable", description: "역 첫차·막차 시간표(TAGO, 서비스데이 기준 라벨 포함)", path: "/api/station/timetable", method: "GET",
    params: [{ key: "station", type: "string", required: true, description: "역명" },
             { key: "lang", type: "string", required: false, description: "en이면 노선명·역명·도착 문장의 영문 필드(*En) 병기 — 한국어 필드는 그대로(조인 키)" }], envelope: "timetable", locationParam: false, mcp: true },
  { name: "subway-arrival", description: "지하철역 실시간 도착(역명)", path: "/api/station/subway-arrival", method: "GET",
    params: [{ key: "station", type: "string", required: true, description: "역명" },
             { key: "lang", type: "string", required: false, description: "en이면 노선명·역명·도착 문장의 영문 필드(*En) 병기 — 한국어 필드는 그대로(조인 키)" }], envelope: "arrivals", locationParam: false, mcp: true },
  ...
  { name: "route-transit", description: "대중교통 경로(추천+대안, ODsay)", path: "/api/route/transit", method: "GET",
    params: [{ key: "origin", type: "string", required: true, description: "출발 좌표 '위도,경도'" },
             ...
             { key: "lang", type: "string", required: false, description: "en이면 노선명·역명·도착 문장의 영문 필드(*En) 병기 — 한국어 필드는 그대로(조인 키)" }],
