...
### 추가
- `route-walk`·`route-car`에 경유지 1개 **`via`**(`위도,경도`). 응답 `waypoint.stepIndex`가 경유지 도착 뒤 첫 단계이고, 텍스트 출력은 그 자리에 `경유지 도착` 줄을 넣는다. `route-transit`에 `via`를 주면 대중교통은 경유지를 지원하지 않는다는 정직 응답(`unsupported: "waypoint"`).
