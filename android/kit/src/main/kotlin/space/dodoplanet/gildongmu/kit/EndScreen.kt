...
// 웹 `src/lib/end-screen.ts` ↔ Kit `EndScreen.swift` 미러. 공유 fixture `end-screen-stale-cases.json`이 동조 강제.
 ...
 */
const val endScreenStaleSeconds = 1800.0

/**
 * 경과는 잠자기 중에도 전진하는 단조 시계로 잰다(iOS `ContinuousClock`, 안드로이드는
 * `SystemClock.elapsedRealtime` 계열) — 잠자기 동안 멈추는 시계는 "주머니에 8시간"을 0분으로 읽고,
 * 벽시계는 앞으로 교정되면 1분 된 새 화면을 30분 지난 것으로 읽는다.
 * 음수·NaN·무한은 소거하지 않는다(근거 없는 소거 금지 — 종료 화면은 걸음 요약의 유일한 채널이다).
 */
fun isEndScreenStale(secondsSinceEnd: Double): Boolean =
