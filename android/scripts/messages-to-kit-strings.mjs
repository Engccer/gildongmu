...
// 값 단위로 대응하고, 웹 vitest `android-kit-strings-drift.test.ts`가 (1) 이 파일이 최신인지,
// (2) Kit xcstrings와 값이 일치하는지 매 커밋 대조한다.
//
...
/** :kit 카탈로그 객체(결정론: 키 정렬·로케일 순서 고정). */
export function buildKitStrings() {
  const { catalog } = buildCatalog(TARGETS.kit);
  const strings = {};
...
export function renderKitStrings() {
  return `${JSON.stringify(buildKitStrings(), null, 2)}\n`;
}

/** manifest 대조: 우리 키의 ko 인자 순서가 iOS manifest와 다르면 그 목록을 돌려준다(빈 배열 = 정합). */
export function argOrderMismatches() {
  if (!existsSync(ARG_ORDER_PATH)) return [];
  const manifest = JSON.parse(readFileSync(ARG_ORDER_PATH, 'utf8'));
  const { argOrder } = buildCatalog(TARGETS.kit);
...
if (isCli) {
  ...
  if (process.argv.includes('--check')) {
    ...
    console.log(`[kit-strings] ${path.relative(REPO_ROOT, OUTPUT_PATH)} ${Object.keys(buildKitStrings().strings).length}키`);
  }
}
