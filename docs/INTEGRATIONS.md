...
- **표결·확정은 최종 접근 진입(6a)보다 앞이다.** 뒤로 옮기면 종점 부근에서 `finalApproachEnter`가 먼저 반환되고 다음 fix부터 0a 가드가 모든 판정을 멈춰 확인된 이탈이 영구 소실된다. ⚠ 6a에서 다시 보는 조건은 **죽은 코드다**(위 블록이 무조건 return하므로 타입 좁힘이 증명한다) — 순서가 곧 불변식이고, 그 자리를 옮기지 않는 것이 계약이다.
