...
- ⚠ **역명은 그 역이 열차 위치임이 실측된 문법에만 싣는다**(불변식 I2). `전전역 출발`의 `arvlMsg3`는 두 역 앞이 아니라 **한 역 앞**을 가리키고(2/2), `{X} 전역출발`의 X는 **조회 역 자신**이다(12/12) — 두 문법은 문장에도 꼬리에도 역명을 싣지 않고 출발 사실만 말한다. 실으면 열차가 없는 역을 "현재 위치"로 낭독한다.
