...
⚠ **착지 대상은 A35 재설계와 다른 축이다**(E38 위원장 판정 2026-09-12): 대중교통 안내 시트에서 **무엇을 누르든 커서는 상태 문장 행**(`SheetControl.status`)에 앉는다. 예외 없음 — 하차 도착도 [다음 구간] 버튼이 아니라 "하차 지점 도착. …" 문장이고 버튼은 한 번 스와이프 아래다. 동기는 착지 성공률이 아니라 **읽기 순서 비용**이었다(피드백 4, 2026-09-11 실승차): 액션마다 커서가 컨트롤로 튀는데 확인하고 싶은 정보는 그 위에 있어 매번 되짚어 올라가야 했다. 남은 판정은 **여부**뿐이라 iOS 술어가 `phaseTransitionLands(...) -> Bool`이고(반환형이 `SheetControl?`로 돌아가면 대상이 다시 갈린다) 웹 `TransitGuidePanel`의 전이 effect도 같은 `lands` 술어 하나로 접힌다. 살아남은 다른 대상은 **자기 질문을 여는 화면**뿐이다 — 역 선택 헤딩(`reboardPrompt`)·급행 확인 헤딩과 그 답 문장(`expressPrompt`·`expressBlocked`)·차량 선택 라벨(`waitingLabel`, 목록 안 포커스 소실 복귀 겸용)·띠바 복귀(`minimize`)·목적지 전환 상태 행(`destChangeStatus`). 그 자리는 착지 낭독이 곧 질문이라 상태 문장으로 옮기면 사용자가 무엇을 고르는지 모른 채 목록 위에 선다. 종전 대상 `advance`·`changeBoarding`·`boardAlready`·`title`은 enum에서 함께 사라졌고 소스 가드 `transit-landing-guard.test.ts`가 그 집합을 잠근다. ⚠ **대상이 하나가 되면 테스트 검출력이 조용히 0이 된다** — "착지했다"와 "애초에 거기 있었다"가 구별되지 않아 전이 착지를 통째로 지워도 초록이다(변이 주입 실측). 웹 테스트는 **누르기 전에 그 컨트롤로 커서를 옮겨**(`clickFocused`) 실기기 경로를 재현한다.
