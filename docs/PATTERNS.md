...
⚠ **`waitingLabel`은 역할이 둘이고 축이 다르다**(a11y 감사 M3): ⓐ pickVehicle·→waiting의 **질문 라벨**과 ⓑ 폴링으로 후보 행이 사라졌을 때의 **목록 안 제자리 복구**(§13.4). ⓑ는 강탈 없는 복구라 성격이 다르다 — ⓐ를 다른 대상으로 옮기는 사람이 ⓑ까지 옮기면 폴 한 번에 커서가 목록 밖으로 튕긴다(헌장 §5 "목록 밖 포커스는 강탈 금지"). 소스 가드가 ⓑ의 대상을 따로 잠근다.
...
- **통지와 상시 표시가 한 조립기를 공유한다**: 이벤트 통지도 `arrivalText`(iOS) / `arrivalPiece`(웹)를 지나 같은 문장을 쓴다. 갈라 두면 같은 관측이 두 문장으로 갈린다(§12.3 드리프트 차단).
...
- **목적지 없는 시작 문장도 남긴다**(`transitGuide.started`): 세션 중 경로 교체(`changeRoute`)는 `guide.destChanged`·`transitGuide.routeSwitched`가 이미 앞에 서므로 목적지를 두 번 말하지 않는다. 웹 패널은 목적지 좌표 없이도 뜨므로 라벨이 없을 때 이 키로 떨어진다.
