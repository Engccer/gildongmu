...
- **착지는 상태 문장(`SheetControl.status`)이다**: 차량 선택 전이에서 사라지는 것은 누른 후보 행인데 그 국면엔 "다음 행동"이 없다. [다른 차량 선택]에 착지시키면 커서가 "다시 고르라"는 권유 위에 앉아 혼선을 재생산한다.
...
- ⚠ **boarding → riding 전이는 착지 대상이 아니다**: 종전엔 그 순간 [탑승했습니다]가 사라져 선점 이동이 헌장 §5였지만, 이제 커서가 얹힌 상태 문장은 사라지지 않고 승격은 폴이 일으킨다 — 착지시키면 듣던 문장을 끊는 포커스 강탈이다. `waiting → riding`(사용자가 고른 열차로 직행)은 종전대로 착지한다.
