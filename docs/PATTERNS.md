...
- **서울버스 완성 문장은 `parseBusArrmsg` 구조로 우리 문장을 고른다**(→ `docs/INTEGRATIONS.md` §시내버스). 초가 있으면 정확값, 없으면 "약"이고 국면으로 가르지 않는다. `운행종료`는 잠금 국면에 도달하지 않아 문장을 만들지 않으며, 미지 모양은 원문을 그대로 병치해 **실패 방향이 종전과 같다**.
...
- **E41 — 통지는 "방금 무슨 일이 일어났고 다음에 무엇을 하는가"만 말한다**(위원장 판정 2026-09-12, 두 안 중 통지 축소). E38로 모든 전이가 상태 문장에 착지하면서 통지와 착지 낭독이 같은 노선·역을 두 번 말하게 됐다. `arrived`는 선두 "하차 지점에 도착했습니다"를 버리고 지시만, `boarded`는 인자 없는 한 문장(`boardedCount` 폐지), `legAdvanced`는 다음 구간 문맥 대신 `legAdvancedNext`. ⚠ **상태 문장에 없는 것은 남긴다** — `arrived`의 다음 구간 조각(`nextLeg`)은 도착 국면 상태 문장이 *현재* 구간을 말하므로 중복이 아니다. 미추적 고지(`untrackable`)도 같다.
