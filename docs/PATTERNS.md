...
- ⚠ **관측 승격 직후 하차 정류소 첫 폴은 즉폴이다**: 선언 경로(`confirmBoarded`)는 종전부터 즉폴이었는데 관측 경로엔 그것이 없어, ①로 관측이 기본 경로가 되자 탑승 직후 최대 60초가 통째로 비게 됐다. iOS는 폴 루프의 `continue`, 웹은 `repollRef`(역 선택 즉폴과 같은 창구).
