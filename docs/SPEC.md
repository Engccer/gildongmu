...
| **보행 인프라** (서울 음향신호기 OA-15543 정적 seed + OSM Overpass) | 접근성/보행 | **운영 중 (2026-07-22)**: `walk-infra.ts`가 음향신호기 seed(EPSG:5186, golden 가드)와 Overpass 횡단보도·점자블록 노드를 합성, `/api/walk/nearby` + 채팅 도구. 상태는 discriminated union(0건 ≠ 서울 외 미제공 ≠ 조회 실패)이고 "등록 ≠ 작동" 각주를 단다. ⚠ Overpass 질의는 타일 anchor 좌표로만 나가고(정밀 좌표 외부 전송 금지), 실패 대응 범위·예산은 `../CLAUDE.md`가 정본. 후속 후보: 무신호 횡단 안내·OSM way/area·전국(경찰청) 확장 | **높음**: 시각장애인 1급 정합 |
