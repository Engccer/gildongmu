...
> **2026-06-24 검색창 자연어 라우터 출시** — 단일 검색창을 유지한 채 자연어 검색을 통합(새 외부 서비스 없음 — Gemini·Perplexity·카카오 기존 키 재활용). 폐기된 "검색⇄채팅 모드 분기"를 되살리지 않고, **Gemini 단발 분류(1왕복)**가 자연어를 `search_places{keyword,region?}`/`search_web`으로 라우팅한다(출력은 기존 장소 리스트·웹 카드 그대로). 실측 근거: 카카오는 필러·지역 토큰을 이미 처리하고 진짜 0건은 "암사동 캐나다 식당"처럼 그 상호가 실재하지 않는 복합 의미 쿼리 → 가치는 **복합 의미 재해석·지역 좌표 앵커링·시의성 웹 라우팅**. 주소(juso)는 무료·결정론이라 라우터 밖 병렬 유지. Perplexity 길 A(분류 라우팅)+길 B(장소 0건 코드 폴백). ⚠ 키워드 재해석은 **단일 카테고리어 하나**여야(compound 구문은 카카오 상호명 매칭으로 앵커 무력화 — 실호출 진단, 프롬프트가 latent 제어판). 회귀 0(Gemini 키 없으면 naive 검색 강등, Perplexity 키 없으면 `search_web` 미노출). 설계/계획 `docs/superpowers/{specs,plans}/2026-06-24-natural-language-search-router*`. 프로덕션 배포·실호출 검증 완료(암사동 캐나다 식당→암사동 레스토랑·해운대 카페→부산·길동 카페→웹 미발동·환율 최신→웹).
