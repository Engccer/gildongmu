...
| 한국관광공사 TourAPI 4.0 | 관광정보 (다국어) | 조사 예정 | 높음 — 외국인 여행자 + dodo 여행 도메인 정합 |
| 카카오톡 메시지(장소 공유) | 공유 | 조사 중 | 중 — 가족 공유 시나리오 |
| 카카오내비 REST | 자동차 경로 | 조사 중 | 중 |
| ODsay / TMAP | 대중교통/도보 경로 | 조사됨 (보고서) | 중 — 텍스트 경로 안내용 |
| 예약 도메인 (캐치테이블/네이버예약 등) | 예약 | 조사 중 (공개 API 존재 여부) | 미정 |
