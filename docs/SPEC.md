...
길동무는 **"검색 → 장소 정보 → 길찾기 연결"의 최소 흐름만을** 접근성과 다국어를 1급으로 두고 다시 구현한다. 풍부한 지도 경험은 목표가 아니다 — 그건 네이버 지도 앱에 위임한다.
...
| 기능 | v0 (현재) | v1 후보 | 비고 |
|------|-----------|---------|------|
| 장소 검색 | mock → 네이버 지역 검색 API | 카카오 로컬 병행 검토 | 네이버는 최대 5건 제한이 큼 |
| 주소↔좌표 | (미구현) | NCP Geocoding | 결제수단 등록 후 |
| 지도 표시 | 없음 (리스트만) | NCP Dynamic Map (JS SDK) | enhancement 레이어로만 |
| 자동차 경로 | nmap:// 딥링크 | NCP Directions 5 (텍스트 안내 추출) | guidance를 텍스트 낭독용으로 |
| 도보/대중교통 | nmap:// 딥링크 | TMAP 보행자 / ODsay 검토 | NCP에 없음 (확정) |
