...
| **서울 지하철 실시간 도착** (A2, data.seoul.go.kr OA-12764 / data.go.kr 15058052·일괄 15125683) | 지하철 | **보류 — 실시간 인증키 차단 (2026-06-17 실측)** — 따릉이 일반키로 `ERROR-338 "해당 인증키로는 실시간 서비스를 사용할 수 없습니다"`. data.seoul.go.kr **실시간 데이터 인증키 별도 신청 필요(사용자 직접, 일반키와 다른 메뉴)**. `swopenapi.seoul.go.kr/api/subway/{KEY}/json/realtimeStationArrival/`, `arvlMsg2`(완성 한국어 도착 문장)·`barvlDt`(초)·`bstatnNm`. 키 발급 후 버스 근접/도착 패턴 재사용, 역 식별은 **A3 seed(`findStationMeta`) 받침대 활용**. 상세 `RESEARCH-2026-06-seoul-open-data.md` | **높음** — 역 상세 "다음 열차 N분/현재 위치" 텍스트. 도시철도 공백 메움 |
