...
> ✅ **2026-08-16 완주.** 아래 체크박스는 미체크로 둔다 — **실행이 이 순서를 따르지 않았기 때문**이고, 일괄 체크는 따르지 않은 순서를 따른 것처럼 위장한다. 실제 경과는 `CHANGELOG.md` 2026-08-16이 정본이다.
>
> **계획과 달랐던 것 셋**: ①**Task 3(문구)을 Task 1 도중으로 앞당겼다** — 판정 계층이 union을 넓히자 소비자가 즉시 타입 에러를 냈고, 그 소비자가 문구 키를 필요로 해 의존이 역방향으로 드러났다 ②Task 1·3·6 일부를 **한 커밋으로 묶었다**(각각 단독으로는 컴파일되지 않아 "커밋마다 green"을 지키려면 분리가 불가능했다) ③Task 7의 변이 주입이 **계획에 없던 2종을 더 돌렸고**, 그 결과가 코드 주석과 spec §4 표로 남았다.
