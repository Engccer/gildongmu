...
|---|---|---|---|---|
| `android-m1`(M0~설정·스토어 준비, 14시간) | fable | 677 | 396M | 612K |
| `android-m4` | fable | 179 | 110M | 707K |
| 코디네이터(이 세션) | fable | 232 | 94M | 400K |
| `android-m6` | opus | 197 | 105M | 573K |
| `android-kit-guide` | opus | 216 | 95M | 362K |
| `android-kit-core` | opus | 129 | 66M | 506K |
| `android-m3` | fable | 90 | 46M | 542K |
| `android-kit-fix` | opus | 102 | 28M | 273K |
| 서브에이전트 합계 | opus 1,765회·468M / fable 127회·27M / sonnet 92회·12M | | | |

**fable 합계 1,305회 · 캐시 읽기 673M · 캐시 쓰기 13.6M · 출력 3.15M**, `android-m1` 한 세션이 캐시 읽기의 59%.

**요금 구조(API 정가 환산, Fable 5.1 $10 입력·$12.50/$20 캐시 쓰기 5분/1시간·$0.25 캐시 읽기·$50 출력)**: fable 약 577달러 = 캐시 쓰기 43%(1시간 쓰기 37%) · 캐시 읽기 29% · 출력 27%. 캐시가 없었다면 약 7,025달러(절약 92%), Fable 5 캐시 단가($1)였다면 약 1,082달러(5.1 인하 효과 47% — Anthropic 발표 "고도 에이전트 작업은 최대 약 45%"와 일치). 전 모델 합계 실제 약 1,235달러 대 캐시 없음 약 11,169달러.

**원인 순위**: ① 한 세션을 7마일스톤 이어 쓴 것(m1) ② 세션들이 500~700K 문맥에서 수백 회 요청 ③ **1시간 넘게 멈춘 큰 문맥 세션의 캐시 재구축** — 15만 토큰 이상을 한 번에 다시 쓴 요청 7건이 캐시 쓰기 비용의 31%이고, 그중 6건이 86~263분 공백 뒤였다(1시간 캐시 만료 → 문맥 전체를 2배 단가로 재기록) ④ 코디네이터 자동 깨움(세션 메시지 39·idle 13·sleep 확인 6) ⑤ `model: opus` 지시 전 fable 서브에이전트 119회 ⑥ 설계 리뷰 최대 5회전. 교훈은 `parallel-sessions` 스킬 2.5.0.
