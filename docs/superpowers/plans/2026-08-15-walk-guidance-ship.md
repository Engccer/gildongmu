...
**결과: 리뷰 2건 모두 도착(지연), MAJOR 2건 + MINOR 6건. 그 사이 컨트롤러가 병행 직접 검증도 수행했다.**

⚠ **기록 정정(2026-08-15)**: 이 절은 한때 "리뷰어 2회 무응답"으로 적혀 있었다. 사실이 아니다 — **두 리뷰 모두 뒤늦게 도착했고**, 컨트롤러가 그 지연을 무응답으로 오판해 헌장 §리뷰 계층의 대체 경로(직접 검증)를 먼저 밟았을 뿐이다. **"응답이 없다"와 "아직 안 왔다"를 구분하지 못한 것이 이 오판의 기제**이고, 판정선은 경과 시간이 아니라 종료 신호여야 했다. 아래 직접 검증 표는 유효하되 **대체가 아니라 병행**으로 읽어야 하고, 종전에 적었던 "관점 비대칭이 빠졌다"는 우려는 리뷰 도착으로 해소됐다.

**컨트롤러 직접 검증 범위**(리뷰에 맡겼던 6축 전부):

| 축 | 방법 | 결과 |
|---|---|---|
| 1. 표 밖 세션 진입점 | `AppShortcuts.swift` 실독 + 딥링크·`onOpenURL` 전수 grep + `transitGuide.start` 호출부 전수 | **문제 없음**. App Intent 2종은 `LaunchActionStore.pending`만 설정하고 세션을 시작하지 않는다. URL 핸들러 없음. 대중교통 시작 호출부는 `altTransitGuideStartable` 게이트 안 1곳뿐 |
| 2. 정식판에서만 죽는 코드 | 안내 경로 조건부 컴파일 전수 + Release 시뮬레이터 **세션 실시작** | **문제 없음**. `GuideDiag`·`TransitGuideDiag`만 `#if DEBUG \|\| EXPERIMENTAL`이고 둘 다 `#else` no-op 스텁 보유(순수 계측). `DirectionsEndpointSearchView`의 `#if DEBUG`는 `chatFocusLog` 래핑뿐. Release 실행에서 세션이 시작되고 잔여 거리·현재 행동·다음 예고까지 정상 |
| 3. 모달 상태 계약 | `.task` 재실행 조건(`directionsEpoch` 3경로) 추적 + `teardown()` 확인 + 시뮬레이터 실측 | **문제 없음**. 조건 판정이 `await` 앞이라 취소에 안 걸린다. 탭 이탈 시 `teardown()`이 세션을 끝내 `.task` 시점엔 다른 시트가 내려가 있어 경합 없음. 확인 후 재진입 미노출 실측 |
| 4. 화면 힌트 삭제 잔여 | `landStopFocus` 호출부 확인 + 시뮬레이터 시트 실측 | **문제 없음**. 호출부 4곳 잔존, 시트에 힌트 없음, `soundDegraded`도 미노출(백그라운드 오디오 실제 부착) |
| 5. 스크립트 `set -e` 부재 | 실패 경로 추적 + Experimental 빌드 실행 | **문제 없음**. 추출 실패는 `[ -z ]`가, `plutil -replace` 실패는 **재읽기 검증**이 잡아 각각 `exit 1`. 재설계의 요지가 정확히 이 공백을 메우는 것이다. 빌드 로그 "6개 로케일 표시 이름 검증 통과" 확인 |
| 6. 가드 오탐·구멍 | 변이 주입 6건(소스 3 + 산출물 3) + 설계 검토 | **구멍 1건 발견·수정**(커밋 `56142d6`): 인자 없는 `check-release-artifact.mjs`가 "가장 최근 아카이브"를 골라 **낡은 산출물을 통과**시킬 수 있었다. `--expect-version`·`--expect-build` 대조를 넣고 `asc-submit`이 제출 대상 값을 넘기게 했다. 잔여 수용: `windowBefore`의 고정 줄 수는 게이트-버튼 사이 삽입에 헛실패할 수 있으나 **조용히 통과하는 방향이 아니라** 수용. `beacon.toggle(` 카운트가 주석 처리된 호출도 세는 것은 보수적 선택으로 유지 |

**Release 시뮬레이터 end-to-end 실측**(정적 리뷰가 줄 수 없는 축): 공지 모달 노출·문안·헤딩 구조(마크다운 `###` 미출력) → 확인 후 재진입 미노출 → 3수단 실호출 조회 성공 → **"도보 안내 시작" 노출** → **세션 실시작**(안내 시트·잔여 거리·현재 행동·다음 예고) → 대중교통 경로 행은 렌더되나 **"대중교통 안내 시작" 부재**(봉인 end-to-end 확인).

**독립 리뷰 2건의 판정** (지연 도착):

- **리뷰 A**: BLOCKER/MAJOR 0건, SHIP 권고. 6축 전부 독립 재확인 + 변이 주입 실측. 잔여 관찰 1건(진입점 계수 가드 스캔 범위) → **반영**(스캔을 `ios/Gildongmu` → `ios` 전체로 확대).
- **리뷰 B**: BLOCKER 0건, **MAJOR 2건**, MINOR 6건.

| # | 심각도 | 지적 | 처리 |
|---|---|---|---|
| M1 | MAJOR | 모달 본문 1이 "도보 섹션에 버튼이 나와요"라고 하는데, 시작 버튼은 disclosure **안**이라 도보 30분 초과 경로(추천 행 기본 접힘)와 최단 행(항상 기본 접힘)에서는 **펼치기 전까지 접근성 트리에 없다** | **기각(위원장 판정 2026-08-15)** — "문안은 의도적으로 단순화한 것이고 그대로 문제 없다". 사실관계는 확인됐으나(`walkCollapseThresholdMinutes = 30`, `walkShortestExpanded = false`) 1회성 공지의 몫은 절차 완전 기술이 아니라는 판단이다. 근거·재개 조건은 `docs/BACKLOG.md` A14 |
| M2 | MAJOR(추측) | 설정 시트를 연 채 언어를 ko로 바꾸면 TabView 트리 재생성 → `.task`가 시트에 가린 채 `walkNoticePresented = true`를 세우고, SwiftUI가 표시하지 않은 바인딩이 `true`로 남아 그 세션 내내 공지가 안 뜬다 | **백로그 등재.** spec §5의 "첫 진입에는 어느 시트도 떠 있지 않다"가 트리 재생성 경로를 안 본 것은 맞다. 다음 콜드 런치에는 뜨므로 영구 유실은 아니고, 처방이 §5.1 표시 계약을 건드리므로 실기기 확인 후 별도 처리 |
| m1 | MINOR | 가드 2번이 선언 본문만 봐서 **소비 지점**을 플래그로 감싸는 회귀를 못 잡는다 | 백로그 등재(창 기반 검사는 서식에 취약해 처방 설계가 선행) |
| m2 | MINOR | 진입점 계수 스캔 범위 | **반영 완료**(리뷰 A와 동일 지적) |
| m3 | MINOR | 간략 버튼 가드가 조건 줄바꿈·주석 삽입에 오탐 | **수용·유지**. 조용히 통과가 아니라 시끄럽게 실패하는 방향이라 안전 측이다 |
| m4 | MINOR | 산출물 검사가 "제출 대상"이 아니라 "최근 아카이브"를 본다 | **이미 수정됨**(`56142d6` — 리뷰 대상 커밋 범위 이후). `--expect-version`·`--expect-build` 대조 |
| m5 | MINOR | ko만 부분 문자열 마커라 카탈로그 드리프트를 놓친다 | **반영**(마커 + 카탈로그 전문 일치 2축, 변이로 검출 확인) |
| m6 | MINOR | 미출시 1.7이 업데이트 이력에 노출(`ReleaseNotesView`가 현재 버전으로 안 거른다) | 백로그 등재. 버전 상향 시 자연 해소되나 지금 기기에 깔린 1.6 빌드에는 보인다 |
