...
| # | 심각도 | 지적 | 처리 |
|---|---|---|---|
| M1 | MAJOR | 모달 본문 1이 "도보 섹션에 버튼이 나와요"라고 하는데, 시작 버튼은 disclosure **안**이라 도보 30분 초과 경로(추천 행 기본 접힘)와 최단 행(항상 기본 접힘)에서는 **펼치기 전까지 접근성 트리에 없다** | **사실 확인(`walkCollapseThresholdMinutes = 30`, `walkShortestExpanded = false`). 위원장 판정 대기** — 문안이 확정본이라 임의 재작성 금지. §5.4가 이 문장을 넣은 목적("버튼을 찾다가 없다고 판단")이 그대로 남는 정확도 결함이다 |
