...
- **재현 결과**(858행 코퍼스 `~/gildongmu-private/probes/subway-probe-2026-09-11.jsonl`): 시간형이 아닌데 `barvlDt != 0`인 행이 **65행**(코디네이터 집계 — 세션은 71행, 시간형 분류 정규식 차이이고 결론은 같다). 내역은 `전역 출발` 25 · `전역 도착` 24 · `전역 진입` 8 · 역명 접두형 7. **최대값은 2026-09-12 00:29:50 `전역 출발`의 `barvlDt = 1200`(20분)** — 심야 표본이다.
