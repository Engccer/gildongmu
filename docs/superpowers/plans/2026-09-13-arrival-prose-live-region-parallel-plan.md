...
- **틀린 전제**(출처: `docs/BACKLOG.md` E37 절의 실측 표, 2026-09-10 20건 표본): *"시간 문장이 아닌 행은 `barvlDt`가 전부 0이다."*
- **재현 결과**(858행 코퍼스 `~/gildongmu-private/probes/subway-probe-2026-09-11.jsonl`): 시간형이 아닌데 `barvlDt != 0`인 행이 **65행**(양쪽 합의 — 세션의 첫 집계 71행은 시간형 정규식이 `^(\d+)분`으로 시작해 **초만 있는 시간형**(`15초 후 …`)을 비시간형으로 센 데다 소계 합산까지 틀린 값이었고, 세션이 원인을 특정해 65로 정정했다). 내역은 `전역 출발` 25 · `전역 도착` 24 · `전역 진입` 8 · 역명 접두형 7. **최대값은 2026-09-12 00:29:50 `전역 출발`의 `barvlDt = 1200`(20분)** — 심야 표본이다.
