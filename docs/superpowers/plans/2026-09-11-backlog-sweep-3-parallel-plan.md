...
- **웨이브 4**: doc-audit → 실기기 배포 최종 → §2 실승차 판정은 위원장. ⚠ **`AGENTS.md` 재생성은 전 웨이브 뒤 한 번**(`cd ~/Mac-Projects && python sync_agent_docs.py`) — 세션들이 CLAUDE.md에 한 줄씩 더하지만 워크스페이스 루트에서만 돌고 웨이브마다 돌리면 중복 커밋이라 세션은 돌리지 않는다(from-here 보고 2026-09-11 05:35). 이 줄이 남아 있으면 아직 안 돌린 것이다.
