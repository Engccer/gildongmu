...
> **웨이브 1 종료 상태(2026-09-11 06:15 KST)**: 4세션 전부 로컬 main ff 통합 — from-here `af98af5f`(B10·E32) · arrival-tail `2734fcc9`(A32, A38 신규 배정) · weight-notice `6ab8c705`(E31, A39 신규 배정) · transit-1 `ee45e7c8`(A33·A37 ②·A34·E34). push는 동결(09-22)이라 origin 미반영. 실기기 배포는 이 SHA로 코디네이터가 Release·Experimental 두 구성. 남은 위원장 확인: transit-1 ko 문안 16키(`~/gildongmu-wt/reports/transit-1.md`) · 선언 식별 잠금의 확정 도착 권한(실승차 A34 행 ③에서 회수) · §2 실승차 행 A37·A34·E34·E31·E32·A32 관찰.
>
> **웨이브 2 종료 상태(2026-09-11 08:40 KST)**: transit-2 `e213e55d`(A36 ①·E36·PORTS 오디오 세션 세 구멍, 9커밋 ff). 위원장 판정 1건(세션이 직접 물음): 프로세스 생존 = riding 동안 keep-alive 저정밀 위치 스트림(spec §4.2.3 ⓐ). 실기기 배포 보류(아이폰 분리). 남은 판정: `docs/BACKLOG.md` §2 A36·E36(11축)·PORTS 오디오 실기기 행.
>
> **웨이브 3 종료 상태(2026-09-11 09:45 KST)**: transit-3 `31d1845b`(A35 착지 재설계·E33 지하철역 → 장소 상세, 6커밋 ff). E37-spec은 수집기 종료(09-12 07:00) 뒤 별도 착수. 실기기 배포 보류(아이폰 분리 — 기기엔 웨이브 1 `9e808111`만). 남은 판정: `docs/BACKLOG.md` §2 A35 재판정(A19 되살림)·E33 실기기 행. 웨이브 4 doc-audit 착수.
>
> **웨이브 4 종료 상태(2026-09-11 10:30 KST)**: doc-audit `5839ef53`(A38·A39 신설, 앵커 9건, E33 전제 정정, E35 재측정 기록, E30 판정 위치, 종결 표 7행, N3 미착수 판정 2건 발굴). AGENTS.md 재생성·doc-audit 마커는 코디네이터가 메인 체크아웃에서 완료. **웨이브 5 small-batch 착수**(N3 재설계 판정 2건 + A39 — doc-audit이 "코드로 열리는 항목은 다 닫혔다"가 거짓임을 잡았다).
>
> **웨이브 5 종료 상태(2026-09-11 11:40 KST)**: small-batch `e6e721eb`(N3 ① [탑승했습니다] 제거·② "이미 탔습니다"·A39 설정 체중 범위 밖 유지+통지+푸터, 5커밋 ff — 리뷰가 승격 즉폴을 철회). 로컬 main은 origin/main보다 약 45커밋 앞(push 동결 09-22). 남은 것: 위원장 문안 확인(N3 새 버튼·통지 3종·A39 통지·푸터 — `~/gildongmu-wt/reports/small-batch.md`) · 실기기 배포(아이폰 재연결 때 두 구성) · §2 실승차 판정 · E37-spec(수집기 종료 09-12 07:00 뒤) · E35 13:00 재측정 기록 · 동결 해제 뒤 E35 구현·웹 잔여(E33 웹·B6·B8·B9 ②·A38).
>
> **릴리스(2026-09-11 14:11 KST)**: 위원장 지시로 별도 세션 release-1.16(opus, `ios-release-submit`)이 정식판 **1.16(빌드 24)** 을 App Store 심사에 제출 — `WAITING_FOR_REVIEW`, 아카이브 `b6a7e5c2`, 문서 분배 `41faafdd`(둘 다 로컬 main 직접 커밋, push 없음 — 동결 해제 뒤 이력 재작성 금지). What's New는 정식판 도달 5건(E32·E31·A39·A32·검색 탭 마이크 행). ⚠ Xcode 계정 0개 → Admin API 키 우회로 서명·업로드(위원장 조치: Xcode 계정 재등록, 실기기 배포도 같은 이유로 막힐 수 있다). 유휴 정지 통지(E36 ⑫) `f635024f`도 이 릴리스 앞에 통합됐으나 실험판 봉인 안.
>
...
| **transit-2** | A36 ① · E36 · PORTS "오디오 세션 소유권 세 구멍" | **인계(transit-1 2026-09-11 06:14)**: 비관측 잠금(A34 목록 0건 → 근사) riding은 폴 0이라 `neverSeen` 축 자체가 없다 — 횟수 상한은 **식별 잠금 riding에만**. `restartPollLoop` 즉폴 게이트가 `interval <= 0`이면 무조건 반환하도록 바뀌었으니 백그라운드 폴 주기는 그 게이트를 지나야 한다. `GildongmuApp.swift`의 `onWalkHandoff` 클로저·`hasWalkHandoff` 전달·Kit `GuideBand.hasWalkHandoff`는 E34 뒤 사실상 항상 false(무해, 후속 정리 후보 — transit-2 범위 밖). ①**A36** `neverSeen` 상한을 벽시계가 아니라 **실제 riding 조회 횟수**로 센다(Kit `TransitGuide.swift` ↔ 웹 `transit-guide.ts` 미러 + 공유 fixture; 상한 횟수는 10분÷폴 주기 등가로 잡되 잠정 상수라 spec에 근거). ②**E36** 백그라운드에서도 riding 폴을 계속하고(`handleScenePhaseChange`의 `pausedInBackground` 폐지 또는 백그라운드 전용 주기), **첫 관측(`trackingStarted`) 한 건만 소리**로 알린다(사다리·도착은 범위 밖). 음성은 억제 유지. **첫 관문은 오디오 세션 생존** — `BeaconTonePlayer` 세션 소유·원복(`didPromote`)을 읽고, PORTS.md gildongmu 행 "오디오 세션 소유권의 세 구멍"(①카테고리/활성화 분리 ②원복 자격 저장 ③`routeChangeNotification` 메아리) **세 가지를 gildongmu 코드에서 각각 재현·확인**한 뒤 필요한 것만 고친다(dodo 관찰이지 gildongmu 재현이 아니다 — 확인 결과를 PORTS 행에 적어 닫는다). 조회 횟수 증가는 일 1,000회 예산과 대조해 백그라운드 주기를 정한다. 배터리는 실측 항목으로 §2에 등록(단정 금지). 실험판 봉인 안. **설계 리뷰 필수**(오디오 세션·백그라운드 실행 = 안전·정확성 축): spec 1개 + 적대적 리뷰 1회 | fable | 2 (transit-1 통합 뒤) |
| **transit-3** | A35 · E33 | **인계(transit-2 2026-09-11 08:40)**: `TransitGuideModel`에 `touchUserAction()`(사용자 조작 표식)·`idlePaused`·`updateKeepAlive()`가 생겼다 — A35 착지 재시도·E33 역 상세 열기가 새 사용자 조작을 더하면 `touchUserAction()`을 부른다(안 부르면 유휴 시계가 안 되돌아간다). `playTone`은 `allowedInBackground:` 필수 인자. 도보·대중교통 공통 `BeaconTonePlayer`·`GuideAudioSession` 계약이 바뀌었다(spec `2026-09-11-transit-background-poll-design.md` §4.3). **인계(transit-1 2026-09-11 06:14)**: 새 착지 대상 `SheetControl.boardAlready`(id `transit-board-already`), 역 선택 화면은 `stationPicker(promptKey:leg:proxy:onPick:onCancel:)` 공용 빌더(탑승 변경·이미 탑승 두 흐름 공유). ①**A35** 착지 실패(`landed=false` 1/6~4/7): `TransitTrackingSheet.landControlFocus` 재시도 1회·고정 지연·`phaseTransitionLanding` 대상 미렌더 가능성·`scrollTo` 인자 ≠ 포커스 키·`onMiss` 폴백 부재를 정본 시퀀스(가시화→지연→경합 해제→대입→검증→재시도, `SearchView.landFirstRowFocus`)와 대조해 고친다. ⚠ 시뮬레이터로 검출 불가 — 판정은 실승차 로그 `landed=`. 계측(`TransitGuideDiag`)에 시도 횟수·실패 사유를 남겨 다음 로그 회수가 판정하게 한다. ②**E33** 안내 시트의 지하철역(경유역 목록·상태 문장)을 장소 상세로 연다: ~~표현 계층은 **시트 위 시트 금지**(N1 리뷰 기각). 강한 디폴트 = 안내 시트 자체 NavigationStack **push**(시트가 그것을 수용하면) → 안 되면 최소화 후 검색 탭에서 열고 띠바로 복귀.~~ **정정(2026-09-11 09:05, transit-3 보고 → 코디네이터 재현 `6f3d93a9`)**: 그 전제는 N1 M3의 반대 방향 오독이었다 — M3는 *장소 상세가 이미 시트(채팅·주변)일 때 루트 안내 시트를 올리는 것*의 기각이고(`2026-08-22-guide-session-minimize-design.md` §리뷰 M3 — 처음 적은 `transit-boarding-phase` 경로는 오기, doc-audit 정정 2026-09-11 10:20), *안내 시트 위 장소 상세*는 `2026-08-12-guide-destination-menu-design.md` §2가 채택한 표준 중첩(`NavigationStack { PlaceDetailView }`, 현행 목적지 상세 경로)이다. BACKLOG E33 본문의 같은 문장도 doc-audit이 고친다. transit-3는 현행 중첩 시트(`detailPlace`) 하나로 일반화했다. 웹 미러는 미연결로 E33 잔여 등록(웹 대중교통 세션이 `TransitGuidePanel` 컴포넌트 수명이라 `requestOpenPlace`가 뷰를 언마운트하는 순간 안내가 통지 없이 끝난다 — 선행 조건: 앱 수준 세션 또는 오버레이). 식별은 `leg.viaStops[]` 좌표·`stationId`(이름 재검색은 최후). 접근성: 경유역 한 줄을 두 객체로 가르지 말 것(채팅 산문 선례 — 1개면 블록 전체 버튼, 2개 이상이면 로터 액션). 웹 `TransitGuidePanel` + `place-open-request` 브릿지 미러. **설계 리뷰 필수**(E33 표현 계층): spec + 적대적 리뷰 1회 | fable | 3 (transit-2 통합 뒤) |
| **E37-spec** | E37 (+E35 판정 재료) | **인계(arrival-tail 2026-09-11 05:50)**: 꼬리 판정 함수 `subwayShowsCurrentLocationTail`(웹 `station-arrivals.ts` ↔ Kit `SubwayArrivalLine.swift`, fixture `src/lib/__tests__/fixtures/subway-arrival-tail-cases.json` 26행)을 같은 자리에서 이어받는다. 수집기에서 E37 표에 없던 문법 2종 관측: `{역} 진입`(arvlCd 0) · `N분 M초 후 (현재역)`. 웨이브 교훈: 판정 함수에는 **그 줄에 실제로 렌더될 값**을 먹인다(원본 필드를 주면 `pickLine`류 자리 표시 계약에서 '자격은 있는데 값이 없는' 조용한 결함). 수집기 데이터(2026-09-12 07:00 KST 이후)로 문법 표·운행 종료 `barvlDt` 판정 → spec(`완성 문장 정본` 계약을 이 화면에서 바꾸는 근거, 승차 국면·안내 통지는 범위 밖) → 6로케일 + 웹 `station-arrivals.ts` ↔ iOS `subwayArrivalLine` 미러(⚠ arrival-tail 통합 뒤 같은 자리). E35 재측정 결과(코디네이터 수집기 2026-09-11 09:00 KST): **5호선 `realtimePosition`이 살아 있다** — `INFO-000` 50행(방화·김포공항·마곡·목동…), 대조군 2호선 50행·8호선 20행. 2026-08-03의 "5호선 결측(INFO-200 2회)"은 지금 재현되지 않는다(13:00 재측정으로 한 번 더 확인). 위원장 판정의 조건 갈래대로 **어림 없이 실제 현재역을 채우는 안이 열렸다**(표시 전용, 판정은 도착 API 그대로, `updnLine` 미사용, 열차번호 조인 `btrainNo`↔`trainNo`). ⚠ 단 `realtimePosition` provider·라우트가 저장소에 없어 **새 서버 동작이 필요**하므로 구현은 push 동결(09-22) 뒤 — 지금은 BACKLOG E35에 재측정 결과와 설계 제약(조회 켜는 시점·쿼터 배증·E36 keep-alive 좌표 ⓐ′와의 관계)만 doc-audit이 기록한다 | fable | 3 |
| **small-batch** | N3 재설계 판정 2건 · A39 | **N3**(BACKLOG §5 N3 위원장 판정 2026-09-10): ①boarding 국면 [탑승했습니다](`transitGuide.confirmBoarded`) 버튼 제거 — 승차역 도착 관측이 자동으로 riding 승격, 관측이 원리적으로 불가능할 때(심야·미제공·조회 실패)만 **다른 문구**의 수동 진행 수단 ②대기 국면 [이미 탑승했습니다](`transitGuide.boardAlready`) → **"이미 탔습니다"**(6로케일), 예외 수단은 이미 착지한 `continueWithoutTrain`. `phaseControls`·`phaseTransitionLanding`(`.confirmBoarded` 착지 대상)·`guidance-gate-drift`·웹 `useTransitGuide` 미러 함께. **A39**(BACKLOG §1): 설정 체중 범위 밖 값 무신호 저장 — 코디네이터 확정: 확정 시점은 필드 편집 종료(`onSubmit`/포커스 이탈)이고 타자 중간값엔 판정하지 않는다, 범위 밖이면 **직전 유효값 유지**(0으로 덮지 않음) + 통지 1회(`.high`, 허용 범위 포함), `Section` 푸터에 허용 범위·"서버에 저장되지 않아요" 상시(E31 §5 잔여 흡수). 설계 리뷰 생략(판정 확정·국소) | opus | 5 |
| **doc-audit** | 문서 전수 정합 | 전 웨이브 뒤 새 창. **A38 항목 신설**(arrival-tail 보고 2026-09-11 05:45: `arvlMsg3`가 비고 현재역이 99 괄호로만 오는 도착에서 영문 줄이 현재역을 통째로 잃는다 — A32 이전부터, 영문 요구 자리 확장은 E27 줄 원자성 계약 변경이라 범위 밖. BACKLOG A32 "남은 것"에서 승격, 식별자는 코디네이터 배정) · **A39 항목 신설**(weight-notice a11y 감사 관찰 2026-09-11 05:55, 범위 밖 기존 결함: `SettingsView.commitWeight()`가 범위 밖 체중(20~300 밖)을 화면·낭독 신호 없이 0으로 접어 SR 사용자가 저장된 줄 안다 — 헌장 §1 3-state 위반, iOS 설정 화면 단독·소규모. 코디네이터 판정: 등록, 웨이브 3 소규모 묶음 후보). 종결 식별자 이동, §2 실승차 표에 A37·A34·E34·A36·E36·A35·E33 판정 행 확인, PORTS 행 닫기, E30 판정 위치 기록 | opus | 4 |
...
- **웨이브 2**: transit-2 (transit-1 통합 뒤) — ~~통합 뒤 코디네이터 실기기 배포 1회(두 구성)~~ **보류(2026-09-11 08:20 위원장: 출근으로 아이폰 분리)**. 기기에 올라간 것은 웨이브 1 `9e808111`(정식판·실험판 06:30). transit-2·transit-3 결과의 실기기 배포는 다음 연결 때 한 번에(두 구성)
- **웨이브 3**: transit-3 · E37-spec (수집기 종료 2026-09-12 07:00 KST 이후)
- **웨이브 4**: doc-audit → 실기기 배포 최종 → §2 실승차 판정은 위원장. ✅(2026-09-11 10:30 코디네이터 실행) ~~⚠ **`AGENTS.md` 재생성은 전 웨이브 뒤 한 번**~~(`cd ~/Mac-Projects && python sync_agent_docs.py`) — 세션들이 CLAUDE.md에 한 줄씩 더하지만 워크스페이스 루트에서만 돌고 웨이브마다 돌리면 중복 커밋이라 세션은 돌리지 않는다(from-here 보고 2026-09-11 05:35). 이 줄이 남아 있으면 아직 안 돌린 것이다.
