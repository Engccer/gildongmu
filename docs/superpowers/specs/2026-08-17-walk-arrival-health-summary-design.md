...
> **개정 2026-08-19(위원장 판정 — 요약은 도착만이 아니라 도보 세션의 모든 정상 종료에)**: 종전엔 확정·추정 도착 두 경로에서만 요약이 나왔고 사용자 중지(중지 버튼·시트 스와이프·VO escape)·목적지 변경·권한 상실 종료는 시트가 즉시 닫혀 요약이 없었다. 이제 도착이 아닌 종료도 도보 세션이면 같은 종료 화면(`arrivalDest` + `endKind == .stopped`, 헤딩 `ios.beacon.endedHeading` "안내 종료", 첫 문장은 중지 사유 `endText`, 주변 확인 없음)을 남겨 요약을 보여 준다(`BeaconModel.stopLeavingSummary`, 사용자 중지는 `stopByUser`, 권한 상실은 `stopAndFail`). **판정은 동기다**: §3의 "라이브 구독 없이 사후 질의"를 뒤집어 도보 세션 시작 시 `CMPedometer.startUpdates(from:)`로 누적(`liveHealthSample`)을 받고, 종료 순간 그 값으로 "요약을 보여 줄 만큼 걸었는가"를 가른다 — 사후 질의만으로는 판정이 비동기라 종료 화면을 띄운 뒤에야 지울 수 있고 그것이 깜빡임·VO 착지 경합이 된다(리뷰 MAJOR). 도착 경로도 누적이 있으면 동기 커밋(착지 전 행 존재)하고 사후 질의는 누적이 없을 때의 폴백이다. 중지 경로엔 그 폴백이 없다(첫 라이브 콜백 전 중지 = 요약 없음, 의도된 수용 — 50m는 30초 이상이고 콜백은 걸음이 쌓이면 수 초 안에 오므로 이 창에서 지는 요약은 임계값 미만이거나 만보계가 죽은 세션이다. 폴백을 태우면 판정이 비동기로 되돌아간다). 스와이프 종료 뒤 재-present가 예정된 동안(`hasPendingEndScreen`) 길찾기 탭은 시작 버튼 복귀 포커스를 예약하지 않는다(재-present 착지와 경합). **표시 임계값**: 유효 거리(만보계 거리, 없으면 걸음×0.7m)가 `WalkHealth.minMeaningfulDistanceMeters`(50m, 잠정) 미만이면 요약 행이 없다 — 도착 종류에도 같은 규칙이라 §2의 "0걸음 0kcal 그대로 표시"는 폐기(`ArrivalHealthLoad.negligible`). 요약이 없는 `.stopped` 종료는 종전 "중지 = 닫힘"과 과정까지 같다(화면이 스치지 않는다). **스와이프·VO escape**는 presentation 바인딩의 set 클로저 안에서 같은 바인딩을 되돌리면 방금 커밋된 dismiss와 경합하므로 `stopByUser(endScreen: .afterDismiss)`가 보류하고 시트 `onDismiss`(`presentPendingEndScreen`)가 띄운다. 종료 화면이 남는 사용자 중지는 착지 문장(`ios.beacon.stopped`)을 `.high`로도 통지한다(도착 동형 — 중지 버튼이 사라지는 전이). 종료 화면 밖 경로: 다른 세션의 claim·`teardown`은 종료 화면을 남기지 않고, OS 강제 종료는 화면 자체가 없다(비정상 종료). `.stopped`의 `clearArrival`은 `statusText`를 비우지 않는다(권한 상실 실패 문장이 인라인 상태 줄·복구 버튼의 근거로 남아야 한다).
