...
## 8.1 검증 결과 (구현 2026-09-18)

**Kit 단위**(`TransitBriefingStationsTests`, 28건): 줄 3종 × 조인 성공·실패·부분 목록·역순 목록·같은 이름 두 번·빈 이름·nil `stops`·버스·도보 연속. 전부 통과.

**변이 주입 6축**(커밋 뒤, `git checkout` 복원):
...
- 2026-09-18 ko, 10 OD / 184줄 / 이름이 들리는 줄 95 → **조인 성립 95/95 = 100.0%**
- 표본 구성(부재는 미실측으로 FAIL): 환승 2회 이상 3경로 · 버스↔지하철 혼합 16경로 · 도보 줄 없는 지하철 연속 7자리 · 수도권 밖 4도시. **전부 있었다.**
- `.walk` 게이트의 전제(도보 줄 `toName` == 다음 non-walk leg `fromName`): 66쌍 중 불일치 0.
- ⚠ **10번째 OD에서 ODsay 429**가 왔다(12개 중 2개 미실행). 게이트가 429라는 것은 그날 프로덕션도 429라는 뜻이라 재시도하지 않았다. **en 표본은 미실측**이다 — 라벨 언어 축은 기존 술어 `transitLegUsesEnglish` 재사용이고 §6 말미의 en 실측(226 leg, 자격 미달 0건)이 그 술어를 이미 덮는다.

**앱 소스 가드**: `src/lib/__tests__/briefing-station-entry-guard.test.ts` 16건. E44 가드(`station-detail-guard.test.ts`)는 §5.3 개정에 맞춰 상시 노출 계약으로 고쳤다.
