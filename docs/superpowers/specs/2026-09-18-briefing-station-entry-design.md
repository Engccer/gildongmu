...
**진동**은 문장이 나가는 조건과 같다(`ResultHaptic` 계약). 번호가 있으면 통지가 없으므로 진동도 없고, 없음·모름은 같은 `.attention`, 조회 실패만 `.failure`다 — 촉각은 "실패"를 가를 뿐 없음과 모름은 가르지 않으며, 그 구분은 통지 문장이 맡는다.
...
## 8.2 구현 리뷰 (2026-09-18, 서브에이전트 3회 — Kit·뷰·접근성)

세 리뷰 모두 **BLOCKER 0**. 잡힌 것은 전부 "검증했다고 믿은 자리가 실은 비어 있었다"는 계열이다.

| 리뷰 | 등급 | 내용 | 처리 |
|---|---|---|---|
| Kit | MAJOR | **하차 축 검출력 0** — `.alight`를 `stops.last`로 되돌려도 27건이 전부 초록(변이 12종 실측). fixture의 하차역이 늘 목록 마지막이었다 | fixture 3건 추가, 세 변이 검출 확인 |
| Kit | MINOR ×4 | 이름 게이트 등가 변이 · `.walk` 탐색 방향 무검증 · `lineName: String?`와 저장소 API 마찰 · **키 문자열 반환이 `appLocalized(변수)`로 소비되면 `check-xcstrings-keys.mjs`를 우회** | 전부 채택(마지막은 리터럴 `switch`로 되받기) |
| 뷰 | MAJOR ×2 | 소스 가드 ①-2·③-1이 **되돌림을 통과시킨다** — 옵트인 기본값은 부분 매치라 `= { _, _ in }`를 놓치고, 액션 상태 분기는 목록을 **만드는 자리**(`rotorItems`)를 비켜 있었다 | 술어를 구조 기반으로 교체, 되돌림 주입해 확인 |
| 뷰 | MAJOR | **라벨 영문 이름의 출처가 자격 술어가 보는 필드와 다르다** — 술어는 leg 필드(`fromNameEn`·`toNameEn`)를 보는데 라벨은 `stop.nameEn`(서버가 `passStopList`에서 따로 채운다)을 읽어, 한쪽만 빈 응답에서 **줄은 영어인데 라벨만 한국어**가 된다. §6의 en 실측 226 leg는 leg 필드만 쟀고 이 필드는 표본 축에 없었다 — 구현 이탈이 아니라 **이 spec 전제의 공백** | Kit이 그 줄이 쓴 영문을 함께 싣는다(`TransitBriefingStation.nameEn`). 판정과 값이 같은 필드를 보면 술어가 참일 때 값의 존재가 함께 보장돼 폴백 경로가 사라진다 |
| 뷰 | MINOR ×2 | 로터 역순 가드가 `.reversed()` 존재만 봄 · "줄이 `Text`로 남는지"가 주석 포맷 단언(오탐 + 미탐) | 둘 다 교체 |
| 뷰 | 정상 | `.task(id:)` 루프(언어 전환에 재시작 안 함·취소 스핀 없음·접힘 시 취소, 펼치면 즉시 1회) · `NavigationStack(path:)`가 기존 동작을 바꾸지 않음(`directionsEpoch` 재생성이 `stationPath`를 함께 비우는 것이 §7 판정과 맞물림) · `place-detail-sheet-guard` 충족(우회가 아니라 push 요건 충족) · `@ViewBuilder` 저장 프로퍼티·`accessibilityActions` 안 `ForEach`(E33 배포 선례) | — |
| 접근성 | MAJOR | **번호가 있어도 걸지 못하면 무반응** — `URL(string:)` nil·`openURL` 거부에서 통지 0·진동 0·화면 변화 0이고 라벨은 그대로 남는다. 보이는 전화 줄은 같은 조건에서 줄이 사라져 부재가 단서지만 로터 액션은 단서가 0 | 둘 다 기존 실패 창구로(새 문구 없음) |
| 접근성 | 합격 ×7 | 한 줄=한 객체 · 통지 설계(성공 무통지는 시스템 알럿이 응답, 역 이름 제외, `.high` 근거) · 3-state · 로터 부담(커스텀 액션은 선형 탐색 정류장을 늘리지 않는다) · 과잉 ARIA 0 · 문구(**"역" 접미 부재가 오히려 맞다** — 구간 줄이 쓰는 이름과 일치) | — |
| 접근성 | 기록 | 노선 표 미지가 "없습니다"로 접힘(E44 상속, 신규 개통 직후만) · "찾고 있습니다" 뒤 완료 신호 0(도착 통지는 헌장 §2와 충돌) | 현행 유지, BACKLOG에 근거 기록 |

**보류한 것**(리뷰도 동의): 클로저 저장으로 뷰 동등성 스킵 상실(재계산 비용 마이크로초) · `@Observable` 프로퍼티 단위 추적이라 격리 입자가 "그 줄 하나"가 아님(주석만 정정) · push 중 미리 조회 루프 계속(신선도 300초에 접혀 네트워크 0) · 가드 ⑤ 들여쓰기 하드코딩(대상 문자열이 파일에 한 자리) · push 목적지 정책이 소비자별 복제(현 소비자 1개).

⚠ **선례가 실기기 항목을 면제하지 않는다**(접근성 감사): 라벨 전파는 E33에 같은 구조가 출하돼 있지만, 거기서 깨졌다면 증상이 `178m`→"178분" 오독뿐이라 지나쳤을 수 있다. 브리핑은 실패하면 괄호 한글이 통째로 낭독돼 **증상 크기가 다르다**. §8의 실기기 항목 ③을 선례를 근거로 빼지 말 것.
