...
- 런타임 정본 `ManualLocationStore(store: KeyValueStore)`(앱 싱글턴 `AppConfig.manualLocationStore`, 매체는 기존 `SharedPreferencesStore(context)` 기본 파일에 키 `manualLocation` 하나 — 파일을 따로 두지 않는다, 값 `KitJson`의 `ManualLocation` JSON): `current: StateFlow<ManualLocation?>` · `verdict: StateFlow<ManualVerdict?>`(**비영속** — 며칠 전 판정이 새 세션 라벨을 정하면 안 된다; `set`/`clear`가 함께 null로) · `set(label, labelRoman, lat, lng, origin)`(revision = 직전+1, `setAt` = epoch 초, `isValid` 통과 시에만 저장·`verdict = null`) · `setVerdict(next)`(같으면 무시; `Judge.run`만 부른다) · `clear()`. 첫 읽기는 디스크 I/O — `AppConfig` lazy 초기화가 아니라 `MainActivity` 첫 판정(`ON_START`) 안에서 `Dispatchers.IO`로 로드한다(`SharedPreferencesStore` KDoc 계약). 복원 시 `isValid` 실패는 폐기(손상 값이 haversine NaN → 영구 유지되는 최악 방향). `isValid`는 iOS 그대로: lat/lng/setAt 유한·범위, 라벨 공백 아님, origin이 있으면 그 필드도 유한·`accuracy > 0`.

### 13-2. 판정과 유효 좌표 (`location/ManualLocationJudge.kt`·`location/EffectiveLocation.kt`)

- `ManualLocationJudge(manual, location, now, minIntervalSeconds = 30)`: `suspend fun run()` — iOS `run()` 그대로: 수동 없음 → 무시 / `origin == null` → `undecidable`(측위 비용 0) / 권한 `Fine` 아니면 → `undecidable`(팝업 없음) / `captured = revision` → `location.currentFix(force = true, silent = true)`(`ManualFix?`, 실패 null) → `judgeManualLocation` → **CAS**(revision 다르면 폐기) → `drop`이면 `clear()` + `AppNotices.post(autoCleared)`, 그 밖은 `setVerdict`. **어느 갈래로 끝나든 결과를 남긴다**; 단 `CancellationException`은 그대로 통과하고 그 경로에서는 결과를 남기지 않는다(미완이지 실패가 아니다). **재진입 가드**: 진행 중 `run()`은 그 Job을 `join`한다(동시 트리거 2회 = 측위 1회·통지 최대 1회). **과발화 억제**: 마지막 실행 뒤 `minIntervalSeconds` 안의 재호출은 건너뛴다(회전·다크모드·앱 언어 변경마다 액티비티가 재생성돼 `ON_START`가 다시 오기 때문 — `configChanges` 없음; force 조회는 이 억제를 받지 않는다).
...
- **판정 측위는 화면이 요청하지 않은 측위**(CLAUDE.md `silent` 규칙): `LocationStore.currentFix(force, silent)`는 `silent`면 실패에 `lastFixFailed`를 세우지 않는다(표시줄의 "이 세션에서 확정된 실패" 정의가 넓어지지 않게). 성공 fix의 `stored` 갱신은 그대로(더 나은 좌표는 표시에도 좋다). 반환 `ManualFix(lat, lng, accuracy, at = 지금 epoch − 나이)`. 지정 시점 origin은 `silent = false`.
- `EffectiveLocation(location, manual, judge)` — **앱 층의 좌표 진입점은 이것뿐**(iOS `LocationService` 안에 수동 우선이 있어 우회 경로가 없는 것과 같은 효과): `suspend fun coordinate(force): NearbyCoord`(`force`면 `judge.run()` 먼저 → 수동 있으면 그 좌표 → 아니면 `location.currentCoordinate(force)`) · `suspend fun coordinateForRanking(): NearbyCoord?`(수동 있으면 그 좌표 — **측위 없음** — 아니면 `location.coordinateForRanking()`) · `fun nearbyCoordinateSource(): NearbyCoordinateSource.Current`(어댑터·오류 번역을 `LocationStore`에서 **옮겨 온다**; `LocationStore.nearbyCoordinateSource()`는 삭제). `LocationStore.coordinateForRanking()`은 `gpsCoordinateForRanking()`으로 개명(GPS 층임을 이름이 말한다). **소비자 전수**(웹 `effective-location` 동형): 내 주변 허브 진입 화면(`MainActivity` `nearbyFactory`의 `current`) · 검색 거리 가중(`SearchViewModel.coordinate`) · 길찾기 `EndpointLocator.currentCoordinate`(출발지)와 **`coordinateForRanking`(끝점 후보 근접 가중 + 길찾기 탭 진입 주소 병기 — 수동이면 측위 0)** · 채팅 앵커(M6 통합 뒤 전파). **표시줄 주소 조회(`coordinateForDisplay`)와 실시간 안내는 실좌표**. 소스 가드: `:app`에서 `gpsCoordinateForRanking(`·`currentCoordinate(` 호출은 `location/` 안뿐.

### 13-3. 지정 화면 — M3 끝점 검색 재사용 (`directions/EndpointPicker.kt` 추출)

- 현행 `EndpointSearchContent(vm: DirectionsViewModel, p)`는 뷰모델에 묶여 있어 허브가 못 연다. **끝점 검색 로직을 `EndpointPicker`로 추출**(iOS `EndpointSearchModel` 동형): `class EndpointPicker(search: SearchService, store: RecentSearchStore, strings, io, scope, ranking: suspend () -> NearbyCoord?, onSelect: (DirectionsEndpoint, DirectionsFieldTarget) -> Unit)` — `state: StateFlow<EndpointSearchState?>`·`consumedCandidateRevision`·`open(target)`·`close()`(picker 상태만 비운다; **착지 발급은 `DirectionsViewModel.closePicker` 래퍼에 남는다**)·`submitCandidates()`(`ranking()`으로 근접 가중 — M3 동작 유지)·`selectPlace/selectRecentEndpoint/selectAddress/selectCurrent`·`removeRecentEndpoint/togglePinRecentEndpoint/clearRecentEndpoints`. `selectCurrent`의 가드는 `target ∈ {from, manualLocation}`, **`refreshCurrentLocation()` 부수효과는 VM의 `onSelect`(from일 때만)로 옮긴다**(지정 화면에서 해제 직후 8초 강제 측위·`isRefreshingCurrent` 토글 금지). `DirectionsViewModel`은 합성 + 기존 공개 함수 위임(M3 호출부·테스트 무변경 — `DirectionsViewModelTest`가 `setEndpoint` 이중 기록 금지·`select` 뒤 착지·`closePicker` 취소를 단언한다). `EndpointSearchContent(picker: EndpointPicker, p)`. `directions/DirectionsStrings.kt` `stringId` 표에 `manualLocation.pickTitle`·`manualLocation.useGps`·`manualLocation.locating` 추가(기존 가드 `DirectionsSourceGuardTest`).
- `DirectionsFieldTarget`에 **`manualLocation`** 추가(iOS 동형). exhaustive `when` 전수: `recentScope` → `to`(전용 스코프 없음 — 자주 가는 곳이 지금 서 있는 곳의 후보이기도 하다) · 화면 제목 → `manualLocation.pickTitle` · "현재 위치 사용" 자리 버튼 → `manualLocation.useGps`("현재 위치로 되돌리기") · `fieldText`(라벨·미확정 문구)와 `LandingTarget.Field` 착지 → **도달 불가**(`error("manualLocation은 길찾기 폼 필드가 아니다")`) — 길찾기 폼에 그 필드가 없다. ⚠ 이분 삼항 금지(iOS 실사고: 제목이 "도착지 검색"으로 떨어짐).
- 허브 진입점: 표시줄 버튼 → `AppRoot`가 `returnFocus.slot.remember("location-bar")` 뒤 `ManualLocationRoute`(스택, `location/ManualLocationPickerScreen.kt`, 등록 한 줄). `ManualLocationPickerViewModel`이 `EndpointPicker`를 `manualLocation` 타깃으로 열고 `onSelect`에서 **`commit`(웹 `commitManual` 동형)**: in-flight 가드(연타 무시) → `isCommitting = true`, `StatusLine`에 `manualLocation.locating` 진행 통지 → `Current`면 `manual.clear()` / `Place(label, lat, lng, labelRoman)`면 `origin = if (location.authorization() == Fine) location.currentFix(force = true)?.takeIf { isEligibleManualFix(it, now) } else null`(**권한 다이얼로그 없음** — 판정 41) → `manual.set(...)` → **그 다음** pop(`onDone` 콜백). 실내 8초는 진행 통지가 말한다. 뒤로(취소)는 진행 중이면 잡을 취소하고 pop(지정 안 됨). 허브 복귀 착지는 표시줄 버튼(`NearbyHubScreen`의 복귀 resolver가 `"location-bar"`를 그 버튼 requester로 푼다 — kind 키만 알던 resolver 확장), 그때 라벨은 이미 확정돼 있다(pop이 `set` 뒤라 "표시줄 라벨 자체가 결과"가 성립). 지정 확인 통지는 없다(iOS 동형).
...
적대적 설계 리뷰 판정(§13, `review-m2c-design.md`): 1차 REQUEST_CHANGES(MAJOR 6·MINOR 12·NIT 6) → 24건 전부 반영(MAJOR-5 → 판정 37, MAJOR-6 → 판정 33 병합 규칙·§13-5, MAJOR-3·MINOR-6·7 → 판정 38, 나머지 본문), 기각 0.
