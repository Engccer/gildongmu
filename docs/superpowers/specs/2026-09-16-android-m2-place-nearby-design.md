...
- 모든 화면이 Material3 `Scaffold`에 **`TopAppBar`**(제목 `Text`에 `semantics { heading() }`, 스택 화면은 왼쪽에 뒤로 버튼 `android.common.back`("뒤로"), 오른쪽에 화면별 동작 버튼 — 내 주변은 새로고침 — 그 **뒤**에 공용 `SettingsAction`(설정, §14-1)이 탭 루트 4개 모두에 온다) + 하단 `NavigationBar`(골격: 항목 4개 `AppTab.order`, `NavigationBarItem`이 `Role.Tab`·`selected`를 낸다). 검색 화면도 M1의 본문 제목을 상단 바로 옮긴다(화면마다 제목 자리가 다르면 위치를 외워 쓰는 탐색이 깨진다).
...
- **권한**(`PermissionGate` — 대기 슬롯은 **`LocationStore`(앱 싱글턴)가 쥔다**: `suspend fun requestLocation(): PermissionOutcome`은 스토어의 `CompletableDeferred` 목록에 매달리고, Activity는 "요청을 띄우는 손"(`ActivityResultContracts.RequestMultiplePermissions`, FINE+COARSE 동시 — Android 12 다이얼로그가 "정확한/대략적인" 선택을 준다)과 "결과를 전달하는 손"만 `onCreate`/`onDestroy`에서 등록·해제한다. 다이얼로그 중 Activity가 재생성되면 새 Activity의 콜백이 스토어의 같은 슬롯을 재개한다(iOS `authContinuations` 동형). 동시 요청(검색 랭킹은 요청하지 않으므로 사실상 내 주변 두 화면)은 한 다이얼로그에 전부 재개. 프로세스 재생성 뒤 도착한 결과는 슬롯이 비어 있다 — 로그만 남기고 버린다(화면 `load()`가 `checkSelfPermission`으로 다시 판정한다). `launch()`는 Activity가 STARTED 이후여야 하는데 요청은 화면 진입 로드에서만 시작되므로 성립한다):
  ...
  - 권한 요청은 **내 주변 화면 진입(로드)·길찾기의 현재 위치 조회·채팅 첫 전송의 위치 prime**(M6, `EffectiveLocation.prime` — 수동이면 측위 0)에서만 일어난다(iOS도 같은 세 곳). 검색·상세는 묻지 않는다(iOS "When In Use" 계약). 앱 시작 즉시 요청 금지. `ACCESS_BACKGROUND_LOCATION` 요청 없음(D11). 설정 열기 인텐트는 `location/LocationSettings.kt` 함수 둘(앱 상세·기기 위치)로 통일 — M3도 같은 함수.
...
> M2b 통합(`fc246219`) 뒤 코디네이터 지시. 동작 정본 iOS `ManualLocationStore.swift`·`ManualLocationJudge.swift`·`LocationBarView.swift`(버튼·`commit`)·`LocationService.swift`(`coordinateForRanking`의 수동 우선)·`Directions/DirectionsEndpointSearchView.swift`(`.manualLocation` 타깃)·`DirectionsTabView.swift`(출발지 라벨·`effectiveCoordinate`)·`Nearby/{NearbyLoadState,AroundNearbyView}.swift`, 판정 계층은 :kit `ManualLocation.kt`(**이미 이식됨, 무변경**), 문구 계약은 웹 `src/lib/__tests__/manual-location-copy.test.ts`(금지 표현 가드), 지정 화면 계약은 웹 `ManualLocationPicker.tsx`(`commitManual` in-flight·진행 통지·확정 뒤 닫기). 신설 문자열 키 0(`manualLocation.*` 11키·`android.nearby.aroundHereManual*`·`aroundLoadedManual`·`directions.*` 전부 실재). §1~§12 계약 승계.

**한 줄 요약**: 사용자가 GPS 대신 자기 위치를 장소·주소로 지정하면 그 좌표가 "내 주변"·검색 거리·길찾기 출발지·끝점 후보 정렬을 정한다(우선순위 **장소 앵커 > 수동 위치 > GPS**, 실시간 안내만 실좌표 — M4). 지정 화면은 M3의 끝점 검색을 재사용하고, 표시줄은 버튼이 되어 "지정한 위치, X, 위치 지정하기"를 말한다. 100m 넘게 이동한 것이 실측되면 자동 해제하고 통지한다.

### 13-1. 상태·저장 (`location/ManualLocationStore.kt`)

- 런타임 정본 `ManualLocationStore(store: KeyValueStore)`(앱 싱글턴 `AppConfig.manualLocationStore`, 매체는 기존 `SharedPreferencesStore(context)` 기본 파일에 키 `manualLocation` 하나 — 파일을 따로 두지 않는다, 값 `KitJson`의 `ManualLocation` JSON): `current: StateFlow<ManualLocation?>` · `verdict: StateFlow<ManualVerdict?>`(**비영속** — 며칠 전 판정이 새 세션 라벨을 정하면 안 된다; `set`/`clear`가 함께 null로) · `set(label, labelRoman, lat, lng, origin)`(revision = 직전+1, `setAt` = epoch 초, `isValid` 통과 시에만 저장·`verdict = null`) · `setVerdict(next)`(같으면 무시; `Judge.run`만 부른다) · `clear()`. 첫 읽기는 디스크 I/O(`SharedPreferencesStore` KDoc 계약): `GildongmuApplication.onCreate`가 `Dispatchers.IO`에서 **hydration Job**을 띄우고, `current`를 읽는 **suspend 경로 전부**(`EffectiveLocation.coordinate`·`coordinateForRanking`·`judge.run`)는 그 Job을 `join`한 뒤 읽는다 — **불변식: hydration 전 `current` 읽기는 없다**(웹 동기 `hydrate`·iOS `init` 읽기와 같은 효과; 창이 열리면 프로세스 재생성 직후 복원된 내 주변 탭이 "이번 조회만 GPS 기준"이 되고 화면으로 반증되지 않는다). 표시줄은 `StateFlow`를 관찰해 따라온다(첫 프레임 GPS 갈래 표시는 수용). 복원 시 `isValid` 실패는 폐기(손상 값이 haversine NaN → 영구 유지되는 최악 방향). `isValid`는 iOS 그대로: lat/lng/setAt 유한·범위, 라벨 공백 아님, origin이 있으면 그 필드도 유한·`accuracy > 0`.

### 13-2. 판정과 유효 좌표 (`location/ManualLocationJudge.kt`·`location/EffectiveLocation.kt`)

- `ManualLocationJudge(manual, location, now, notify, minIntervalSeconds = 30)`: `suspend fun run()` — iOS `run()` 그대로: 수동 없음 → 무시 / `origin == null` → `undecidable`(측위 비용 0) / 권한 `Fine` 아니면 → `undecidable`(팝업 없음) / `captured = revision` → `location.currentFix(force = true, silent = true)`(`ManualFix?`, 실패 null) → `judgeManualLocation` → **CAS**(revision 다르면 폐기) → `drop`이면 `clear()` + `notify(autoCleared)`(생성자 인자 `notify: (String) -> Unit`, 프로덕션은 `AppNotices::post` — JVM 단언이 싱글턴에 의존하지 않는다, 웹·iOS `announcer` 주입 동형), 그 밖은 `setVerdict`. **어느 갈래로 끝나든 결과를 남긴다**; 단 `CancellationException`은 그대로 통과하고 그 경로에서는 결과를 남기지 않는다(미완이지 실패가 아니다). **재진입 가드**: 진행 중 `run()`은 그 Job을 `join`한다(동시 트리거 2회 = 측위 1회·통지 최대 1회). **과발화 억제**: 마지막 실행 뒤 `minIntervalSeconds` 안의 재호출은 건너뛴다(회전·다크모드·앱 언어 변경마다 액티비티가 재생성돼 `ON_START`가 다시 오기 때문 — `configChanges` 없음; force 조회는 이 억제를 받지 않는다).
- **트리거 3종**(iOS 미러): ① 앱 시작·② 전경 복귀 = `MainActivity` `Lifecycle.Event.ON_START`(`lifecycleScope`, 단일 액티비티라 프로세스 전경과 같다; 콜드 런치의 첫 ON_START가 ①을 겸한다) ③ `force` 조회(`EffectiveLocation.coordinate(force = true)`). ON_START 배선은 뷰 계층이라 **소스 가드**로 잠근다(`MainActivity.kt`가 `manualLocationJudge.run()`을 `ON_START`에서 부른다).
- **판정 측위는 화면이 요청하지 않은 측위**(CLAUDE.md `silent` 규칙): `LocationStore.currentCoordinate(..., silent = false)`에 인자를 흘려 `lastFixFailed`를 세우는 세 자리(Coarse·기기 위치 꺼짐·취득 실패)가 그것을 보고, `currentFix(force, silent)`는 통과 호출이다 — `silent`면 실패에 `lastFixFailed`를 세우지 않는다(표시줄의 "이 세션에서 확정된 실패" 정의가 넓어지지 않게). 성공 fix의 `stored` 갱신은 그대로(더 나은 좌표는 표시에도 좋다). 반환 `ManualFix(lat, lng, accuracy, at = 지금 epoch − 나이)`. 지정 시점 origin은 `silent = false`.
- `EffectiveLocation(location, manual, judge)` — **앱 층의 좌표 진입점은 이것뿐**(iOS `LocationService` 안에 수동 우선이 있어 우회 경로가 없는 것과 같은 효과): `suspend fun coordinate(force): NearbyCoord`(`force`면 `judge.run()` 먼저 → 수동 있으면 그 좌표 → 아니면 `location.currentCoordinate(force)`) · `suspend fun coordinateForRanking(): NearbyCoord?`(수동 있으면 그 좌표 — **측위 없음** — 아니면 `location.coordinateForRanking()`) · `fun nearbyCoordinateSource(): NearbyCoordinateSource.Current`(어댑터·오류 번역을 `LocationStore`에서 **옮겨 온다**; `LocationStore.nearbyCoordinateSource()`는 삭제). `LocationStore.coordinateForRanking()`은 `gpsCoordinateForRanking()`으로 개명(GPS 층임을 이름이 말한다). **소비자 전수**(웹 `effective-location` 동형): 내 주변 허브 진입 화면(`MainActivity` `nearbyFactory`의 `current`) · 검색 거리 가중(`SearchViewModel.coordinate`) · 길찾기 `EndpointLocator.currentCoordinate`(출발지)와 **`coordinateForRanking`(끝점 후보 근접 가중 + 길찾기 탭 진입 주소 병기 — 수동이면 측위 0)** · 채팅 앵커(M6 통합 `45c8c4ab` 뒤 전파 완료 — `ChatLocation.prime` = `EffectiveLocation.prime(soft 상한)`(수동이면 측위 0), `last` = 동기 `EffectiveLocation.last()`(수동 > GPS 저장 좌표; M6 spec §4-1의 "M2c 수동 위치 자리")). ⚠ `last()`는 동기라 `awaitHydrated()`를 못 지나고 장소 채팅은 `prime`을 건너뛴다(M6 계약: 장소 채팅은 측위 0) — 그래서 **`last()`가 hydration 미완이면 멱등·`@Synchronized` `hydrate()`를 스스로 먼저 부른다**(콜드 스타트 직후 장소 채팅 첫 전송 창에서만 디스크 1회, 그 뒤 no-op; M6 파일 무변경, 설계 리뷰 4차 M13 — `EffectiveLocationTest`가 잠근다). **표시줄 주소 조회(`coordinateForDisplay`)와 실시간 안내는 실좌표**. 소스 가드는 **수신자 기준**: `location/` 밖에서 `AppConfig.locationStore.` 참조·`LocationStore` 타입 참조가 없다(`EndpointLocator.currentCoordinate`는 인터페이스 이름이라 문언 가드에 걸리면 안 된다). `LocationStoreLocator(effective: EffectiveLocation, permissions)`로 생성자를 바꾸고 `directionsLocator()`가 `AppConfig.effectiveLocation`을 준다.

### 13-3. 지정 화면 — M3 끝점 검색 재사용 (`directions/EndpointPicker.kt` 추출)

- 현행 `EndpointSearchContent(vm: DirectionsViewModel, p)`는 뷰모델에 묶여 있어 허브가 못 연다. **끝점 검색 로직을 `EndpointPicker`로 추출**(iOS `EndpointSearchModel` 동형): `class EndpointPicker(search: SearchService, store: RecentSearchStore, strings, io, scope, ranking: suspend () -> NearbyCoord?, onSelect: (DirectionsEndpoint, DirectionsFieldTarget) -> Unit)` — `state: StateFlow<EndpointSearchState?>`·`consumedCandidateRevision`·`open(target)`·`close()`(진행 중 후보 검색·지오코딩 잡을 취소하고 picker 상태를 비운다; **착지 발급만 `DirectionsViewModel.closePicker` 래퍼에 남는다**)·`submitCandidates()`(`ranking()`으로 근접 가중 — M3 동작 유지)·`selectPlace/selectRecentEndpoint/selectAddress/selectCurrent`·`removeRecentEndpoint/togglePinRecentEndpoint/clearRecentEndpoints`. `selectCurrent`의 가드는 `target ∈ {from, manualLocation}`, **`refreshCurrentLocation()` 부수효과는 VM의 `onSelect`(from일 때만)로 옮긴다**(지정 화면에서 해제 직후 8초 강제 측위·`isRefreshingCurrent` 토글 금지). `DirectionsViewModel`은 합성 + 기존 공개 함수 위임(M3 호출부·테스트 무변경 — `DirectionsViewModelTest`가 `setEndpoint` 이중 기록 금지·`select` 뒤 착지·`closePicker` 취소를 단언한다). `EndpointSearchContent(picker: EndpointPicker, p, onBack: () -> Unit)` — 껍데기(`AppScreenScaffold`·단일 `StatusLine(p.notice)`)는 그대로 콘텐츠 안에 있고 `onBack`이 인자가 된다(길찾기는 `closePicker`, 지정 화면은 진행 중 잡 취소 + pop). 지정 화면의 진행 통지는 **picker의 단일 창구**로 흘린다(`EndpointPicker.postNotice(text)` — 두 번째 `StatusLine` 금지). `directions/DirectionsStrings.kt` `stringId` 표에 `manualLocation.pickTitle`·`manualLocation.useGps`·`manualLocation.locating` 추가(기존 가드 `DirectionsSourceGuardTest`).
- `DirectionsFieldTarget`에 **`manualLocation`** 추가(iOS 동형). exhaustive `when` 전수: `recentScope` → `to`(전용 스코프 없음 — 자주 가는 곳이 지금 서 있는 곳의 후보이기도 하다) · 화면 제목 → `manualLocation.pickTitle` · "현재 위치 사용" 자리 버튼 → `manualLocation.useGps`("현재 위치로 되돌리기") · `fieldText`(라벨·미확정 문구) → **도달 불가**(`error(...)` — 호출부가 리터럴 enum이라 프로그래머 오류) · `LandingTarget.Field(manualLocation)` 착지 → requester null + `Log.w`(착지 하나로 릴리스 앱을 죽이지 않는다 — 착지 소비부의 `runCatching` 관용구) — 길찾기 폼에 그 필드가 없다. ⚠ 이분 삼항 금지(iOS 실사고: 제목이 "도착지 검색"으로 떨어짐).
- 허브 진입점: 표시줄 버튼 → `AppRoot`가 `returnFocus.slot.remember(LOCATION_BAR_KEY)` 뒤 `ManualLocationRoute`(스택, `location/ManualLocationPickerScreen.kt`, 등록 한 줄; 팩토리는 길찾기 관용구대로 `location/`이 앱 컨텍스트로 스스로 만든다 — `LocationStore`를 잡는 자리가 `location/` 안에 남는다). `ManualLocationPickerViewModel`이 `EndpointPicker`를 `manualLocation` 타깃으로 열고(**`closesOnSelect = false`** — 확정 뒤에도 화면을 닫지 않아 진행 통지가 이 화면의 `StatusLine`에서 난다; 길찾기는 `true`) `onSelect`에서 **`commit`(웹 `commitManual` 동형)**: in-flight 가드(진행 중 잡이 곧 상태 — 별도 `isCommitting` 플로우 없음, 연타 무시) → `Current`면 `manual.clear()` 즉시 / `Place(label, lat, lng, labelRoman)`면 `authorization() == Fine`일 때만 진행 통지 `manualLocation.locating`(측위가 실제로 일어나는 갈래에서만 — 웹 `commitManual` 동형) → `origin = location.currentFix(force = true, silent = false)?.takeIf { isEligibleManualFix(it, now) }`, `Fine`이 아니면 통지 없이 `origin = null`(**권한 다이얼로그 없음** — 판정 37) → `manual.set(...)` → **그 다음** pop. pop 신호는 콜백이 아니라 **`done: StateFlow<Boolean>`**이다 — ViewModel은 액티비티 재생성을 넘어 살아 `NavController`를 붙들면 옛 컨트롤러에 pop하므로, 화면이 `done`을 읽어 자기 `onBack`을 부른다. 실내 8초는 진행 통지가 말한다. 뒤로(취소)는 `cancel()`이 진행 중 잡을 끊고 pop(지정 안 됨); 시스템 뒤로는 `NavHost`가 pop하고 ViewModel 소거가 같은 잡을 끊는다. 허브 복귀 착지는 표시줄 버튼(`NearbyHubScreen`의 복귀 resolver가 `LOCATION_BAR_KEY`를 그 버튼 requester로 푼다 — kind 키만 알던 resolver 확장), 그때 라벨은 이미 확정돼 있다(pop이 `set` 뒤라 "표시줄 라벨 자체가 결과"가 성립). 지정 확인 통지는 없다(iOS 동형). 표시줄이 읽는 `ManualLocationStore`는 `AppFactories.manualLocation`으로 허브에 주입한다(화면이 `AppConfig`를 직접 잡지 않는다).
- 주소 후보의 좌표 실패는 M3 `directions.coordError` 한 문구로 합친다(iOS 동형; 웹의 `manualLocation.addrNoCoord`는 웹 전용으로 남는다).

### 13-4. 라벨 (`location/LocationBar.kt` 확장)

- `manualLocationLabel(manual, verdict, lang, accessible, verified: (String) -> String, unverifiable: (String) -> String): String?` — 수동 없으면 null; `isManualLocationVerified(manual, verdict)`면 `manualLocation.manual(label)`, 아니면 `manualLocation.manualUnverifiable(label)`; 라벨은 `manualLocationBilingualName`(비-ko `labelRoman` 1순위, 시각 `display`/낭독 `primary`). **표시줄·길찾기 출발지 필드가 이 한 함수를 쓴다**.
- 표시줄 최종 라벨은 **순수 함수 하나**가 낸다: `locationBarLabel(input, manual, verdict, lang, words): LineText` — 수동이면 `manualLocationLabel`, 아니면 §12-4 갈래(1a·1b·2·3·4), 끝에 `", {manualLocation.pickTitle}"` 꼬리(상태만 이름으로 쓰면 "현재 위치, 버튼"으로 읽혀 누르면 무엇이 되는지 단서가 0). 행은 **버튼**(`landingTarget`), `contentDescription`은 낭독형이 시각형과 다를 때만(M2b 규율). `CurrentAddressStore.ensureLoaded`는 **수동이 없을 때만**이고 키는 `LaunchedEffect(manual == null)`(iOS `.task(id:)` 동형 — 자동 해제 직후 "그럼 지금 어디냐"에 주소가 따라온다).
- 길찾기 출발지 필드(M3 `DirectionsViewModel.currentLocationText`, additive): 수동이 있으면 `manualLocationLabel` 문장, 아니면 현행. `directions.useCurrentLocation` 버튼 이름은 그대로(웹 근거 동형).
- 둘러보기: **문장과 완료 통지가 같은 값**을 읽는다 — `usedManualCoordinate(coord, manual) = manual != null && coord.lat == manual.lat && coord.lng == manual.lng`(정확 비교: 수동이면 `coordinate()`가 그 좌표를 그대로 돌려준다)를 **조회 시점**에 `fetchAround`가 `AroundPayload.usedManual`로 굳히고, 헤딩·통지는 그 값만 읽는다(지금의 수동 위치와 다시 비교하지 않는다 — 자동 해제 뒤에도 화면에 남은 데이터의 기준은 수동 좌표이고 헤딩은 반복해서 읽히는 문장; 구현 리뷰 MAJOR). 장소 앵커 화면은 `manual`을 넘기지 않는다(같은 장소를 지정해 둔 경우 좌표가 비트 단위로 같아 "지정한 위치 기준"으로 오독). 위치 문장 `aroundHereManual`/`…ManualNoPlace`, 완료 통지 `android.nearby.aroundLoadedManual`(iOS `AroundNearbyView` 동형 — "수동 위치일 때 '현재 위치'라고 알리지 않는다"). `NearbyKinds.around(service, strings, manual: () -> ManualLocation?)`(팩토리가 앵커 없을 때만 실제 스토어를 넘긴다 — 화면·ViewModel은 수동 위치를 모른다), `NearbyStrings.aroundLoadedManual`. 유니버스 술어에 `usedManualCoordinate(`·`aroundHereResId(`를 더해 그 배선에서도 스캔에 든다.
- **금지 표현 가드**(웹 `manual-location-copy` 축의 안드로이드판, `AppSourceGuardTest`, 하드코딩 없음): ① 소비자 유니버스 = `:app` 소스 중 `EffectiveLocation`(`effectiveLocation.` 포함)·`nearbyCoordinateSource(`·`manualLocationLabel(`·`ManualLocationStore`·`usedManualCoordinate(`·`aroundHereResId(`를 참조하는 파일(표시줄·허브·내 주변·검색·길찾기 VM이 자동으로 든다; `LocationBar.kt`가 유니버스에 드는지 단언 — D20② 재발 방지) ② GPS 문구 키 = `values/strings.xml`에서 `manualLocation_gps`의 값("현재 위치")을 **담는 모든 리소스 이름**(코드에 문구를 박지 않는다) ③ 유니버스 파일이 GPS 키를 참조하면 같은 파일이 수동 분기 키(`manualLocation_manual*`·`aroundHereManual*`·`aroundLoadedManual`)를 참조하거나, 그 키가 **근거를 적은 `KNOWN_UNBRANCHED`**에 있어야 통과 — 초판 목록은 웹의 근거 문장을 옮긴다(`directions_useCurrentLocation`·`directions_locating`·`directions_currentLocation*`·`directions_refreshingCurrent`·`manualLocation_gps*`·`manualLocation_locating`·`manualLocation_guideStartsFromCurrent`·`android_nearby_aroundHere*`·`android_common_locationFailed`·`directions_geoError`(웹 근거: 수동이 유지되는 동안 좌표 해석이 성공해 닿지 않고, 닿았다면 직전 판정이 해제한 뒤다)·`android_common_outOfCoverage`·`android_nearby_whereAmIChat`·`manualLocation_useGps`(지정 화면 전용 문구 — 수동 맥락에서만 렌더; 추출된 `EndpointPicker`가 유니버스에 들지 않으면 목록에서 뺀다 — 축 ④가 죽은 항목을 잡는다)) ④ 어느 파일도 참조하지 않는 죽은 예외 항목은 실패 ⑤ **수동 분기 키 전수**(안드로이드가 소비하는 `manualLocation_manual*`·`android_*Manual*` 5키 — 웹 화면의 미러 키 `around_*Manual`은 생성물에 있을 뿐 소비자가 없어 제외)가 6로케일에 있고 각 키를 최소 한 파일이 참조하며 GPS 문구를 되풀이하지 않는다(웹 마지막 describe 포팅 — `aroundLoadedManual`이 놓여 있던 상태를 잡는다). 구현의 예외 목록은 축 ④로 다듬어 12키(`manualLocation_{locating,gps,gpsNear}`·`directions_{currentLocation,currentLocationNear,refreshingCurrent,locating,geoError}`·`android_nearby_aroundHere{,NoPlace}`·`android_common_{locationFailed,outOfCoverage}`) — 초판의 `guideStartsFromCurrent`·`whereAmIChat`·`useGps`는 유니버스 밖이라 뺐다.

### 13-5. 자동 해제 통지 — 앱 통지와 화면 통지의 병합 (`a11y/AppNotices.kt`)
...
- `AppNotices`: 앱 수명 싱글턴 `pending: StateFlow<Notice?>`(**덮이지 않는 큐 1칸** — 소비될 때까지 화면 통지가 밀어내지 못한다), `post(text, spoken, haptic)`(§14: 자동 해제 = `attention`, 언어 적용 = `success`; 앱 통지의 `haptic`이 null이면 화면 것), `claim(seq)`·`consume(seq)`·`restore(seq)`.
- `StatusLine`이 `AppNotices`를 **스스로** 읽는다(화면마다 배선하지 않는다 — 총체성). 병합은 순수 함수 `mergeNotices(screen: Notice, app: Notice?, nextSeq: Int): Notice` — 앱 통지가 대기 중이면 **한 문장으로 합친다**(`joinText(app.text, screen.text)`, 앱 통지가 앞; 낭독형도 같은 순서) 그리고 결과에 **새 단조 seq**를 찍는다(두 카운터를 비교하지 않는다; 같은 수의 텍스트 교체가 `LaunchedEffect(seq)`를 건너뛰는 침묵 방지). 화면 통지 없이 앱 통지만 오면 그것만. 표시 뒤 `consume`. **`StatusLine`의 초기 seq 억제(재마운트 재발화 방지)는 화면 통지만으로 계산한다**(`initialSeq = remember { screen.seq }`) — 앱 통지가 붙은 병합 결과는 **항상 새 seq**를 받아 `LaunchedEffect` 경로를 반드시 지나고, **`consume(seq)`는 그 경로의 끝(발화 성공 시점)에서만** 부른다(메모리 `once-only-warning-delivery-contract` — latch는 발화 성공 시점에; 첫 컴포지션 직전에 도착한 pending도 발화 없이 소비되지 않는다). `nextSeq`는 `StatusLine` 안의 **발화 단위 세대**(`SpokenUnit(rev, text, app)`, `rev`는 0에서 시작하는 단조 카운터 — 화면 seq 공간과 무관해 충돌하지 않는다; 화면 seq를 시작값으로 쓰면 병합 seq가 이미 쓰인 화면 seq와 같아져 발화·소비 없이 큐가 영구 점유된다 — 구현 리뷰 BLOCKER)이고, 새 단위는 화면 통지가 바뀌었거나 새로 집은 앱 통지가 있을 때만 만든다(앱 통지가 소비돼 `pending`이 null로 돌아가는 변화는 단위를 만들지 않는다 — 화면 문장이 다시 낭독되지 않게; 아직 발화되지 않은 단위의 앱 통지는 다음 단위로 넘겨 잃지 않는다). **소유자는 하나**: 내비게이션 전환 프레임에는 나가는 화면과 들어오는 화면의 `StatusLine`이 동시에 컴포즈되므로(`NavController`는 최상단 엔트리 하나에만 `RESUMED`를 주고 전환 중·아래 엔트리는 `STARTED`가 상한), 앱 통지는 `LocalLifecycleOwner`가 `RESUMED`이고 **모달이 떠 있지 않은**(`a11y/LocalModalOpen`, 다이얼로그를 여는 화면이 참으로 제공 — Compose `Dialog`는 라이프사이클을 바꾸지 않아 뒤 화면이 계속 RESUMED이고, 모달 뒤 라이브 리전은 읽히지 않을 수 있어 `consume`이 "낭독됐다"가 아니라 "텍스트를 썼다"로 퇴화한다; 큐는 덮이지 않으므로 모달을 닫는 순간 같은 화면이 집어 읽는다) `StatusLine`만 `claim(seq)`해 가져간다. **그 판정은 관측 가능한 Compose 상태여야 한다** — `Lifecycle.currentState`를 컴포지션에서 그냥 읽으면 구독이 없어 claim이 한 번만 시도되고, 재생성 직후 첫 컴포지션은 엔트리가 아직 `STARTED`라 언어 변경 통지가 영영 침묵한다. `StatusLine`이 `LifecycleEventObserver`(`DisposableEffect(owner)`)로 `resumed: MutableState<Boolean>`을 유지하고(새 의존성 0 — `androidx.lifecycle.compose.LocalLifecycleOwner`는 `navigation-compose`의 전이 의존으로 클래스패스에 있다; 끊기면 `lifecycle-runtime-compose` 한 줄을 선언한다), **claim은 컴포지션 안이 아니라 발화 단위 효과 `LaunchedEffect(screen.seq, app?.seq, resumed, spokenRev)`에서** 하고, 그 효과가 새 발화 단위 `SpokenUnit(rev, text, app)`를 만든다(적용되지 않은 컴포지션의 `remember` 블록에서 집으면 폐기될 때 복구 경로가 돌지 않아 유일한 사본이 소실된다). 발화 효과는 **단위 세대 `rev`에만** 키를 건다(`LaunchedEffect(unit.rev)`) — 단위는 언제나 claim을 마친 뒤에 만들어지므로 화면 통지가 앱 통지와 같은 프레임에 와도 화면 단독 문장이 먼저 나가지 않고, 프레임 클록·디스패치 순서에 기대는 논거는 없다. **네 번째 상태 `spokenRev`**(마지막으로 발화를 마친 세대): 발화 효과의 끝에서 `spokenRev = rev` + `consume`(발화 성공 latch). 합치는 화면 문장은 **아직 발화되지 않은 것**(`screenChanged`)뿐 — 이미 읽힌 화면 문장을 앱 통지 뒤에 다시 붙이면 뻔한 꼬리다(CLAUDE.md; 앱 통지 단독이면 그것만). 같은 단위의 앱 통지는 `spokenRev < rev`인 동안만 다음 단위로 넘겨지고 발화 뒤에는 뗀다(이후 화면 통지에 다시 붙지 않는다); `spokenRev`가 키에 드는 것은 발화가 끝난 뒤 큐의 다음 통지를 집기 위해서다. 되돌림은 `DisposableEffect(Unit)`의 `onDispose`에서 **발화 여부**로 가른다(`unit.app != null && spokenRev < unit.rev`면 `restore` — 키 변경 dispose가 아니라 컴포지션 이탈만). **M2c에서 구현·통합됨**(`a11y/A11y.kt`·`AppNotices.kt`, ATF `StatusLineA11yTest`). `AppNotices`의 세 연산과 중간 상태(`@Synchronized`): `claim(seq)`는 `pending → claimed`(같은 seq이고 `claimed`가 비었을 때만, 아니면 null) · `consume(seq)`는 `claimed`를 비우고 `queue` 머리를 `pending`으로 승격 · `restore(seq)`는 `claimed → pending`(싱글턴 `claimed`가 그 seq일 때만 — `claim`과 대칭 가드; 그 사이 `pending`에 앉은 통지는 `queue` 앞으로 민다 — **되돌아온 것이 먼저 낭독된다**). 병합 문장의 `haptic`은 **앱 통지의 종류가 이긴다**(자동 해제 = 주의). ATF는 프로세스를 공유하므로 테스트 전용 `AppNotices.reset()`을 둔다. 언어 변경처럼 액티비티 재생성을 넘겨야 하는 통지가 이 큐를 탄다(§14-2). CLAUDE.md 선례("같은 커밋에 두 문장이 나는 자리는 대기 꼬리로 합친다") 동형 — 새로고침 중 drop이 나면 "이동이 감지되어 지정한 위치를 해제했습니다, 주변 역 3곳"으로 한 번에.
- 허브에 `StatusLine`을 둔다(지금 없는 유일한 화면 — `BusRouteStopsScreen`은 `NearbyShell`을 지나 이미 있다). **소스 가드**: `AppScreenScaffold(`를 부르는 파일은 `StatusLine(`도 부른다(정의 파일 `a11y/AppTopBar.kt`만 allowlist; §14의 설정·정보 출처 화면도 같은 판정을 받아 자동 해제 통지가 그 화면에서도 들린다).

### 13-6. 테스트·실기기

- JVM: `ManualLocationStore`(왕복·손상 폐기·`set`이 verdict 초기화·revision 단조·`clear`) · `ManualLocationJudge`(수동 없음 → 무호출, origin 없음 → undecidable 측위 0, 권한 아님 → undecidable 팝업 0, keep/drop/undecidable, **CAS**, drop → clear + 앱 통지 1회, **동시 트리거 2회 → 측위 1회·통지 1회**, 30초 안 재호출 건너뜀, 취소는 통과·결과 없음) · `EffectiveLocation`(수동 우선, force가 judge를 먼저, drop 뒤 GPS로, ranking 수동이면 측위 0) · `LocationStore.currentFix`(성공 accuracy·at, 실패 null, `silent` 실패는 `lastFixFailed` 불변) · `manualLocationLabel`·`locationBarLabel`(수동 갈래·꼬리·병기·낭독형 동일 시 null) · `usedManualCoordinate` + 둘러보기 통지 `aroundLoadedManual` · `DirectionsViewModel.fieldText` 수동 분기 · `EndpointPicker`(ranking 가중 유지·`selectCurrent` 가드·close가 착지를 발급하지 않음) · **M3 기존 테스트 전량 초록** · `ManualLocationPickerViewModel.commit`(in-flight·진행 통지·set 뒤 onDone·Fine 아니면 origin null·취소 시 set 없음) · `mergeNotices`(합침 순서·새 seq·앱 통지만·소비 뒤 화면만) · `CurrentAddressStore`(수동 해제 전이 재실행은 ATF) · hydration 전 `coordinateForRanking()`이 join 뒤 수동을 돌려준다 · `StatusLine` 첫 컴포지션에 pending이 있어도 발화 뒤 소비(ATF) · 소스 가드 5종(금지 표현 유도 가드(축 ①~⑤)·`ON_START` 배선·`LocationStore` 수신자 참조 위치·Scaffold↔StatusLine 총체성·착지 순서 가드 `selectable`).
- ATF: 허브 표시줄이 버튼이고 라벨 꼬리 "위치 지정하기"; 지정 화면 제목·"현재 위치로 되돌리기"; 수동 해제 전이에 `ensureLoaded` 재실행(스텁 카운트).
- 실기기(§9 이어서): 23 표시줄 버튼 → 지정 → 진행 통지 → 허브 복귀 착지가 표시줄이고 "지정한 위치, X, 위치 지정하기" 24 지정 뒤 내 주변 지하철이 그 좌표 기준 25 100m 이동 뒤 전경 복귀 → 자동 해제 통지(다른 화면에 있어도) 26 길찾기 출발지 필드 "출발지, 지정한 위치, X"·끝점 후보가 지정 좌표 기준 정렬.

### 13-7. 판정 (§12 이어서)

32. **끝점 검색은 `EndpointPicker` 추출로 재사용**(복붙·허브 전용 검색 화면 금지, iOS `EndpointSearchModel` 동형). M3 종료 상태라 `directions/` 파일을 이 세션이 고친다(코디네이터 지시) — 공개 함수 위임으로 M3 호출부·테스트 무변경.
33. **자동 해제 통지는 `AppNotices`(덮이지 않는 큐) → 화면 `StatusLine`이 한 문장으로 병합**(순수 함수·새 단조 seq) — 화면당 라이브 리전 하나 유지, 앱 수준 별도 리전·`announceForAccessibility` 금지. `StatusLine`이 스스로 읽어 총체적.
34. **판정 트리거는 `ON_START`**(앱 시작 겸 전경 복귀, 30초 디바운스로 재생성 과발화 억제) + force 조회. `ProcessLifecycleOwner` 의존성을 더하지 않는다.
35. **수동 상태에서 GPS 측위를 하는 자리는 판정(`run`)뿐** — ranking 경로도 수동 우선이라 끝점 후보 정렬·길찾기 진입 주소 병기가 측위를 일으키지 않는다. 표시줄 주소 조회는 수동이 없을 때만.
36. **`guideStartsFromCurrent`는 M4** — 판정 재료(`usedManualOrigin`)는 iOS처럼 **조회 시점**에 기록해야 하므로 M4가 `runQuery`를 다시 연다.
37. **지정 시점에 권한을 묻지 않는다** — `Fine`일 때만 origin을 잡고 아니면 팝업 없이 `origin = null`(라벨은 "(위치 확인 불가)" — 사실 그대로). §4의 권한 요청 자리(세 곳)를 늘리지 않는다. iOS는 이 자리에서 묻는다 → **역이식 후보**로 코디네이터에 보고.
38. **앱 층 좌표 진입점은 `EffectiveLocation`뿐** — `LocationStore`의 어댑터를 옮기고 ranking을 `gps*`로 개명, 소스 가드로 잠근다(복붙 진입점 하나 누락 = 그 화면만 수동 무시, 화면으로 반증 불가).

적대적 설계 리뷰 판정(§13, `review-m2c-design.md`): 1차 REQUEST_CHANGES(BLOCKER 0·MAJOR 6·MINOR 12·NIT 6) → 24건 전부 반영(MAJOR-5 → 판정 37, MAJOR-6 → 판정 33 병합 규칙·§13-5, MAJOR-3·MINOR-6·7 → 판정 38, 나머지 본문), 기각 0 → 2차 diff APPROVE_WITH_CHANGES(BLOCKER 0·MAJOR 2·MINOR 8·NIT 6 — hydration 레이스·`StatusLine` 초기 seq와 앱 통지 소비·수동 키 사문화 축·`geoError` 등재·`manual` 출처·가드 수신자 기준·picker 껍데기 `onBack`/`postNotice`·`error` 강도·`silent` 위치·진행 통지 갈래·허브만·allowlist·`notify` 주입·`close` 취소) → 16건 전부 반영, 기각 0. **설계 확정.**

구현 리뷰 판정(§13, `review-m2c-spec.md`·`review-m2c-quality.md`): spec-compliance REQUEST_CHANGES(BLOCKER 3·MAJOR 1·MINOR 5·NIT 4 — 컴파일 불가 기준 SHA·채팅 앵커 우회·`StatusLine` 병합 seq 충돌·ATF 3종 부재·hydration 전 주소 조회·채팅 표시줄 복귀 착지·가드 술어·`Log.w`·`selectable`) → BLOCKER 3 전부 해소(앞 둘은 리뷰 중 커밋, 셋째는 발화 단위 세대 + §13-5 claim 설계 구현), MAJOR-1 ATF 3종 추가, MINOR 4 반영·1 유예(채팅 탭 표시줄 버튼의 pop 복귀 착지는 채팅 화면의 착지 규칙(M6 `ChatFocusTargets`) 몫 — 코디네이터 보고), NIT 본문. code-quality REQUEST_CHANGES(BLOCKER 1·MAJOR 3·MINOR 12·NIT 6 — 같은 seq 충돌·소비 뒤 화면 문장 재발화·채팅 표시줄 복귀 착지·`usedManual` 수명·앵커 좌표 일치·force 직렬 판정·디바운스 예산·hydration 전 조회·복귀 키 이중 관리·가드 유니버스/파일 단위/키 판별/별칭/자리 면제·`set` 실패 무시·주소 후보 미검증·NIT) → BLOCKER·MAJOR 전부 반영, MINOR 11 반영·1 판정 유지(m2 force 조회의 판정 직렬 대기는 spec §13-2 판정 — "지금 어디 있는가"를 다시 묻는 행동이라 실기기 확인 대상으로 남긴다), NIT 5 반영·1 불가(생성물 문자열 2키는 웹 카탈로그 수입분이라 삭제 대상이 아니다). 이후 재게이트 초록으로 통합.

## 14. 설정 화면 (2026-09-16 추가)

> 코디네이터 지시(M2c에 이어). 동작 정본 iOS `SettingsView.swift`·`DataSourcesView.swift`·`TitleMenu.swift`(진입)·`AppLanguage.swift`·`ResultHaptic.swift`. 범위 = **언어 · 받아쓰기 방식(탭/홀드) · 결과 진동 스위치(실험판) · 정보 출처 · 개인정보 처리방침·문제 신고 링크**. 업데이트 이력은 **유예**(판정 43). 테마·듣기 속도·자동차 청취자·좌우 안내음·체중·AI 동의는 소비 마일스톤(M4·M6)이 이 화면에 **행을 additive로 더한다**(자리와 관용구만 여기서 정한다). 문자열: §14가 쓰는 `android.settings.*` 9키(`title`·`language`·`languageApplied`·`dictationStyle`·`dictationTap`·`dictationHold`·`trendHaptics`·`reportProblem`·`themeSystem`("시스템 설정 따름" — 언어 목록 첫 행에 재사용)) + `android.common.{privacyPolicy,noAppToOpen}` + 정보 출처 `dataSources.title/osmLicense/osmLink/osmCopyRequest`·`chat.source.*` 16 전수 실재. **신설 1키**: `android.settings.resultHapticsFooter`(android-extra 6로케일, 이 세션이 6개 문안을 쓴다) — iOS 푸터를 오버라이드하면 `android-strings-drift`의 `INTENDED_DIFFERENCES`(`src/**`, 수정 금지) 등재가 필요해 **iOS 키가 없는 새 키**로 우회한다(생성기는 `ios.X`가 없는 키의 드리프트 검사를 건너뛴다). `dataSources.walkHealth`는 안드로이드 카탈로그에 없고 그 행이 고지하는 걸음 요약 자체가 M4라 **M4가 행과 키를 함께 더한다**. 푸터는 위 신설 키에 지금 참인 것만 적는다(내 주변·검색 결과 진동 + "화면이 켜져 있고 기기의 촉각 피드백 설정이 켜져 있을 때만" — M4가 안내 문장을 더한다; 조건 문장은 헌장이 유지하는 정보다).

### 14-1. 설정 저장소·진입·구조 (`settings/` 패키지, 이 세션 소유)

- **`settings/SettingsStore(store: KeyValueStore)`**(앱 싱글턴 `AppConfig.settings`) — 설정 값의 **단일 소유자**: `language: StateFlow<String?>`(null = 시스템 따름; 값은 `AppLocale.supported` 안 코드만, 밖이면 null — 순수 `localeOverride(stored): String?`가 3분기를 가른다) · `dictationStyle: StateFlow<String>`(`"tapToggle"` 기본/`"hold"`, 키 `dictationStyle` = iOS `DictationStyle.key`·값 rawValue 동일 — M6가 같은 키를 읽는다) · `resultHapticsEnabled: StateFlow<Boolean>`(키 `TrendHaptics.storageKey`(:kit `trendHapticsEnabled`), 기본 false). 최근 검색·수동 위치·설정은 **한 파일**(iOS `UserDefaults.standard` 동형; 채팅 AI 동의는 M6의 `gildongmu.chat` 별도 파일 — 시작 경로에 없어 그대로): `SharedPreferencesStore`의 기본 파일 이름을 `gildongmu.recent`에서 **`gildongmu`로 개명**한다(최근 검색·수동 위치·설정이 한 파일 — 미출시라 마이그레이션 없음, KDoc 갱신). **언어 키는 `attachBaseContext`에서 동기로 읽는다**(첫 프레임이 옳은 언어여야 한다). `SharedPreferences`는 파일 단위로 로드되므로 이 한 번의 동기 읽기로 한 파일에 든 최근 검색·수동 위치·설정이 **함께** 파싱된다 — 시작 경로 비용으로 수용(`SharedPreferencesStore` KDoc 갱신). `GildongmuApplication`의 IO `hydrate()`와 다른 키의 IO 첫 읽기가 남는 이유는 비용이 아니라 **순서**(`awaitHydrated()` 불변식)다.
- 진입: iOS는 탭 제목 메뉴의 "설정". 안드로이드는 탭 루트 상단 바의 **기존 `actions` 슬롯**에 공용 `SettingsAction(onOpen)`(아이콘 버튼, `contentDescription = android.settings.title`, `tapTarget`, 새로고침 등 화면 고유 액션 **뒤** — 이 순서 규칙은 §3-1 상단 바 계약에 한 줄 추가) — 검색·내 주변 허브·길찾기(M3 파일, 한 줄 additive)·채팅 탭(M6 `ChatTabScreen`이 이미 `AppScreenScaffold`를 쓴다 — M6 통합 `45c8c4ab` 뒤라 그 파일에 같은 한 줄 additive)은 이 세션이 배선한다(4탭 전부 도달). → `SettingsRoute`(스택, 등록 한 줄). pop 복귀 착지는 그 버튼: 내 주변 허브는 기존 `ReturnFocusViewModel` 슬롯(키 `"settings"`), 검색도 `AppRoot`의 `ReturnFocusViewModel`(허브·길찾기·채팅과 같은 갈래 — `SearchViewModel.takeReturnFocus`는 결과 행 전용 게이트(`outcome != null`)라 결과 없는 첫 진입에서 키를 버린다; §3-1 소비 규칙은 결과 행 키에 한한 그대로), 길찾기·채팅은 `AppRoot`에 `ReturnFocusViewModel`을 붙인다(`DirectionsScreen`·`ChatTabScreen`이 `takeReturnFocus`를 받아 버튼에 착지 — M3·M6 파일 additive). 설정 화면의 화면 통지는 화면 로컬 `remember { mutableStateOf(Notice(0, "")) }`(링크 열기 실패 `noAppToOpen`만)이고 정보 출처 화면도 같은 화면 로컬 `Notice`(링크 2개의 `tryStartActivity` 실패 → `noAppToOpen` — 화면 변화 없는 활성화는 통지가 유일한 증거) — 행 목록이 순수 함수인 것과 같은 이유로 ViewModel을 두지 않는다.
- 화면: `AppScreenScaffold(title = android.settings.title, onBack, titleFocus)`(재생성 뒤 착지를 제목에 **배선**한다 — `titleFocus` 없이는 제목이 `focusable`조차 아니다; 재생성 직후 한 프레임 뒤 `requestFocus`) + `StatusLine` + 순서 = iOS: ① 언어 ② 받아쓰기 방식 ③ (실험판) 결과 진동 스위치 + 푸터 ③-1 체중(kg) 입력(M4 종료로 코디네이터가 인계 — iOS와 같은 자리, `OutlinedTextField` 숫자 키패드, 확정은 편집 종료·화면 이탈(A39, 멱등), 판정은 :kit `WalkHealth.weightCommit`, 저장 키 `walkWeightKg`(M4 걸음 요약이 읽는다), 범위 밖은 저장 없이 `weightRejected`/`weightRejectedNone` 통지, 푸터 `weightFooter`) ④ 정보 출처 → `DataSourcesRoute` ⑤ 개인정보 처리방침(웹 `AppConfig.privacyPolicyURL`과 같은 URL, `ACTION_VIEW`, `tryStartActivity` 실패 → `noAppToOpen`) ⑥ 문제 신고(`ACTION_SENDTO` + `mailto:engccer@gmail.com`, 실패 통지 동일). 각 행 한 객체. 실험판 게이트는 `AppConfig.resultHapticsSettingEnabled = BuildConfig.EXPERIMENTAL`(기존 관용구 — 졸업 때 지울 자리 하나) — 정식판엔 행 자체가 없다. 행 목록은 순수 함수 `settingsRows(experimental): List<SettingsRow>`가 낸다(JVM 검증).
...
- 기제: `AppConfig.localized(base: Context): Context` = `localeOverride(settings.language)`가 null이면 `base`, 아니면 `Configuration(base.resources.configuration).apply { setLocales(LocaleList(Locale.forLanguageTag(code))) }`로 `base.createConfigurationContext(...)`(`setLocales`는 void — 복사본에 적용 뒤 넘긴다). **`createConfigurationContext` 호출은 `AppConfig.kt` 한 곳**(소스 가드). `MainActivity.attachBaseContext(base)`가 이것으로 감싼다(화면 `stringResource`). **ViewModel 쪽은 컨텍스트를 넘기지 않고 읽는다**: `AppConfig.localizedApp(): Context`(호출 시점에 `localized(applicationContext)` — `@Volatile` 캐시, 언어 저장 시 무효화)를 두고 `nearbyStrings`·`searchStrings`·`placeStrings`·`dataLocale`·시각 포맷 람다가 **호출 시점에 그 리소스를 읽는다**(`res` 캡처 금지 — ViewModel은 `recreate()`를 넘어 살아남고 `createConfigurationContext`의 오버라이드는 제자리 갱신되지 않으므로 캡처하면 옛 언어로 굳는다). 문장 공급 팩토리는 `nearbyStrings(res: () -> Resources)` 꼴로 바꾼다(테스트·androidTest는 `{ context.resources }`). **캡처 지점 전수 6곳**: `nearbyStrings`(`NearbyStringsRes.kt`) · `searchStrings`(`MainActivity.kt`) · `placeStrings` · `resourceStrings`(`DirectionsViewModelFactory.kt`·`ManualLocationPickerViewModel.kt`의 팩토리 — `() -> Resources` 오버로드를 더하고 컴포저블 호출부 2곳은 그대로) · `AppConfig.manualLocationJudge`의 `autoClearedText`(`AppConfig.localizedApp().getString(...)` — 순수 `applicationContext`는 오버라이드를 받지 않아 지금은 항상 기기 시스템 언어로 나간다) · **`chat/ChatFactories.kt`의 셋**(`chatStrings(app.resources)`·`lang`·`dataLocale` — `lang`은 서버 요청 파라미터라 답변 언어가 옛 언어로 남는다; `chatStrings`도 `() -> Resources` 오버로드; M6 통합분이라 M6 소유 파일을 additive로 고치고 보고한다). 나머지는 이 세션 소유 파일(`directions/`는 M3 종료 뒤 이 세션). `localizedApp()` 캐시는 언어 저장 시 무효화하며, `createConfigurationContext` 컨텍스트는 로케일 밖 구성 축(글꼴 크기·야간 모드)도 생성 시점 스냅샷이라 **문자열·시각 포맷만** 읽는다(치수·테마 읽기 금지). `localeOverride(stored)`는 `settings/SettingsStore.kt`의 **최상위 함수**(object 밖 — JVM 테스트가 `AppConfig` 초기화를 끌어오지 않게). `AppLocale.current(res)`는 그대로 리소스 마커를 읽는다. 시스템 "앱 언어" 설정(`locales_config`)은 저장값이 null("시스템 설정 따름")일 때 효력 — 목록 첫 행이 그 값으로 되돌리므로 죽은 설정이 되지 않는다.
- 적용: 저장 → `AppConfig.localizedApp()` 무효화 → `AppNotices.post(languageApplied)`(새 언어 리소스로 만든 문장) → `activity.recreate()`. 재생성을 넘기는 근거는 §13-5(프로세스 수명 큐, `RESUMED` 소유자가 claim). 선택 목록은 **다이얼로그**(`AlertDialog`, 라우트 없음 — 선택 즉시 닫히고 재생성되므로 백스택에 흔적이 없어 재생성 뒤 최상단이 설정 화면이고 그 `StatusLine`이 통지를 읽는다; 뒤로 = dismiss). 재생성 뒤 착지는 설정 제목(`titleFocus` 배선, 통지와 겹치는 자리라 실기기 28에서 커서 위치 확인).
- ⚠ 언어를 바꿔도 **이미 로드된 payload는 옛 `dataLocale`**이다(탭 백스택에 살아 있던 내 주변·검색은 재생성 뒤 새 언어 껍데기 + 옛 언어 데이터; §5 "구성 변경 재진입은 재조회하지 않는다"). **유예** — 사용자가 새로고침·재검색하면 새 언어이고 언어 변경은 드문 전이라 리비전 축을 새로 두지 않는다(BACKLOG 후보로 코디네이터에 보고: `SettingsStore.localeRevision`으로 1회 재조회).
- 행 = 라벨 + 현재 값 한 객체("언어, 한국어" / 저장값 없으면 "언어, 시스템 설정 따름"), 활성화 → 다이얼로그 라디오 목록 **7행** = "시스템 설정 따름"(`android.settings.themeSystem` 재사용 — M4가 테마 행을 더하며 이 문안을 다듬으면 언어 목록 첫 행도 함께 바뀐다는 것을 판정 41에 적어 둔다; 고르면 `language = null`) + `AppLocale.supported` 6개(자국어 표기는 기존 6키 `nav_korean`·`nav_english`·`nav_japanese`·`nav_spanish`·`nav_french`·`nav_italian` — 6로케일에서 값이 같아 새 표 없음); 컨테이너 `selectableGroup()` + 행 `selectable(selected, role = RadioButton)`; **다이얼로그 진입 착지는 현재 선택된 행**(없으면 첫 행, 한 프레임 뒤 `landingTarget`), 닫힌 뒤(취소)는 언어 행으로 복귀; 착지 순서 가드 정규식의 `selectable`·`toggleable`은 M2c에서 이미 들어갔다(그대로 통과).

### 14-3. 받아쓰기 방식·결과 진동

- 받아쓰기: 언어와 **같은 관용구** — 행 = 라벨 + 현재 값 한 객체("받아쓰기 방식, 탭 토글") → 다이얼로그 라디오 2행(`android.settings.dictationTap/dictationHold`, 진입 착지 = 현재 선택 행; 한 화면에서 같은 종류의 선택이 두 표면으로 동작하지 않게, 그리고 인라인 라디오는 무엇에 대한 선택인지가 이름에 없다), 저장은 `SettingsStore.dictationStyle`. `settingsRows`의 "행"은 논리 항목(정식판 6·실험판 7)이다. 소비는 M6(`DictationSession`·마이크 버튼) — **판정 41의 예외**(코디네이터 지시로 선반영, M6가 키를 그대로 읽는다). 같은 커밋에서 낡는 문장 여섯을 정정한다: `speech/Dictation.kt` KDoc("M2c/설정 마일스톤" → "M6 통합 뒤"), `MainActivity.kt`의 "앱 컨텍스트의 리소스도 앱별 언어 변경을 따라간다"(→ 호출 시점 읽기), `i18n/AppLocale.kt` KDoc(시스템 앱 언어 설정은 저장값이 null일 때만), `DirectionsViewModelFactory.kt`·`ManualLocationPickerViewModel.kt`·`chat/ChatFactories.kt`의 "호출 시점에 읽어 앱별 언어 변경을 따라간다"(앱 컨텍스트의 `Resources`는 오버라이드를 받지 않는다 — `localizedApp()` 경유로 바뀐 뒤에만 참).
- 결과 진동(실험판): 스위치 행 `Modifier.toggleable(role = Switch)` `android.settings.trendHaptics` + 푸터(`android.settings.resultHapticsFooter`). 수입된 `android.settings.trendHapticsFooter`는 **쓰지 않는다**(안내 진동을 말해 안드로이드에서 아직 거짓 — 스위치 키 `trendHaptics`와 짝이 맞는 이름이라고 고르지 말 것). `LocalResultHaptics`는 **`a11y/`에 정의**(`compositionLocalOf { false }` — `a11y`는 `settings`를 모른다)하고 `AppRoot`가 `AppConfig.settings.resultHapticsEnabled`로 제공한다. **소비 채널은 통지와 같다**(CLAUDE.md "문장이 나가는 조건 = 진동이 나가는 조건"): `a11y/Notice`에 `haptic: HapticKind? = null`(`success/attention/failure` — **3-state 유지**, CLAUDE.md "3-state를 촉각에도"; D10은 판정층 공유·재생만 안드로이드 방식) 추가. `StatusLine`이 **발화 효과 안에서**(`rev == 0`이면 내지 않고, 한 프레임 빈 문자열 프레임은 제외 — 문장이 나가는 조건 = 진동이 나가는 조건) `LocalResultHaptics`(CompositionLocal, `AppRoot`가 `AppConfig.settings.resultHapticsEnabled`를 제공)가 참이면 `LocalHapticFeedback.performHapticFeedback(성공 = Confirm, 주의 = ContextClick(실기기 판정 — 0건과 N건이 손으로 갈리는가), 실패 = Reject)`. `VIBRATE` 권한 불필요(프레임워크가 낸다); 기기 촉각 피드백 설정이 꺼져 있으면 무동작(푸터 조건 문장). D10 대응표의 수단 칸은 `VibrationEffect`지만 `performHapticFeedback`을 택한다(권한 불필요·시스템 촉각 설정 존중·Compose 의미 상수 — 판정 40). 병합 통지는 앱 통지의 종류가 이긴다(§13-5). 게시자: `NearbyScreenViewModel.onEvent`(iOS `nearbyAnnouncer` 동형 — Loaded n건 = success, 0건 = attention, RefreshFailed·전락 = failure), 검색 결과(성공·0건·실패, `SearchViewModel`). 1회성 결과에만, 반복 상태 통지·진행 통지엔 금지. 소스 가드: `performHapticFeedback` 호출 파일은 `A11y.kt` 하나(iOS `result-haptic-guard` 동형).

### 14-4. 정보 출처

- `DataSourcesScreen`: iOS 그대로 — `chat.source.*` 16행(iOS `sourceLabels` 순서, 각 한 객체 — 카탈로그 17키 중 `chat.source.osm`은 제외: 바로 아래 OSM 라이선스 절이 같은 이름을 말해 두 번 낭독된다) + OSM 라이선스 문장(`dataSources.osmLicense`) + 링크 2(`dataSources.osmLink` → openstreetmap copyright `ACTION_VIEW`, `dataSources.osmCopyRequest` → `ACTION_SENDTO` mailto). `dataSources.walkHealth` 행은 M4.

### 14-5. 테스트·실기기

- JVM(`:app` 유닛 레인은 `android.*` 타입을 못 부른다 — 순수 부분만): `SettingsStore`(왕복·받아쓰기 기본값·진동 기본 off) · `localeOverride(stored)`(null·미지 코드·지원 코드 3분기) · `settingsRows(experimental)`(정식판 행 6·실험판 행 7) · `Notice.haptic` 게시(내 주변 성공/0건/실패·검색 3분기 → 종류) · `mergeNotices` haptic 규칙(앱 우선) · 소스 가드(진동 트리거 파일 1개 · 착지 순서 가드의 `selectable`·`toggleable`(M2c에서 기존) · `createConfigurationContext`는 `AppConfig.kt` 한 곳 · 리소스 캡처 가드는 목록이 아니라 **규약**으로: `:app` 전역에서 `app.resources`·`app.getString(`·`applicationContext.resources`·`applicationContext.getString(` 참조 0이고 `applicationContext`를 지역 변수·필드에 담지 않는다(`\bapplicationContext\b` 참조는 `AppConfig.kt`(`localized`/`localizedApp` 정의)와 저장소 생성자 인자 `SharedPreferencesStore(app…)`뿐) + 이름 규약 파일(`*Factory.kt`·`*Factories.kt`·`*StringsRes.kt`·`MainActivity.kt`·`AppConfig.kt`)에서 `context.resources`·`context.getString(` 0 — 새 팩토리가 자동으로 유니버스에 든다(컴포저블의 `LocalContext.current.resources`는 다른 수신자라 걸리지 않는다) · `SettingsScreen`이 `settingsRows(AppConfig.resultHapticsSettingEnabled)`를 부른다 · 설정 아이콘이 4탭 루트 상단 바에 있음). 언어 전환 실동작은 실기기 28이 유일한 검증임을 명기.
- ATF: 설정 화면 행이 각 한 객체·언어 행 라벨·다이얼로그 라디오 목록 집합 맥락 · **다이얼로그 진입 착지 = 현재 선택 행**(저장값 null이면 첫 행; 누르기 전에 커서를 다른 곳에 옮겨 두는 방식 — 대상이 하나로 몰리면 검출력 0) · 모달이 떠 있는 동안 `StatusLine`이 앱 통지를 집지 않음(`StatusLineA11yTest`, M2c) (`AppNotices.reset()`을 `@Before`에서). (실험판 스위치 존재는 JVM `settingsRows`가 잠근다 — androidTest는 debug 변형만 돈다.)
- 실기기: 27 설정 아이콘 → 화면 → 뒤로 복귀 착지 28 언어 변경 → 재생성 뒤 "언어를 바꿨습니다"가 새 언어로, **이후 내 주변 통지·검색 lang도 새 언어**(ViewModel이 살아남아도 — B5) · "시스템 설정 따름"으로 되돌리기 · 재생성 뒤 커서가 제목에 앉는가 28-1 언어 다이얼로그를 연 채 전경 복귀(100m 이동) → 자동 해제 통지가 다이얼로그를 닫은 뒤 들리는가 28-2 개인정보 처리방침·문제 신고를 연달아 실패시켰을 때 `noAppToOpen`이 두 번 들리는가(한 프레임 빈 문자열 트릭의 검증) 29 진동 스위치 켜고 내 주변 조회 → Confirm, 0건 → ContextClick이 손으로 갈리는가 30 문제 신고가 메일 앱으로 열림.

### 14-6. 판정 (§13 이어서)

39. **언어는 앱 저장값 + `createConfigurationContext` 한 경로**(AppCompat 기각 — API 32 이하 앱 컨텍스트 미적용·테마 교체 요구; minSdk 33 상향은 D8 위반). 저장값이 시스템 앱 언어 설정을 이기되 "시스템 설정 따름" 행으로 되돌아간다. **ViewModel 문장은 컨텍스트를 캡처하지 않고 호출 시점에 `AppConfig.localizedApp()`을 읽는다**(ViewModel은 재생성을 넘어 살고 오버라이드 컨텍스트는 제자리 갱신되지 않는다). 언어 키는 `attachBaseContext`에서 동기로 읽는다(시작 경로 디스크 읽기 1회 수용).
40. **진동 채널은 `Notice.haptic` → `StatusLine`**, 3종 유지(D10: 판정층 공유·재생만 안드로이드 방식). 수단은 `performHapticFeedback`(D10 대응표의 `VibrationEffect` 대신 — 권한 불필요·시스템 촉각 설정 존중·Compose 의미 상수). 주의 질감 `ContextClick`은 실기기 판정.
41. **테마·듣기 속도·체중·AI 동의 행은 소비 마일스톤이 더한다**(YAGNI). 예외: 받아쓰기 방식 행은 코디네이터 지시로 선반영(M6가 같은 키를 읽는다). 체중 행도 예외(M4 세션 종료 뒤 코디네이터 인계 — 키·범위는 :kit `WalkHealth`). 테마 행을 더하는 M4는 `android.settings.themeSystem` 문안이 언어 목록 첫 행과 공유됨을 안다.
42. **설정 진입은 기존 `actions` 슬롯의 공용 `SettingsAction`** — Scaffold 인자 신설 없음. 채팅 탭은 M6 `ChatTabScreen`에 같은 한 줄 additive(M6 통합 뒤라 자리표시 래핑은 불필요) — 4탭 전부 도달.
...
적대적 설계 리뷰 판정(§14, `review-settings-design.md`): 1차 REQUEST_CHANGES(BLOCKER 4·MAJOR 7·MINOR 11·NIT 3) → 25건 전부 반영 → 2차 diff REQUEST_CHANGES(BLOCKER 1·MAJOR 6·MINOR 11·NIT 2 — ViewModel이 옛 로케일 컨텍스트를 붙듦·attachBaseContext 동기 읽기·푸터 오버라이드의 드리프트 벽·"시스템 따름" 복귀 행·목록 표면·전환 프레임 이중 낭독·JVM 불가 테스트) → 20건 전부 반영(B5 → 호출 시점 읽기, M2 → 신설 키 1, M3 → 7행, M4 → 다이얼로그, M5 → §13-5 claim 소유자, M6 → 순수 `localeOverride`+가드, m1~m11·n1·n2 본문), 기각 0 → 3차 diff REQUEST_CHANGES(BLOCKER 1·MAJOR 3·MINOR 9·NIT 2 — `RESUMED`를 관측 가능한 상태로·claim/consume/restore 중간 상태·컴포지션 밖 claim·캡처 지점 5곳·IO 구분의 실효·가드 유니버스·`localeOverride` 자리·사문화 푸터 키·`themeSystem` 공유·§3-1 순서 줄·설정 화면 통지 출처·restore 순서·판정 40 근거·캐시 축) → 15건 전부 반영(B6·M8·M9 → §13-5, M7 → 캡처 5곳 + 주석 5, m6 → M6 통합으로 자리표시 래핑 불필요), 기각 0 → 4차 diff REQUEST_CHANGES(BLOCKER 1·MAJOR 4·MINOR 5·NIT 2 — 발화 뒤 claimed 전이·캡처 6곳(채팅)·검색 복귀 게이트·프레임 순서 논거·`last()` hydration 창·§4 세 자리·다이얼로그 착지·받아쓰기 표면·정보 출처 링크 실패·`remember` 키 값/객체·파일 문장·deprecated 심볼) → 12건 전부 반영(B7·M12·m14 → §13-5는 M2c 구현(발화 단위 세대·`spokenRev`·`rev` 키)으로 확정, M10 → 6곳 + 규약 가드, M11 → `AppRoot` 슬롯, M13 → `last()` 자체 hydrate(M6 계약 무변경), m10~m13·n3·n4 본문), 기각 0 → 5차 diff+코드 APPROVE_WITH_CHANGES(BLOCKER 0·MAJOR 1·MINOR 7·NIT 4 — 첫 모달 뒤 claim·발화된 화면 문장 재부착·진동 배선·가드 낱말 대칭·`chat.source.osm`·제목 착지 배선·옛 `dataLocale` payload·다이얼로그 착지 ATF·n5~n8) → 12건 전부 반영(M14 → `LocalModalOpen`(M2c 코드에 즉시), m15 → 미발화 화면 문장만 병합(코드 즉시), m20 → 유예+BACKLOG 후보, 나머지 본문), 기각 0. **설계 확정.**

구현 리뷰 판정(§14, `review-settings-spec.md`·`review-settings-quality.md`): (게이트·리뷰 뒤 기록) (1차 반영 요지: B1·B2 → 판정 39 개정, B3 → §13-5 앱 통지 소비 규칙, B4 → 판정 43 유예, M1 walkHealth 행 M4, M2 → 판정 40 3종, M3·M4 → `SettingsStore`·`LocalResultHaptics`, M5 → 판정 42, M6 채팅 M6, M7 JVM `settingsRows`, m1~m3·n2·n3 유예로 소멸, m4 `selectableGroup`+가드, m5 예외 명기+KDoc, m6 푸터 오버라이드, m7 `AppConfig` 이름, m8 12키, m9 `ACTION_SENDTO`, m10 슬롯, m11 조건 문장, n1 유도), 기각 0.
