...
37. **지정 시점에 권한을 묻지 않는다** — `Fine`일 때만 origin을 잡고 아니면 팝업 없이 `origin = null`(라벨은 "(위치 확인 불가)" — 사실 그대로). §4 "권한 요청은 두 곳뿐"을 지킨다. iOS는 이 자리에서 묻는다 → **역이식 후보**로 코디네이터에 보고.
...
- 행 = 라벨 + 현재 값 한 객체("언어, 한국어" / 저장값 없으면 "언어, 시스템 설정 따름"), 활성화 → 다이얼로그 라디오 목록 **7행** = "시스템 설정 따름"(`android.settings.themeSystem` 재사용 — M4가 테마 행을 더하며 이 문안을 다듬으면 언어 목록 첫 행도 함께 바뀐다는 것을 판정 41에 적어 둔다; 고르면 `language = null`) + `AppLocale.supported` 6개(자국어 표기는 고유명사 표); 컨테이너 `selectableGroup()` + 행 `selectable(selected, role = RadioButton)`; 착지 순서 가드 정규식에 `selectable`·`toggleable`(진동 스위치 행) 추가.

### 14-3. 받아쓰기 방식·결과 진동

- 받아쓰기: 라디오 2행(`android.settings.dictationTap/dictationHold`), 저장은 `SettingsStore.dictationStyle`. 소비는 M6(`DictationSession`·마이크 버튼) — **판정 41의 예외**(코디네이터 지시로 선반영, M6가 키를 그대로 읽는다). 같은 커밋에서 낡는 문장 다섯을 정정한다: `speech/Dictation.kt` KDoc("M2c/설정 마일스톤" → "M6 통합 뒤"), `MainActivity.kt`의 "앱 컨텍스트의 리소스도 앱별 언어 변경을 따라간다"(→ 호출 시점 읽기), `i18n/AppLocale.kt` KDoc(시스템 앱 언어 설정은 저장값이 null일 때만), `DirectionsViewModelFactory.kt`·`ManualLocationPickerViewModel.kt`의 "호출 시점에 읽어 앱별 언어 변경을 따라간다"(앱 컨텍스트의 `Resources`는 오버라이드를 받지 않는다 — `localizedApp()` 경유로 바뀐 뒤에만 참).
...
- JVM(`:app` 유닛 레인은 `android.*` 타입을 못 부른다 — 순수 부분만): `SettingsStore`(왕복·받아쓰기 기본값·진동 기본 off) · `localeOverride(stored)`(null·미지 코드·지원 코드 3분기) · `settingsRows(experimental)`(정식판 행 5·실험판 행 6) · `Notice.haptic` 게시(내 주변 성공/0건/실패·검색 3분기 → 종류) · `mergeNotices` haptic 규칙(앱 우선) · 소스 가드(진동 트리거 파일 1개 · 착지 순서 가드에 `selectable`·`toggleable` · `createConfigurationContext`는 `AppConfig.kt` 한 곳 · 문장 공급 팩토리 파일(`NearbyStringsRes.kt`·`MainActivity.kt`·`placeStrings` 파일·`DirectionsViewModelFactory.kt`·`ManualLocationPickerViewModel.kt`·`AppConfig.kt`)에서 `app.`/`context.`를 수신자로 한 `resources`·`getString(` 참조 0(전부 `AppConfig.localizedApp()` 경유 — 컴포저블의 `LocalContext.current.resources`는 옳으므로 유니버스 밖) · `SettingsScreen`이 `settingsRows(AppConfig.resultHapticsSettingEnabled)`를 부른다 · 설정 아이콘이 4탭 루트 상단 바에 있음). 언어 전환 실동작은 실기기 28이 유일한 검증임을 명기.
