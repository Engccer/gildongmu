...
41. **테마·듣기 속도·체중·AI 동의 행은 소비 마일스톤이 더한다**(YAGNI). 예외: 받아쓰기 방식 행은 코디네이터 지시로 선반영(M6가 같은 키를 읽는다). 테마 행을 더하는 M4는 `android.settings.themeSystem` 문안이 언어 목록 첫 행과 공유됨을 안다.
