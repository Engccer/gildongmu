...
- 적용: 저장 → `AppConfig.localizedApp()` 무효화 → `AppNotices.post(languageApplied)`(새 언어 리소스로 만든 문장) → `activity.recreate()`. 재생성을 넘기는 근거는 §13-5(프로세스 수명 큐, `RESUMED` 소유자가 claim). 선택 목록은 **다이얼로그**(`AlertDialog`, 라우트 없음 — 선택 즉시 닫히고 재생성되므로 백스택에 흔적이 없어 재생성 뒤 최상단이 설정 화면이고 그 `StatusLine`이 통지를 읽는다; 뒤로 = dismiss). 재생성 뒤 착지는 설정 제목(기본).
- 행 = 라벨 + 현재 값 한 객체("언어, 한국어" / 저장값 없으면 "언어, 시스템 설정 따름"), 활성화 → 다이얼로그 라디오 목록 **7행** = "시스템 설정 따름"(`android.settings.themeSystem` 재사용 — M4가 테마 행을 더하며 이 문안을 다듬으면 언어 목록 첫 행도 함께 바뀐다는 것을 판정 41에 적어 둔다; 고르면 `language = null`) + `AppLocale.supported` 6개(자국어 표기는 고유명사 표); 컨테이너 `selectableGroup()` + 행 `selectable(selected, role = RadioButton)`; **다이얼로그 진입 착지는 현재 선택된 행**(없으면 첫 행, 한 프레임 뒤 `landingTarget`), 닫힌 뒤(취소)는 언어 행으로 복귀; 착지 순서 가드 정규식에 `selectable`·`toggleable`(진동 스위치 행) 추가.
...
- JVM(`:app` 유닛 레인은 `android.*` 타입을 못 부른다 — 순수 부분만): `SettingsStore`(왕복·받아쓰기 기본값·진동 기본 off) · `localeOverride(stored)`(null·미지 코드·지원 코드 3분기) · `settingsRows(experimental)`(정식판 행 5·실험판 행 6) · `Notice.haptic` 게시(내 주변 성공/0건/실패·검색 3분기 → 종류) · `mergeNotices` haptic 규칙(앱 우선) · 소스 가드(진동 트리거 파일 1개 · 착지 순서 가드에 `selectable`·`toggleable` · `createConfigurationContext`는 `AppConfig.kt` 한 곳 · 리소스 캡처 가드는 목록이 아니라 **규약**으로: `:app` 전역에서 `app.resources`·`app.getString(`·`applicationContext.resources` 참조 0(허용은 `AppConfig.kt`의 `localized`/`localizedApp` 정의뿐) + 이름 규약 파일(`*Factory.kt`·`*Factories.kt`·`*StringsRes.kt`·`MainActivity.kt`·`AppConfig.kt`)에서 `context.resources`·`context.getString(` 0 — 새 팩토리가 자동으로 유니버스에 든다(컴포저블의 `LocalContext.current.resources`는 다른 수신자라 걸리지 않는다) · `SettingsScreen`이 `settingsRows(AppConfig.resultHapticsSettingEnabled)`를 부른다 · 설정 아이콘이 4탭 루트 상단 바에 있음). 언어 전환 실동작은 실기기 28이 유일한 검증임을 명기.
- ATF: 설정 화면 행이 각 한 객체·언어 행 라벨·다이얼로그 라디오 목록 집합 맥락(`AppNotices.reset()`을 `@Before`에서). (실험판 스위치 존재는 JVM `settingsRows`가 잠근다 — androidTest는 debug 변형만 돈다.)
- 실기기: 27 설정 아이콘 → 화면 → 뒤로 복귀 착지 28 언어 변경 → 재생성 뒤 "언어를 바꿨습니다"가 새 언어로, **이후 내 주변 통지·검색 lang도 새 언어**(ViewModel이 살아남아도 — B5) · "시스템 설정 따름"으로 되돌리기 29 진동 스위치 켜고 내 주변 조회 → Confirm, 0건 → ContextClick이 손으로 갈리는가 30 문제 신고가 메일 앱으로 열림.
