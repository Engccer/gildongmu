...
- 둘러보기: **문장과 완료 통지가 같은 술어**를 쓴다 — `usedManualCoordinate(payload, manual) = manual != null && payload.lat == manual.lat && payload.lng == manual.lng`(정확 비교: 수동이면 `coordinate()`가 그 좌표를 그대로 돌려준다). 위치 문장 `aroundHereManual`/`…ManualNoPlace`, 완료 통지 `android.nearby.aroundLoadedManual`(iOS `AroundNearbyView` 동형 — "수동 위치일 때 '현재 위치'라고 알리지 않는다"). `NearbyKinds.around(service, strings, manual: () -> ManualLocation?)`, `NearbyStrings.aroundLoadedManual`; `AroundBody`의 `manual`은 **`NearbyScreenViewModel`이 `manual: () -> ManualLocation?`로 들고 넘긴다**(화면이 앱 싱글턴을 직접 잡지 않는다 — 팩토리 주입 관용구). 유니버스 술어에 `usedManualCoordinate(`를 더해 그 배선에서도 스캔에 든다. `AroundPayload.lat/lng` KDoc을 이 소비처로 갱신.
