...
- **`settings/SettingsStore(store: KeyValueStore)`**(앱 싱글턴 `AppConfig.settings`) — 설정 값의 **단일 소유자**: `language: StateFlow<String?>`(null = 시스템 따름; 값은 `AppLocale.supported` 안 코드만, 밖이면 null — 순수 `localeOverride(stored): String?`가 3분기를 가른다) · `dictationStyle: StateFlow<String>`(`"tapToggle"` 기본/`"hold"`, 키 `dictationStyle` = iOS `DictationStyle.key`·값 rawValue 동일 — M6가 같은 키를 읽는다) · `resultHapticsEnabled: StateFlow<Boolean>`(키 `TrendHaptics.storageKey`(:kit `trendHapticsEnabled`), 기본 false). **저장 파일은 하나**(iOS `UserDefaults.standard` 동형): `SharedPreferencesStore`의 기본 파일 이름을 `gildongmu.recent`에서 **`gildongmu`로 개명**한다(최근 검색·수동 위치·설정이 한 파일 — 미출시라 마이그레이션 없음, KDoc 갱신). **언어 키만 `attachBaseContext`에서 동기로 읽는다**(SharedPreferences 최초 로드 1회를 시작 경로 비용으로 수용 — 첫 프레임이 옳은 언어여야 한다; `SharedPreferencesStore` KDoc에 예외 명기), 나머지 키의 첫 읽기는 `Dispatchers.IO`.
- 진입: iOS는 탭 제목 메뉴의 "설정". 안드로이드는 탭 루트 상단 바의 **기존 `actions` 슬롯**에 공용 `SettingsAction(onOpen)`(아이콘 버튼, `contentDescription = android.settings.title`, `tapTarget`, 새로고침 등 화면 고유 액션 **뒤** — 이 순서 규칙은 §3-1 상단 바 계약에 한 줄 추가) — 검색·내 주변 허브·길찾기(M3 파일, 한 줄 additive)는 이 세션이 배선하고, 채팅 탭의 자리표시 `PlaceholderScreen`은 `AppScreenScaffold`로 감싸(§3-1 "모든 화면이 상단 바" 회복, 한 줄) 같은 버튼을 단다 — M6가 자기 화면으로 바꿀 때 그 한 줄을 유지한다(4탭 전부 도달). → `SettingsRoute`(스택, 등록 한 줄). pop 복귀 착지는 그 버튼: 내 주변 허브는 기존 `ReturnFocusViewModel` 슬롯(키 `"settings"`), 검색은 `SearchViewModel`의 슬롯을 같은 키로 공유(동시에 두 대기가 없다), 길찾기·채팅은 `AppRoot`에 `ReturnFocusViewModel`을 붙인다(`DirectionsScreen`이 `takeReturnFocus`를 받아 버튼에 착지 — M3 파일 additive; 채팅 자리표시는 같은 방식).
...
42. **설정 진입은 기존 `actions` 슬롯의 공용 `SettingsAction`** — Scaffold 인자 신설 없음. 채팅 자리표시도 `AppScreenScaffold` 한 줄로 감싸 4탭 전부 도달(M6까지 한 탭에서 설정 불가 vs 한 줄 — 한 줄이 싸다).
...
적대적 설계 리뷰 판정(§14, `review-settings-design.md`): 1차 REQUEST_CHANGES(BLOCKER 4·MAJOR 7·MINOR 11·NIT 3) → 25건 전부 반영 → 2차 diff REQUEST_CHANGES(BLOCKER 1·MAJOR 6·MINOR 11·NIT 2 — ViewModel이 옛 로케일 컨텍스트를 붙듦·attachBaseContext 동기 읽기·푸터 오버라이드의 드리프트 벽·"시스템 따름" 복귀 행·목록 표면·전환 프레임 이중 낭독·JVM 불가 테스트) → 20건 전부 반영(B5 → 호출 시점 읽기, M2 → 신설 키 1, M3 → 7행, M4 → 다이얼로그, M5 → §13-5 claim 소유자, M6 → 순수 `localeOverride`+가드, m1~m11·n1·n2 본문), 기각 0. (1차 반영 요지: B1·B2 → 판정 39 개정, B3 → §13-5 앱 통지 소비 규칙, B4 → 판정 43 유예, M1 walkHealth 행 M4, M2 → 판정 40 3종, M3·M4 → `SettingsStore`·`LocalResultHaptics`, M5 → 판정 42, M6 채팅 M6, M7 JVM `settingsRows`, m1~m3·n2·n3 유예로 소멸, m4 `selectableGroup`+가드, m5 예외 명기+KDoc, m6 푸터 오버라이드, m7 `AppConfig` 이름, m8 12키, m9 `ACTION_SENDTO`, m10 슬롯, m11 조건 문장, n1 유도), 기각 0.
