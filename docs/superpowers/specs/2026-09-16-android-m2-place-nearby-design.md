...
- **본문 = phase 스위치**(`Column` 안, 오버레이가 아니라 본문 교체 — 오버레이는 터치를 삼키거나 트리 순서를 흐린다). ⚠ **0건은 `Loaded`다**: M2 4종의 fetch는 전부 non-null(0건 = 빈 리스트)이라 :kit `Empty` phase(fetch가 null을 돌려줄 때만, 코어 #12)에는 도달하지 않는다. fetch가 0건을 null로 돌려주면 Loaded에서 새로고침 0건이 `RefreshFailed`("기존 정보를 유지합니다")로 뒤집혀 3-state가 깨진다 — 금지. 빈 문구 판정은 도메인 `isEmpty(payload)` 술어(§5 표)다(iOS `descriptor.emptyList`).
...
> M2 통합(`0e8c1204`) 뒤 코디네이터 지시로 M2 spec에 절을 더한다. §1 "범위 밖" 중 이 절이 받는 것: 내 주변 나머지 6개, 둘러보기 "주변 상황" 자동 펼침, 장소 상세의 역 자동 섹션·무장애 자동 섹션, 현재 위치 표시줄. **받지 않는 것(§12-6 판정)**: 수동 위치 지정(M3 `directions/`의 출발·도착지 검색 화면을 재사용해야 한다 — M3 통합 뒤 M2c), 결과 진동(설정 스위치 화면이 없다 — 설정 화면과 함께). 동작 정본 iOS `Nearby/{ClinicNearbyView,BarrierFreeNearbyView,KidsNearbyView,EventsNearbyView,WalkInfraNearbyView,ConditionsView,SurroundingsSceneSection}.swift`, `StationSections.swift`, `BarrierFreeInfoSection.swift`, `LocationBarView.swift`, `CurrentAddressStore.swift`. §3~§10의 계약은 전부 승계하고 여기엔 더하는 것만 적는다.

### 12-1. 내 주변 6개 kind (`NearbyKind` 확장, 허브 순서 = iOS: around·subway·bus·bike·clinic·barrierFree·kids·events·walkInfra·conditions)

전부 `NearbyKinds.kt` 조립기 한 행 + `NearbyKindScreen` 본문 하나. 앵커 화면은 `conditions`만(장소 상세 "이 장소 주변" 4번째 행, iOS 동형). 목록형 5종은 `RevealWindow` 더 보기 + 장소 행 버튼(→ 상세) + pop 복귀 착지(`place-{id}`).

| kind | payload / fetch | isEmpty · firstKey | 행 | 통지·빈 문구 |
|---|---|---|---|---|
| clinic | `ClinicPayload(clinics, basis, supplementFailed)` ← `NearbyService.clinics`(`basis ?: "weekday"`, `supplementFailed ?: false`) | `clinics.isEmpty()` · `place-{clinic.id}` | 머리: `basis == "holiday"`면 `clinicNearby.basisHoliday`, `supplementFailed`면 `clinicNearby.supplementFailedNotice`(둘 다 0건이면 생략). 행 = `PlaceRow(nightClinicToPlace, secondary = joinText(clinicKindText(kind), clinicStatusText(openStatus), place.distance))`. `clinicKindText`: `clinicKindKey` → `clinicNearby.kind.clinic/hospital`, 그 밖 원문. `clinicStatusText`: `open` → `clinicNearby.open` + `end`가 있으면 `clinicEndTimeText`(2400 → `android.nearby.untilMidnight`, 그 밖 `android.nearby.untilTime(HH, MM)`), `closed` → `android.nearby.clinicClosed`, 그 밖 `android.nearby.clinicUnknown` | `announcePlaces(n)` · `android.nearby.clinicEmpty` |
| barrierFree | `List<BarrierFreePlace>` ← `BarrierFreeService.nearby` | `isEmpty()` · `place-{contentId}` | `PlaceRow(barrierFreePlaceToPlace, secondary = joinText(address, place.distance))`. 목록 끝(더 보기 뒤) 출처 `barrierFreeInfo.source` 텍스트(0건이면 없음) | `announcePlaces(n)` · `android.nearby.barrierFreeEmpty` |
| kids | `List<KidsPlace>` ← `kidsPlaces` | `isEmpty()` · `place-{id}` | `PlaceRow(kidsPlaceToPlace, secondary = joinText(kindLabel, inOutLabel, place.distance, roadAddress ?: address))`. `kindLabel`: kidscafe/playground/playcenter → `kidsNearby.kind.*`, park → `android.nearby.kidsPark`, 그 밖 원문. `inOutLabel`: indoor/outdoor → `kidsNearby.indoor.*`, 그 밖 `kidsNearby.indoor.unknown`(3-state — 생략 금지) | `announcePlaces(n)` · `android.nearby.kidsEmpty` |
...
| walkInfra | `WalkInfraPayload(walk, asOf: "HH:mm")` ← `WalkInfraService.nearby`. **`coverage = none`**(라우트에 커버리지 마커가 없고 두 소스가 각자 `Unsupported`로 강등 — `korea`로 "정리"하면 소스별 구분이 사라진다, iOS 주석) | `false`(항상 본문 — 소스별 문장이 3-state를 말한다) · `walkinfra-top` | ① 헤딩 `walkInfra.asOf(asOf)`(착지 지점) ② 그룹 헤딩 3개 **항상**(`walkInfra.groupAudio`·횡단보도 `groupCrossing/CrossingCount/CrossingTruncated(total, listed)`·점자블록 `groupTactile/…`) + 상태별 행: `Ok` → 요약(`audioSummary(deviceCount)`/`audioNone`) + 항목(`walkInfra.audioSite(direction, distance, deviceCount)`; 횡단보도 `joinText(itemLocation(direction, distance), hasSignal?, hasTactile?)`; 점자블록 `joinText(itemLocation, hostBusStop?/hostSubwayEntrance?)`), `Unsupported` → `*Unsupported`, `Error` → `*Error`. 미지 방위는 방위 조각 생략. 거리 낭독 단위 풀어쓰기 ③ 각주: 성공한 소스만 인용(`walkInfra.footnote` + `joinText(sourceOsm?, sourceAudio(baseDate)?)`) | `walkInfraLiveSummary(walk)`(iOS 함수 이식: 소스별 요약 결합, "0기" 합성 금지) · 빈 문구 없음(본문이 말한다). 첫 로드 실패 문구는 `walkInfra.error`(FailedServer) |
| conditions | `ConditionsPayload(weather?, air?, congestion?, freshWeather, freshAir)` ← `ConditionsService` 셋을 `settled`로 독립 조회. 어느 조각이든 `APIError.OutOfCoverage`면 전체 throw(이중 방어). 재조회 실패 조각은 **직전 값 유지**(payload에 `previous` 필요 → 코어 `fetch(coord, previous)`의 둘째 인자를 `NearbyKindSpec.fetch`가 받도록 확장), 혼잡도만 **성공한 null은 덮어쓴다**(핫스팟 밖이 답) | `false` · `conditions-weather` | ① 헤딩 `android.nearby.weatherHeading`(착지) + 하늘·강수·기온·최고/최저·습도·강수확률·기준 시각 행(각 한 줄, null 필드는 줄 없음; 정수 표기 `numberText`) 또는 `weatherFailed` ② 헤딩 `weather.airLabel` + 측정소 줄(병기)·khai·pm10·pm25(`라벨, 등급 (수치)`)·측정 시각 또는 `airFailed` ③ 혼잡도: **있을 때만** 요약 줄(`congestion.summary(name, levelWord)`)·ko일 때 `message`·`congestion.asOf` — 헤더 없음, 부재는 침묵(서울 91%가 핫스팟 밖) | `freshWeather && freshAir` → `conditionsReady` / 하나 → `conditionsPartial` / 둘 다 실패 → `failedTitle` · 빈 문구 없음 |

- 도메인 문장 조립은 `nearby/NearbyLines.kt`에 순수 함수로 더한다(`clinicStatusText`·`eventFeeText`·`walkInfraLiveSummary`·`conditionsNotice`·`pollutantText`·`numberText` 등, 리소스는 람다).
- 장소 상세 **도메인 섹션**(iOS `domainSection` — 그 화면에 온 이유라 서열 1위, 제목 바로 아래): clinic → `ClinicDomainSection`(`clinicStatusText`, `directions`가 있으면 `clinicNearby.directions(directions)`, `designated == true`면 `android.clinic.designated`) · events → `CultureEventSection`(장소·자치구(병기), `joinText(dateText, timeText)`, 요금, `eventsNearby.target(target)` — 빈 값은 줄 없음). 전달은 `PlaceDetailRoute`에 `domainJson: String?`(`@Serializable sealed class PlaceDomain { Clinic(clinic); Event(event) }`)로 싣는다 — 프로세스 재생성 뒤에도 섹션이 남는다.

### 12-2. 둘러보기 "주변 상황" 자동 펼침 (iOS `SurroundingsSceneAutoSection`)

`AroundPayload`에 `scene: SurroundingsScene?`·`sceneFailed` 조각 추가(세 조각 한 fetch·한 커밋, `isAllAbsent`는 셋 다 부재). 위치는 한눈에 보기 **다음**, 가게 목록 **앞**(iOS 순서). 헤딩 `surroundings.ready` 아래: 실패 또는 null → `surroundings.error`, `total == 0` → `surroundings.empty`, 그 밖 묶음마다 헤딩 `bucketTitle`(`surroundings.bucket.{left|right|across|beyond|n|…|nw}`, 항목 3개 초과면 ` + surroundings.count(n)`), 항목 행 = 장소 버튼(`sceneItemToPlace`), 문장 `surroundings.itemWithRoad(distance, name, road)`/`surroundings.item(distance, name)`(비-ko 병기: 시각 `display`, 낭독 `primary`; 도로명은 로마자만), 묶음별 **더 보기**(`RevealWindow` 묶음별 — `SceneRevealWindows`: `Map<bucket, RevealWindow>`, 커밋마다 리셋, 첫 새 행 착지 키 `scene-{bucket}-{index}`), 끝에 `surroundings.source` 각주. 착지는 부모의 위치 문장 1회뿐(조용히 나타나는 섹션 — 헤딩이 발견 경로). 자동 펼침이라 트리거·닫기 없음.

### 12-3. 장소 상세 자동 섹션 (iOS `StationSectionsView`·`BarrierFreeInfoSection`)

- **역 자동 섹션 5종**(`isStation(place)`일 때만, `PlaceDetailViewModel`이 진입 시 `StationService` 5개를 `settled`로 병렬 로드): 조용히 나타난다(로딩 표시·통지 없음), 각 섹션 헤딩이 유일한 발견 경로. 순서 = 웹 배선(meta → arrivals → timetable → korail → metro), 위치는 "이 장소 주변" 다음(iOS). ① 역 메타 `stationMeta.heading`: 한 줄 `joinText(nameSuffixed(name), nameEn, lines(pickLine en), transfer?, operator)`(en은 `bilingualName` 병기·낭독 영문) ② 실시간 도착 `android.station.arrivalHeading`: 0건 `android.station.noArrivals`, 행 = `subwayArrivalLine`(12-1 아님, §3-7의 함수 재사용) ③ 시간표 `timetable.heading` **3-state**: `null` → 섹션 없음(미커버), 실패 → `timetable.error` 문장(숨기지 않는다), 성공 → `joinText(dailyTypeLabel, partial?)` + 노선마다 `coverageText`(noTrains/unavailable/unknown → 문장) 또는 방향 행 `joinText("{line} {direction}", "첫차 {train}", "막차 {train}")`(en은 노선·종착 영문이 다 있을 때만, `trainText`: 익일 접두·`toTerminus`) ④ 코레일 `android.station.railFacilities`: 장애인 화장실·휠체어 리프트(`countText` 3-state: null "정보 없음"·0 "없음"·n)·경사로·엘리베이터 ⑤ 서울 `android.station.seoulFacilities`: 종류마다 `android.station.kindCount(metroKindLabel, n)` + 시설 행 `joinText(facilityName(parts 우선: elevatorAt(compass, meters)+dong / location+lineEn), location, floors, operatingStatus, detail)`, `supplementFailed`면 `subway.supplementFailed`, voiceGuide 그룹이 있으면 `subway.voiceGuideSource`. 실패·null은 그 섹션만 없음(③ 제외).
...
허브 맨 위 한 줄(텍스트 객체, `mergedRow("location-bar")`). 4-state(웹 `LocationBar.tsx` 미러, 수동 위치 갈래는 M2c): 권한 없음(`Denied`) → `manualLocation.gpsFailed` / 좌표 없고 `lastFixFailed` → `gpsFailed` / 좌표 없고 시도 전 → `manualLocation.locating` / 좌표 있음 → 주소가 있으면 `manualLocation.gpsNear(address)`(비-ko는 `bilingualName(address, en = english)` 시각 병기·낭독 영문), 없으면 `manualLocation.gps`. 주소는 `location/CurrentAddressStore`(앱 싱글턴, 좌표 4자리 키 + 언어, 좌표당 1회·재시도 없음, 좌표가 갈리면 옛 주소 먼저 폐기, 취소는 확정 아님)가 `LocationStore.coordinateForDisplay()`(권한이 이미 있을 때만, `softTimeout`, 팝업 없음 — 신설)로 얻어 `SearchService.reverseGeocode(lat, lng, lang)`로 채운다. 표시줄은 **위치를 요청하지 않는다**(허브 진입만으로 팝업 금지). `LocationStore`에 `authorization()` 스냅샷(`LocationPermission`) 노출.

### 12-5. 테스트·실기기 추가

- JVM: 6 kind 조립기(Kit 실캡처 fixture `clinic-nearby.json`·`barrier-free-nearby.json`·`kids-nearby.json`·`walk-nearby.json`/`-degraded`/`-unsupported`·`air-nearby.json`·`weather-nearby.json`·`congestion-nearby.json`; 문화행사는 Kit fixture가 없어 손 fixture 1건을 `app/src/test/resources`에 둔다)로 첫 착지 키·통지·빈 문구; walkInfra 요약 문장 3-state(ok/unsupported/error 조합); conditions 병합 규칙(실패 조각 직전 값 유지, 혼잡도 성공 null 덮어쓰기, 커버리지 마커 throw, 통지 3분기); scene 묶음 리빌 창·문장; 역 섹션 `countText`·`coverageText`·`trainText`·`facilityName`; `barrierFreeFacilityLabel` 27키 리터럴 `when` 전수(키 목록은 messages `barrierFreeInfo.facility.*`에서 읽어 미매핑 0); `CurrentAddressStore` 좌표당 1회·언어 키·취소 비확정; 표시줄 4-state 문장 순수 함수. `PlaceDomain` JSON 왕복. 역·무장애 섹션 문장은 `station-meta/arrival/timetable/facilities/metro-facilities.json`·`barrier-free-match.json`, 주변 상황은 `surroundings-scene.json`·`-compass.json`.
- 실기기(§9에 추가): 17 허브 10행 순서·표시줄 낭독 18 보행 인프라 3그룹 헤딩 점프 19 날씨 화면 착지(날씨 헤딩) 20 역 장소 상세에서 자동 섹션 5종 헤딩 점프 21 둘러보기 주변 상황 묶음 헤딩·더 보기 착지 22 소아진료 상세의 도메인 섹션이 제목 바로 아래.

### 12-6. 판정 (§10 이어서)

22. **수동 위치 지정은 M2c(M3 통합 뒤)** — 지정 시트가 iOS `Directions/DirectionsEndpointSearchView`(`.manualLocation` 타깃)를 재사용하므로 M3의 `directions/` 출발·도착지 검색 화면이 먼저다. 표시줄은 텍스트 행으로 먼저 세우고 M2c에서 버튼(`manualLocation.pickTitle` 꼬리)으로 바꾼다. `ManualLocationStore`·`ManualLocationJudge`·`effectiveCoordinate`도 M2c.
23. **결과 진동은 M2b 밖** — iOS는 실험판 설정 스위치(`TrendHaptics.storageKey`) 뒤이고 안드로이드에 설정 화면이 없다. D10(촉각·청각 재설계)대로 설정 화면(언어 설정과 함께)이 생길 때 `HapticFeedbackConstants.CONFIRM/REJECT`로 판정한다.
24. **walkInfra는 `coverage = none`**(iOS 주석 그대로 — 소스별 미제공 문구가 커버리지 밖 한 문구로 뭉개지지 않게).
25. **conditions·clinic 등 조각 병합은 `NearbyKindSpec.fetch(coord, previous)`** — 코어의 `previous`를 조립기까지 흘린다(기존 4종은 무시).
26. **도메인 섹션은 라우트 JSON 인자**(`PlaceDomain`) — 재생성 뒤에도 남는다(iOS는 뷰 트리 수명).
27. **시간표만 3-state 보존**(실패 문장), 나머지 자동 섹션은 실패 = 미노출(iOS·웹 동형).
28. **표시줄은 위치를 요청하지 않는다**(허가된 세션의 캐시/soft 취득만) — 허브 진입만으로 권한 팝업이 뜨면 "내 주변 최초 사용 시점" 계약이 깨진다.

적대적 설계 리뷰 판정(§12): (기록 예정)
