...
- 허브 진입점: 표시줄 버튼 → `AppRoot`가 `returnFocus.slot.remember("location-bar")` 뒤 `ManualLocationRoute`(스택, `location/ManualLocationPickerScreen.kt`, 등록 한 줄). `ManualLocationPickerViewModel`이 `EndpointPicker`를 `manualLocation` 타깃으로 열고 `onSelect`에서 **`commit`(웹 `commitManual` 동형)**: in-flight 가드(연타 무시) → `Current`면 `manual.clear()` 즉시 / `Place(label, lat, lng, labelRoman)`면 `authorization() == Fine`일 때만 `isCommitting = true` + 진행 통지 `manualLocation.locating`(측위가 실제로 일어나는 갈래에서만 — 웹 `commitManual` 동형) → `origin = location.currentFix(force = true)?.takeIf { isEligibleManualFix(it, now) }`, `Fine`이 아니면 통지 없이 `origin = null`(**권한 다이얼로그 없음** — 판정 37) → `manual.set(...)` → **그 다음** pop(`onDone` 콜백). 실내 8초는 진행 통지가 말한다. 뒤로(취소)는 진행 중이면 잡을 취소하고 pop(지정 안 됨). 허브 복귀 착지는 표시줄 버튼(`NearbyHubScreen`의 복귀 resolver가 `"location-bar"`를 그 버튼 requester로 푼다 — kind 키만 알던 resolver 확장), 그때 라벨은 이미 확정돼 있다(pop이 `set` 뒤라 "표시줄 라벨 자체가 결과"가 성립). 지정 확인 통지는 없다(iOS 동형).
