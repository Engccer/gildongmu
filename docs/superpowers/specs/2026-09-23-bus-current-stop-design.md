...
| 하차 정류장 반경 | 50m | 하차 행만 "그 정류장 앞"을 요구한다. 정류장 간격이 200m인 도심에서도 직전 정류장에 선 버스가 이 반경에 들려면 보고 정확도 100m 안에서 150m 넘게 치우친 fix가 두 번 이어져야 한다. **교환**: 이른 "하차, 현재 위치"를 막는 대신, 하차 정류장 앞 fix가 50~100m에 머무는 동안 목록은 직전 정류장을 최대 90초 말한다("아직 한 정거장 남았다"로 들릴 수 있다, 검증 리뷰 MINOR-2). 하차 판정 정본은 도착 피드라 위험도는 같은 급이고, 어느 쪽이 덜 헷갈리는지는 실승차 관찰(`approachingAlight idx=` 줄의 시각 대 도착 피드 하차 이벤트) |
...
- 웹은 keep-alive가 없다(전경 전용). 버스 riding(적용 조건 ∧ 폴 주기 > 0 — iOS keep-alive와 같은 조건) ∧ 전경(`visibilityState`) ∧ 공유 위치 스토어가 `ready`일 때 **세션 전용 `watchPosition`**(정밀·캐시 무시, 도보 안내 `useRouteGuide`와 같은 옵션)을 열고, 조건이 깨지면 닫는다. 게이트는 **권한이 있다고 알려진 스토어**다: `ready`, 또는 시간 초과·위치 불가로 끝난 `denied`(그 둘은 권한이 허용된 뒤에만 난다 — 팝업을 만들지 않는다). 스토어 상태는 구독한다 — riding 뒤에 권한이 확인되어도 열리고(구현 리뷰 m5), 다른 화면의 강제 재측위가 시간 초과로 끝나도 닫히지 않는다(검증 리뷰 MINOR-1).
...
- **공유 fixture** `transit-bus-stop-cases.json`(63건, 최근접 index 포함): 적용 조건·결박 교체·첫 관측 두 번·정확도/나이 경계(100m·10초 포함, 음수·미래)·노선 밖·모호(접힘)·중간 지점·하차 정류장 50m(경계 49.5·50.5m, 후보 보존, 래치 뒤 멀어짐)·앞으로 튄 fix 폐기·두 번 확인 전진·뒤 1회·뒤 60초 재시작·연속 끊김(같은·앞 관측)·비관측은 연속을 끊지 않음·만료 래치 버림·(0,0) 정류장·표식 보존 창 경계(90초)·옛 결박·국면 밖·범위 밖 표식·세 번째 출처 `max`·조망 후처리(버스 → station, 지하철·국면 밖·추적 불가 지하철 불변). 비유한 좌표는 JSON에 실을 수 없어 웹·Kit 각각 같은 이름 테스트 1건. 웹 vitest·Kit `swift test`가 같은 입력·기대로 돈다.
...
- **웹 배선 테스트**(`TransitGuidePanel.bus-stop.test.tsx`): 스트림 fix 두 번에 "현재 위치"가 서고 한 번으로는 안 선다 · 공유 스토어 좌표·측위 호출이 그대로다 · 마지막 관측 + 90초에 표식이 거둬진다(가짜 시계) · riding 뒤 스토어가 `ready`가 되면 열린다 · 다른 화면의 재측위가 시간 초과로 끝나도 열려 있다 · 탭을 숨기면 닫는다 · 스토어가 `ready`가 아니면 열지 않는다.
...
| 구현 m5 웹 권한 게이트가 구독하지 않는다 | 수용 | `useGeolocation` 구독 + 테스트. ②(시간 초과 `denied`)는 반영분 검증 리뷰 MINOR-1에서 함께 수용 |
...
반영분 검증 리뷰(opus, HEAD `021772f1`, `review-fix-202609231755.md`): BLOCKER 0 · MAJOR 0 · MINOR 2 · NIT 4. 수용 지적 10건 해소 확인.

| 지적 | 판정 | 반영 |
|---|---|---|
| MINOR-1 구독 게이트가 재측위 시간 초과에 스트림을 승차 끝까지 닫는다, m5 보류 근거가 사실과 다르다 | 수용 | 게이트 = `ready` ∨ (`denied` ∧ 사유가 권한 거부 아님) + 테스트 |
| MINOR-2 하차 반경이 직전 정류장을 최대 90초 남긴다 | ⓐ 유지 + 관찰 | §2 표에 교환 명시, 실승차 축 |
| NIT-1 m1 가드 앵커가 `stop()` 블록 | 수용 | `updateKeepAlive` 안에서 찾는다 |
| NIT-2 하차 fixture 중복·경계 공백 | 수용 | 후보 보존·래치 뒤 멀어짐·49.5/50.5m 경계 |
| NIT-3 `busFix` 중복 제거 키에 최근접 없음 | 수용 | (판정, 최근접, 래치) |
| NIT-4 감사 NIT-1 이관 줄 | 통합 때 | BACKLOG E48 본항 |
