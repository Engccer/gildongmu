...
> 배경 조사 정본: `docs/RESEARCH-2026-07-routing-enhancement.md`. 위원장 결정(2026-07-21): 길찾기 탭 신설 승인("복잡도 증가는 고민사항 아님, 앱 취지 정합이 우선"), iOS 4탭(채팅·검색·길찾기·내 주변) 개조. Tmap appKey 발급·실호출 검증 완료(보행자·자동차 단일 키).
...
- **결과**: 수단 섹션 순서 **대중교통 → 도보 → 자동차**(보행 약자 우선 정체성). 조회 버튼 클릭 시 3수단 `Promise.allSettled` 병렬, 수단별 독립 3-state("경로 없음" ≠ "조회 실패" ≠ 게이트 미노출).
...
12. **§5 순서 근거 명시**: 대중교통 → 도보 → 자동차 순서는 의도다. 시각장애인 단독 이동의 주 수단이 대중교통이고 도보는 근거리·마지막 마일 수단이라 정보가치 순. "보행 약자 우선"은 자동차보다 보행·대중교통을 앞세운다는 뜻.
