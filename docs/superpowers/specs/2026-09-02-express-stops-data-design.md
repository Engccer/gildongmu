...
export interface TransitLeg {
  ...
  expressStops?: string[];
  /** expressStops와 같은 순서의 ODsay stationID 원문(길이 동일, 항상 함께). 소비자는 ID가 있으면 정규화 없이 ID로 판정 — transit-guide 설계 리뷰 2차 요청(2026-09-02) */
...
- `expressStopIds`는 `expressStops`와 **항상 함께, 같은 길이·같은 순서**로 온다(`stops[].stationId`와 같은 표기). 하차역 별칭 하나가 이름 정규화에서 어긋나 거짓 차단이 되지 않게 소비자는 ID 우선 판정한다. 서버는 길이 불일치·빈 배열이면 둘 다 싣지 않는다.
...
통과하면 정방향 후보의 `{names, ids}`(순서 보존)를 돌려준다. ⑦에서 `passStopList`가 없는 완행 leg는 비교 대상이 없는 것이라 건너뛴다(반례가 아니다). ⚠ `en` 응답을 쓰지 않는다 — 조회는 항상 `lang` 없는 ko 호출이라 `*Kor` 분기가 없다(집합은 조인용 한국어 원문).
...
- `unstable_cache(fn, ["odsay-express-stops", "v2", line], { revalidate: 7일 })`(v2 = `{names, ids}` 형태). 종전 안 30일에서 줄였다(리뷰 #4 — 다이어 개정 직후 낡은 집합이 발행되는 창을 줄인다. 권위 있는 다이어 버전 API는 없다). 7일 × 2콜은 쿼터에서 무시할 수 있다. 캐시 키에 계약 버전을 넣어 구버전 배포의 캐시가 새 형태를 오염시키지 않게 한다(리뷰 #6).
...
esbuild로 provider를 번들해(transfer-door 게이트 동형) **ODsay 6콜**(집합 정·역 2 + 9호선 길찾기 1 + 그 안의 express 조회 2 + 1호선 길찾기 1)로:

1. `fetchExpressStopsUncached(9호선 항목)`(정·역방향 2콜) → **16역 골든 전수 일치**(이름·순서: 김포공항·마곡나루·가양·염창·당산·여의도·노량진·동작·고속터미널·신논현·선정릉·봉은사·종합운동장·석촌·올림픽공원·중앙보훈병원 — §0 실호출, 리뷰 #14) + **stationID 16개 골든 전수 일치**(902·905·907·910·913·915·917·920·923·925·927·929·930·933·936·938). 부분 단언(당산 포함·노들 미포함)은 같은 길이 치환을 못 잡는다. ⚠ 골든이 어긋나면 **다이어 개정인지 ODsay 오류인지 사람이 가른다** — 개정이면 골든을 고치고 CHANGELOG에 남긴다.
...
4. **1호선(급행·특급 복수 패턴)은 필드 부재** — 용산→동인천 1콜, 1호선 leg 전부에 `expressStops`·`expressStopIds` 키 없음(소비자가 검증할 수 없는 계약이라 생산자 게이트가 정본, transit-guide 요청).

exit 코드: 0 통과 / 1 위반·호출 불가 / 2 ODsay 429(쿼터 — 어느 단계에서 만나도). 429는 재시도하지 않는다. ⚠ 2026-09-02 첫 실행은 25/26(FAIL 1건 = 버스 환승 경로에 중앙보훈병원 하차 출구를 기대한 게이트 단언 결함, 수정), 수정본 재실행은 같은 날 ODsay 쿼터 소진(429)으로 미실측 — 다음 주간 재관측(A22 루틴)이 첫 통과 실행이다. 실행 시점(리뷰 #17): 머지 전 필수 + A22 주간 재관측에 `verify-odsay-transfer-door.mjs`와 함께 돈다(`docs/BACKLOG.md` A22).
