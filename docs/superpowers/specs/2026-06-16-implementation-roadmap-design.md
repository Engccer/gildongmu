...
| 서울 **실시간 지하철 인증키** | **미발급 (2026-06-17 재확인)** — 따릉이 키로 `ERROR-338 "해당 인증키로는 실시간 서비스를 사용할 수 없습니다"`. data.seoul.go.kr 실시간 데이터 인증키 별도 신청 (사용자 직접). A2 보류 레인 | A2 서울 지하철 실시간 |
| `DATA_GO_KR_API_KEY` (=`TOUR_API_KEY`) | 보유. 단 서비스별 활용신청 필요 | B1 소아진료 등 |
...
3. **A2** 서울 지하철 실시간 — **보류 레인**(실시간 인증키 차단, `ERROR-338`). 사용자 키 발급 후 재개. 역 식별은 A3 seed 받침대 활용 가능.
4. **B1** 소아 야간·휴일 진료
5. **C1** NCP en 자동차 경로
6. **B2** 공기질·혼잡도 → **B3** 키즈 장소 → **C2** 영문 도로명주소

대기 레인(A2 실시간 지하철·서울 버스·KRIC)은 외부 게이트가 풀리는 즉시 끼어든다.

## 5. 사용자 액션 아이템 (병행)

- **[사용자, A2 차단 해제]** data.seoul.go.kr 로그인 → "지하철 실시간 도착정보" **실시간 데이터 인증키** 신청. 일반 인증키(따릉이용)와 **별개 신청 메뉴** — 따릉이 키로는 `ERROR-338`로 차단됨(2026-06-17 실측). 발급 후 키를 제공하면 A2 즉시 착수.
- ~~서울 포털 서울교통공사 교통약자 시설 데이터셋 활용신청(A1)~~ ✅ 완료.
- ~~data.go.kr 15013205 활용신청(A3)~~ — 불요(XLSX 정적 데이터, 게이트 없음).

## 6. 진행 현황

A1·A3 완료. **A2만 사용자 키 발급 대기로 보류.** A2 착수 조건: 사용자가 data.seoul.go.kr 실시간 인증키를 발급·제공 → swopenapi 실호출 검증 → 버스 근접/도착 패턴 재사용으로 구현. 역 식별·영문역명은 A3 seed(`findStationMeta`)를 받침대로 쓴다.
