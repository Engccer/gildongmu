...
**받아쓰기 중 톤 억제 (C6)**: 목적지 검색 시트에 `SpeechService` + `HoldDictationButton`이 있고, §5.5에 따라 시트 표시 중에도 추적은 계속된다. 헌장 §6의 "녹음 중 발화 0"은 이 repo가 세 번 데인 계열이고 스피커→마이크 오염은 실측된 경로다. `SpeechService`가 청취 중이면 **톤과 통지를 모두 건너뛴다**(추적·상태 텍스트는 유지).

⚠ **개정(2026-08-01, 구현 리뷰 I-9)**: 초안은 "통지는 유지"였는데 **틀렸다**. 헌장 §6의 불변식은 "녹음 중 SR 발화 0"이고, polite 통지는 VoiceOver가 실제로 내는 **음성**이라 톤보다 전사 오염 위험이 오히려 크다. 헌장이 상위 정본이므로 통지도 억제한다. 억제 구간에 발생한 통지는 폐기가 정답이다(맥락 밖 지연 발화 금지).
