...
- ⚠ **override는 "지금 안내 중인 구간"에만 적용된다** — `waitContextText`는 다음 구간 안내(`nextLeg`·`legAdvanced`)에도 쓰이므로, 범위를 열어 두면 다음 구간 안내가 **이전 구간에서 고른 역**을 말한다. 지금 그 시점에 override가 nil이라 무해하지만 **그 무해함은 이 함수가 표현한 불변식이 아니라 board 소거에 기댄 우연**이라, §3.1의 `phase == riding` 가드를 자립적 표현으로 남긴 것과 같은 논리로 여기서도 자립시킨다(2026-08-16 독립 리뷰가 그 비대칭을 지적했다). 인자 `isCurrentLeg`에 **기본값을 두지 않는다** — 생략이 통과하면 그 결함이 조용히 들어온다([[no-default-for-safety-parameters]]).
...
3. `upstreamFailed` 중 시한 경과 → 덮지 않음, **그리고 복구된 그 폴에서도 내지 않음**(다음 폴에서 발화). 한 시나리오가 두 축을 함께 검증한다 — 실패 폴(`at:900000`)과 복구 폴(`at:960000`)이 이어져 있다.
...
**spec 준수 리뷰 — 뒤늦게 도착(2회 요청 후). MAJOR 1건 + spec 구멍 5건을 추가로 잡았고 전부 반영했다**(커밋 `d9217f0`):
- **MAJOR**: `reboardPickerActive`가 국면을 넘어 살아남는다 → 국면 기반 소거 + 재현 테스트(변이 검출 확인). **`boardOverrideName`은 정확히 이 위험 때문에 중앙화하고 주석까지 적어 놓고 그 짝을 빠뜨린 것**이라 지적이 뼈아프다.
- **spec 구멍 5건**: 픽커 수명 미규정 · §3.4 두 항목이 서로 배타 · 코드 블록이 구현보다 약함 · 지하철 전용 제한 미기재 · 5번째 키 누락 · 불변식 개수 오타.
- 나머지 MAJOR 2건(`waitContextText`·iOS 취소 포커스)은 품질 리뷰와 **독립적으로 같은 결론**에 도달했고 이미 반영돼 있었다 — 두 리뷰어가 겹쳐 찾았다는 사실 자체가 그 둘의 실재성을 뒷받침한다.

⚠ **아래는 그 응답이 오기 전에 컨트롤러가 직접 검증한 기록이다. 남겨 두는 이유는 자기 검증의 한계가 실측으로 드러났기 때문이다** — 나는 아래 범위를 "전부 충족"으로 판정했는데, 그 뒤 도착한 독립 리뷰가 MAJOR 1건과 spec 구멍 5건을 더 찾았다. 검증 범위가 좁았던 게 아니라 **자기 산출물을 보는 관점으로는 안 보이는 종류**였다:
...
5. **포커스 착지 2종**(역 선택 진입 `landReboardPromptFocus` · 취소 복귀 `landChangeBoardingFocus`) — 정본 시퀀스를 따르지만 **시뮬레이터로는 검출 불가**한 계열이고, 실패하면 커서가 body로 떨어져 SR 사용자만 맥락을 잃는다. 판정 근거는 `transitGuideLog`의 `landed=` 값이라 1~4와 **한 번에 확인된다**(독립 리뷰 마감 권고).
