...
- **실호출 게이트 2026-08-23(일요일 다이어 `03`)**: `node scripts/verify-korea-subway-timetable.mjs` **15/15 PASS**(리뷰 반영 뒤 lineName 유일성 단언 추가, 18/18 PASS). 관측 — 강남: 신분당선 `unknown`·2호선 ok / 홍대입구: 공항선 ok·경의중앙선 ok·2호선 `unknown` / 서울역: 공항선 `unknown`·GTX-A ok(하행만)·경의중앙 ok(상행만)·1호선 ok·4호선 ok. 탈락 0, `directions` 비지 않음 ⇔ `ok` 성립. 로컬 dev 서버 경유 `/api/station/timetable` 응답도 동일.
