...
작성 2026-06-20. 기존 공기질(B2, `2026-06-17-air-quality-design.md`)을 "이 지역 날씨" 단일 섹션으로 확장한다. 게이트: data.go.kr 기상청_단기예보조회서비스(15084084) **활용신청 필요**(자동승인 ~5분, 기존 `DATA_GO_KR_API_KEY` 공유) → 실호출로 계약 잠금(아래 §2, **검증 대기**).
...
## 2. 잠글 계약 (실호출 검증 — 활용신청 후 잠금, 현재 **대기**)

격자 변환 함정과 2-오퍼레이션 체인이 핵심. 공기질 좌표계 함정과 동형으로, **문서가 아니라 실응답으로 확정**한다.
