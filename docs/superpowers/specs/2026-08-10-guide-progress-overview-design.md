...
## 3. 전 구간 목록 (iOS 시트, 버튼 펼침)

- 진행 상황 버튼 누름 = 짧은 낭독 + 목록 펼침(이미 펼쳐져 있으면 낭독만). 발견 경로가 버튼이므로 landmark 없음(헌장 §3).
...
- 표식은 fix 갱신에 따라 이동하되 **통지는 없다**(live region 아님 — 조회형 정보).
- 목록 끝 "경로 목록 닫기" 버튼. 닫으면 포커스를 진행 상황 버튼으로 선점 이동(누른 결과로 자신·목록이 사라지는 전이, 헌장 §5).
- 경로 미보유 세션(간략 전용)은 목록 없음 — 버튼 동작은 현행(직선거리 낭독)뿐.
