...
- 닫기: 스와이프·VO escape·"닫기" 버튼 **상하 2개**(라벨 개정 2026-08-11 — 실보행 중 낭독 길이를 줄이려 관례 키 `actions.close` 재사용, 전용 키 `ios.guide.routeListClose` 폐기)(위원장 판정 2026-08-10 — 행 수가 많으면 말미까지 스크롤 압박). 상단 닫기는 나브바 toolbar가 아니라 헤더 다음 첫 행이다 — 나브바 요소는 섹션 헤더보다 먼저 착지 후보가 되어 "모달 착지 = 조망 낭독" 계약을 깬다. 닫으면 시스템 표준 dismiss가 트리거 버튼(진행 상황)으로 포커스를 복원한다(실기기 확인은 BACKLOG 판정 ⑤).
...
| iOS extra | `ios/i18n/ios-extra/{locale}.json` | `ios.guide.routeListRow`·`routeListCurrent` 신설 |
...
## 8. 실보행 라운드1 반영 (2026-08-10 아침, 자택 → 신명중학교)

위원장 실보행 스크린샷 5장 판정 4건. 전부 당일 반영.

1. **"현재 안내" 행은 상태 유도다(발화 이벤트 연동 폐기)**: §4 초판이 행을 `announceSteps`에 연동했는데, 그 이벤트는 경계 40m **선행** + 1회 **래치**라 "지금 구간"과 어긋난다. 짧은 구간에서는 15초 재통독(`bundleReread`)이 선행 낭독분을 도로 덮은 뒤 다시는 갱신되지 않았다(마지막 구간 내내 직전 횡단보도 안내가 잔류한 실사고 — 스크린샷으로 확정). 행은 매 fix에서 `stepAt(d)`의 유닛으로 유도하고(following·bundle 국면만), 발화 채널은 불변.
2. **상태 행 주기 예고에 "다음 안내," 라벨**(iOS 표시 전용): 상태 행은 예고·임박·상태 전이가 섞여 라벨 없이는 종류를 알 수 없다. 특히 마지막 구간의 "{dest}까지 약 N m"는 남은 거리 행과 숫자만 어긋난 채 나란히 보였다. 임박 명령·상태 문구는 문장 자체가 종류를 말해 무라벨 유지. 음성 어순 계약(2026-08-07)은 불변. 웹은 liveText가 발화 채널이라 비대칭 수용.
3. **횡단보도·지하보도 재작성은 행동 동사 우선**: "{거리} 이동, 횡단보도 이용" → "{어디서} 횡단보도를 건너세요, {거리}"(지하보도는 "지하보도로 건너세요"). 임박 큐와 같은 동사, 병합 게이트(`MERGED_CROSSWALK` — "횡단보도 N개" 매칭)·임박 분류("횡단보도" 마커) 계약 유지.
4. **임박 큐 10m → 25m**(위원장 25m 지정): GPS·투영 지연 ~15m가 10m를 잡아먹어 실위치 기준 회전 지점을 지난 뒤 발화했다 — 따르면 차도 진입인 위험 실사고. 공유 시나리오 fixture를 재계산했고, "최종 접근이 큐를 선점한다" 한계 케이스는 25m에서 좁아져(큐 자격선이 진입선보다 앞이면 큐 생존) 기하를 조정해 한계를 계속 잠근다. 상수는 계속 실보행 판정 대상.
