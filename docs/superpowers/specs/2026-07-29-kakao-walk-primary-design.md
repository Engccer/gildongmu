...
- **재캘리브레이션 완료(2026-07-29, 저장된 실호출 4구간 × OA-15543 seed 13개 횡단보도 스텝 대조)**: 판정 좌표는 **횡단보도 스텝의 `path.points` 전체 중 seed 최근접점, 반경 40m 유지**. 첫 점 기준은 신호기 실재 개연 스텝(길동사거리앞교차로 45.5m)을 미탐하고, 전체 점 기준은 분포가 32.5m 이하(양성 군) vs 91m 이상(무신호 군: 한강공원 진입로 91m·천호대교분기점 194m)으로 **완전 분리, 거짓 양성 0**. "긴 스텝 우연 매칭" 우려는 판정 대상이 횡단보도 문구 스텝뿐이라 성립하지 않음(실측 폴리라인 5~60m·점 2~3개). 이에 따라 step 내부 좌표 계약은 단일 `coord`가 아니라 **`pathCoords`(폴리라인 배열) 보존**이며 Tmap은 기존 단일 coord 유지(주석 판정이 두 형태 모두 수용).
- **병합·복수 횡단보도 스텝("2개의 횡단보도 이용" 류 수량 표현)은 주석 생략**: seed 1개 매칭으로 문장 전체에 "음향신호기 있음"이 붙으면 나머지 횡단보도에도 있다는 거짓 안전 정보가 된다(실측 병합 스텝 4건 모두 5~15m 매칭이나 어느 횡단보도인지 특정 불가). positive-only의 연장: 특정할 수 없으면 침묵.
...
4. 주석: 단일 횡단보도 스텝 pathCoords 최근접 매칭·수량 표현 스텝 생략·좌표 없는 스텝 생략·Tmap 단일 coord 경로 회귀 0.
...
4. 주석 재캘리브레이션: ✅ 사전 실측 완료(위 §음향신호기, 2026-07-29). 구현 후 prod 응답에서 규칙 적용 결과만 재확인.
