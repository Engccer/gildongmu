...
- ⑤ 자동차 en(NCP) + `via`는 `unsupported:"waypoint"`다: NCP Directions에 경유지 계약이 없고, ko 서비스(Tmap·카카오)로 내리면 영어 사용자에게 한국어 안내문이 가 LLM이 도로명을 번역하며 오역할 위험이 생긴다(리뷰 검출). 정직 마커가 한국어 안내보다 낫다. 도보는 원래 ko 전용이라 해당 없음.
- ⑤ 도보 스텝 절단(`WALK_STEPS_CAP` 20)은 경유지 도착 스텝까지 늘린다 — `via.stepIndex`가 받은 배열 밖을 가리키면 존재하지 않는 스텝을 추론하게 된다(리뷰 검출).
