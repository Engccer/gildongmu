...
| 2 | 검색 입력 | `TextField(state = viewModel.queryState)` — 값은 `StateFlow`가 아니라 **`TextFieldState`**(공식 지침: 비동기 왕복은 한글 자모·점자 IME 조합을 깨뜨린다). 라벨 `search.label`("장소 검색"), placeholder `android.search.prompt`. `singleLine`(`lineLimits = SingleLine`) + `KeyboardOptions(imeAction = Search)` + `onKeyboardAction`이 제출이고, **물리 Enter**(한소네 점자 키보드 = 하드웨어 키)는 `onPreviewKeyEvent(Key.Enter)`로 같은 제출을 부른다(IME 액션이 하드웨어 Enter에 매핑되는지는 실기기 §8-7 확인 — 되면 폴백을 지운다). 입력이 있으면 뒤에 지우기 버튼(`search.clear`, 누르면 입력으로 포커스 복귀) |
