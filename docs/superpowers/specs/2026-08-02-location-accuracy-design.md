...
⚠ **검색도 reduced 좌표를 받지 않는다.** "순위 가중이라 대략의 좌표라도 낫다"는 논리가 여기서는 성립하지 않는다. 같은 함수의 다른 소비자(`loadCurrentAddressIfAuthorized`)가 그 좌표를 **역지오코딩해 "현재 위치, 〈주소〉"로 표시**하기 때문이다. km급 좌표로 만든 주소는 화면으로 반증할 수 없는 **거짓 위치 주장**이 되고, 그것은 순위가 나빠지는 것보다 나쁘다.
