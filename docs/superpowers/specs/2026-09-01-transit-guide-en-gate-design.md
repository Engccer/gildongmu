...
- ⚠ **타입 분리로 덮이지 않는 방향이 하나 있다**(코디네이터 지적 2026-09-01): 좁은 투영(§3.5)은 *우리 코드가 조인 값을 읽지 않는다*만 보증하고, **서버가 영문 자리에 한국어를 실어 보내는 방향**은 타입을 그대로 통과한다(seed 영문이 비거나 provider 문구가 바뀌면 `Boarded 수도권 5호선`이 "영어 줄"로 판정된다). 경로 축은 E27 §3.1의 "`*En` 값 자격 = 한글 미포함"이 이미 막지만 **실시간 축에는 그 검사가 없었다** — `transit-track.ts`의 `englishFieldOnly`가 fail-closed로 막고(빈 값·한글 섞인 값은 싣지 않는다) 단위 테스트가 그 가드를 직접 검증한다. ⚠ 이 축은 리듀서 sentinel로는 못 잡는다: `enrichArrivalEn`이 표에서 값을 다시 만들어 주입한 한글이 닿지 않기 때문이다(실측). "타입이 정본, 스캔은 보조"가 틀렸던 선례([[brand-types-block-forgery-not-substitution]])와 같은 계열이다.
