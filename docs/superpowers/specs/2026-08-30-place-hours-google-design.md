...
> **2026-09-02 정식판 승격(위원장 판정)**: `AppConfig.experimentalPlaceHoursEnabled` 검사 삭제(플래그 졸업), 웹 장소 상세에 같은 한 줄(`src/components/PlaceHoursLine.tsx`, 3초 상한·침묵 계약·attribution 동일), 문구 4종은 `messages/*.json` `placeHours.*`로 옮겨 웹·iOS 공유, privacy 문안에서 "iOS 실험판" 삭제. 아래 본문의 "실험판 전용" 서술은 그 시점의 기록이다.
