...
⚠ **`navSeq`를 다중 출입구 인덱스로 읽지 말 것**(2026-08-08 저녁 실호출로 반박, 상세는 백로그 §H M1): 실측값이 0·1·2로 나오고 POI 종류를 따라간다. 같은 단지의 101동·102동 `front`가 2.45m 차이로 사실상 동일하다. 반대로 **`front`가 주출입구를 정확히 집는다는 것은 2사례로 확증**됐다(자택 아파트 정문 쪽 봉래면옥까지 62.2m→15.1m, 신명중·초 두 정문이 대각 75.3m→나란 34.1m). **목적지 좌표 보정량도 이 절의 8.77m이 아니라 52~105m다**(4곳 실측).
