...
**개정 2026-08-11 (A10)**: 진입 조건에 **¬투영점프**가 더해지고, 투영 점프 판정의 소유가 오케스트레이터에서 **리듀서로** 옮겨졌다(`GuideTuning.maxSpeedMps`, 판정 결과는 `GuideOutput.projectionJumped`로 노출).

- **왜**: 종전 구조는 리듀서가 `phase = finalApproach`를 커밋한 **뒤에** 오케스트레이터(`BeaconModel.handleDetail` / `useRouteGuide.stepDetail`)가 점프 fix의 진입 이벤트를 거부했다. 거부돼도 phase는 이미 잠겨 0a 가드가 이후 모든 판정을 멈추고, `inFinalApproach`는 false라 최종 접근 층도 돌지 않는다 — **세션 영구 정지**(2026-08-11 하교 실보행: 종점 3초 전 d가 +94m 튀며 진입 fix와 겹쳤고, 잔여 4m 표시 동결·도착 무통지. 로그 `logs/guide-diag-2026-08-11.log.gz` 08:26:09~39Z(UTC 표기 — KST 17:26, 하교 시간대)). "이 fix만 버리면 다음 fix에서 다시 온다"는 종전 주석의 전제는 d 단조 전진 때문에 애초에 성립하지 않았다.
- **리듀서 이관으로 별도 기준값이 사라진다**: 직전 진행거리 = `state.d`, 직전 시각 = `state.lastFixAt`이 이미 상태에 있어(단조 전진이라 감소 방향 없음) 오케스트레이터의 `lastRemaining` 기준값·매 fix 1회 호출 계약이 통째로 불필요해진다. 진입 이벤트와 phase 전이가 리듀서 한 자리에서 원자적이 되어 이 결함 계열이 구조적으로 불가능하다.
- **점프 fix의 나머지 처리는 불변**: d 커밋·낭독·이탈 판정은 종전처럼 진행하고(오케스트레이터도 점프 fix를 통째로 버린 적은 없다 — 진입 거부와 추세 톤 억제뿐이었다), 진입만 한 fix 미룬다. d가 유계라 반복 점프는 자기 종결된다 — 지속 점프로 진입이 영구 억제되는 경로는 없다(다음 fix에서 전진 0 = 점프 아님 → 진입).
- **경계표**: 공유 fixture "최종 접근: 진입 fix의 투영 점프는 진입을 한 fix 미룬다(A10)"가 両플랫폼을 잠근다(변이 주입으로 검출 확인: 게이트 제거·계수 파손 모두 웹·Kit에서 fail).
...
| Tmap POI `frontLat`/`frontLon` | WGS84 | **키 보유** | 전국, **주출입구 1개**(실측 확증) | ⚠ 약관 **"저장 후 24시간 이상 사용 금지"** → seed 불가, 런타임 조회 전용. ⚠ `endPoiId`는 경로를 바꾸지 않는다 — front 좌표를 목적지로 직접 넘겨야 한다 |

⚠ **이 절의 "출입구로 올리면 정밀해진다"는 전제는 단일 출입구 시설에서만 참이다**(2026-08-08 저녁 실호출 + 위원장 정정, 상세는 백로그 §H M1). Tmap `front`가 집는 것은 주출입구가 아니라 **차량 진입점**이고, 보행 전용 출입구는 담기지 않는다. 목적지를 `front`로 바꾸면 **그 반대편에서 접근하는 사용자의 도보 거리가 63m에서 146m로 늘어난다**(자택 아파트 실측). 보정량 자체는 이 절의 8.77m이 아니라 52~105m로 크지만, **방향을 모르는 보정은 개선이 아니다.** `navSeq`도 다중 출입구 인덱스가 아니다(값이 0·1·2, POI 종류를 따라간다).
...
6. ~~**랜드마크 POI 폴백 도입 여부**(§3.7)~~ — **2026-08-08 판정 완료**. 위원장이 용도를 구체화했다: 촉지 표지가 아니라 **인지·확인 기준점**("봉래면옥 옆에 아파트 정문이 있습니다")이므로 §2.5의 촉지 가능성 기준이 적용되지 않는다. 폴백이 아니라 **별도 축**으로 도입한다(백로그 E9). 남은 미정은 도입 여부가 아니라 **어느 POI를 고르는가**이고, 그것은 실사용 튜닝 대상이다.
...
## 7.5 이 spec의 범위 밖 (후속 마일스톤)

**이 문서는 "경로 종점 → 목적지 한 구간"을 닫는다.** 위원장이 2026-08-08에 확정한 상위 의도는 그보다 넓다:

> "최대한 많은 데이터를 효과적으로 사용하여 사용자가 도착지 부근에 왔을 때 **목적지뿐 아니라 근처 상황을 재구성**해 주려고 하는 게 내 의도야."

그 범위는 **백로그 §H M1**이 관리하고, §3.7(랜드마크)·§3.8(출입구)은 거기에 흡수된다. 단독으로 확장하지 말 것 — 실측이 그 셋을 따로 두면 각각 반쪽으로 끝난다는 것을 보였다(자택는 정문과 샛길이 서로 다른 POI에 대응해, "가장 가까운 POI 하나"로는 어느 출입구로 가는지를 못 정한다).

**수단 확장**(자동차·대중교통)은 백로그 §H M2다. 도보 완결이 선행 조건이고, 자동차는 실주행이 딥링크 위임이라 최종 접근이 붙을 자리가 **주차장 이후 도보 구간**이지 차량 경로의 끝이 아니다.
