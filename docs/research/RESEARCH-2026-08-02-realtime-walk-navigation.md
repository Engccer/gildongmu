...
## 10. 다음 세션 인수인계 (E4 착수 가이드, 2026-08-03 작성)

**이 절이 다음 세션의 시작점이다.** 조사는 완결됐고 구현만 남았다. 백로그 E4와 이 문서가 정본이며, 본문(§1~§9)을 다시 조사하지 말 것.

### 10.1 착수 순서

1. **위원장 판단 2건을 먼저 받는다** (백로그 E4 선행 조건. 코드가 아니라 판단이다):
   - **약관**: 한 안내 세션 동안 경로를 메모리에 유지하는 것이 카카오 "저장"인지 유권해석이 없다(§8.2). 확정하려면 devtalk 질의가 필요한데 **외부 발신이라 하드 스톱이다**. 질의 여부·문안을 위원장이 정한다. Tmap은 24시간 명문이라 문언상 안전, 폴백이 Tmap이므로 "카카오 불명 시 Tmap 단독으로 시작"도 선택지다.
   - **백그라운드 위치 승격 여부**: 주머니에 넣고 걸으려면 필요하다. ⚠ 비콘 설계 §8.1의 기각 근거였던 "항상 허용 권한 필요"는 **사실이 아니다**(§6.1: When In Use + `UIBackgroundModes: location`으로 성립, 파란 표시줄은 못 끔). 전경 전용으로 시작하고 실보행 후 승격 판단도 가능하다.
2. **brainstorming → spec** (superpowers 절차). 설계 골격은 §5가 이미 제공한다: 폴리라인 보존 → 진행거리 추적 → 개별 안내 유지 + 긴 구간만 자동 발동 → 마지막 구간 비콘 인계 → 수동 제어는 "반복" → 이탈 시 자동 재조회 금지.
3. **spec 적대적 검토** 후 구현. 웹·iOS 동시 착수(백로그 B 축 갭 예방 규칙).

### 10.2 이 조사 이후 달라진 지형 (2026-08-02 밤 ~ 08-03, 설계에 반영할 것)

조사 본문이 전제한 코드가 그 뒤 크게 바뀌었다. **§5 설계 스케치는 유효하지만 아래를 딛고 선다.**

- **거리 표기가 소수 km 원값으로 바뀌었다**(`1.1km`·`6.285km`, 위원장 실사용 판정). 안내문 낭독 설계 시 §5.2의 확신도 3단("200m"/"약 200m"/"200m쯤")은 이 표기 위에 얹는다.
- **낭독 인프라가 이미 있다**: iOS `distanceText()`(시각 원문 + m만 로케일 단어로 푸는 `accessibilityLabel`)와 통지 경유 `spokenUnits()`. 실시간 안내의 발화 문장도 **반드시 이 채널을 태운다**(VO가 숫자+m을 minutes로 오독, 실기기 확정). ⚠ 정규식은 `(\d)m(?![A-Za-z])`이며 `\b`는 한글 직결("35m입니다")에서 불성립.
- **위치 계층이 전면 개편됐다**(A3, `LocationService`): fix가 `StoredFix(coord, accuracy, fixedAt)`로 정확도·시각과 함께 저장되고, 단발 취득은 게이트(30m·10초)+타임아웃+세대 토큰, 소비자별 진입점(`coordinateForRanking`/`coordinateForDisplay`)이 갈렸다. **E4의 진행거리 추적은 비콘과 같은 연속 모드(`startBeaconUpdates`)를 쓰면 되고, 이 개편과 충돌하지 않는다.**
- **정밀 위치 꺼짐 처리 완비**: reduced 상태는 `reducedAccuracy` 별개 상태 + "정확한 위치 허용" 시스템 팝업(임시 정밀 허가)이 이미 배선돼 있다. E4는 시작 게이트에서 비콘과 같은 검사(`accuracySnapshot`)를 재사용하면 된다.
- **비콘 재사용 전제 재확인**: §5.1-3의 "비콘 타깃을 현재 단위의 끝으로" 전략에서 재사용할 스택은 Kit `beaconStep`·`beaconGateStep`·`BeaconTones`·워치독·`BeaconModel`의 생명주기(전경 전용·중지 계약·받아쓰기 중 억제). 전부 실보행 검증 완료 상태다.

### 10.3 첫 구현 단계 (스펙 확정 후)

폴리라인 보존이 최소 선행 작업이다: `src/lib/walk-route.ts` `annotateAudioSignals`가 주석 판정 후 `coord`·`pathCoords`를 **제거하는 지점**(현재 응답 스키마 유지 목적)을 옵트인으로 바꾼다(예: `includeGeometry` 파라미터. 기존 소비자 스키마 byte-호환 유지, CLI/MCP 출력 팽창 방지). 스텝 이음매 0.00m·기하 신뢰도는 §1에서 실측 완료라 재검증 불요.

### 10.4 실측 재현 좌표·수치 (재실측 불요)

- 구간 분포: 109개 구간 중 25m 미만 21%·p10 12m·중앙 62m(§2)
- 병합 함정: 흡수식 병합은 횡단보도를 577m 앞당긴다(§3). 안내는 개별 유지, 합치는 것은 GPS 발동 지점뿐
- 조회 지연 평균 77ms · 쿼터 일 1,000건(dodo 공유, 초과는 429 차단이지 과금 아님)
