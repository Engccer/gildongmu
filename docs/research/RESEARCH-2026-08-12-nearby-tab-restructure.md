...
적용 대상은 **App Store 배포판 = iOS**다. 웹에도 같은 구조가 있으나 요청 범위 밖이다(§6 플랫폼 갭 참조).
...
웹 `src/components/NearbyHub.tsx`는 같은 11개 구조를 아코디언으로 갖는다(`canShow*` prop 10개 + `WalkInfraNearby`). iOS만 재편하면:

- 웹에는 "현재 위치 확인"과 "둘러보기"가 따로 남는다
- 웹에는 "주변 보행 인프라"가 남는다
- 웹에는 "한눈에 보기"가 없다

백로그 축 4(플랫폼 갭)에 3건이 신규 등재된다. **이것이 의도된 분화인지 후속 이식 대상인지**가 판정 사항이다(선례: "웹 거리 추적도 모달로"는 웹 주 사용처가 컴퓨터라는 근거로 폐기됐고, iOS·웹 UI 구조가 갈리는 것이 의도로 인정됐다 — 같은 논거가 여기 성립하는지는 별개다).

또한 `SurroundingsScene.tsx`(웹)과 `SurroundingsSceneSection.swift`(iOS)는 M1 이식으로 계약을 공유한다. R6·R7이 iOS만 바꾸면 그 미러가 깨진다.
...
1. **조망 5종의 실제 수치가 조망으로서 값어치가 있는가.** "식당 3곳"이 500m 안에서는 늘 두 자리인 상권과 늘 0인 주거지에서 각각 무엇을 알려 주는가는 실호출·실사용으로만 판정된다(판정 기준 5, 데이터 실재 축).
2. **"한눈에 보기"가 통계인가 명명인가.** "식당 및 카페 12곳"과 "가장 가까운 식당은 남쪽 40m"는 다른 물건이고, 후자면 조망이 아니라 요약 목록이다. 위원장 의도 확인 대상.
