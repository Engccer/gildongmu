...
**R4의 성과 정의**(위원장 확정 2026-08-12): "한눈에 보기"는 **브리핑**이다 — *가까운 곳에 어떤 POI가 있는지 핵심만 빠르게 확인하는 간편한 섹션*. 통계가 아니다(§3.5).
...
### 3.5 "간편한 브리핑"이 뜻하는 것 — 낭독 시간은 병목이 아니다

⚠ **위원장이 말한 "30초"는 비유다**(2026-08-12 확인 — "그만큼 핵심 정보를 빠르게 확인할 수 있도록 간편한 섹션으로"). **초를 세는 수용 기준으로 굳히지 말 것.** 아래 실측은 예산 계산이 아니라 **시간이 병목이 아님을 확인해 제약의 정체를 가려낸 것**이다.

`say -v "Jian (Premium)" -o *.aiff` + `afinfo`로 한국어 낭독 시간을 쟀다(2026-08-12):
...
**한국어 약 0.10초/자**(r=250, 보수적). 5불릿 × 48자 = 약 25초. 반경을 병기한 가장 긴 후보도 낭독 부담이 되지 않는다.

**따라서 "간편함"은 시간 제약이 아니라 형태 규율이다.** 지킬 것은 **불릿 5개 · 항목당 한 문장**이고, 짧게 쓰는 이유는 낭독이 길어서가 아니라 훑어 넘길 수 없는 채널에서 문장이 늘면 그 자체로 부담이기 때문이다. 이것이 §3.3의 3-state 문제를 조인다 — "0건"과 "정보 없음"과 "실패"를 **한 문장 안에서** 구분해야 하고, 설명을 덧붙여 해소할 여유가 없다.
...
⚠ **묻는 방식이 정해져 있다**(위원장 지시 2026-08-12): spec 작성 시 아래 항목을 **쉬운 말로 하나씩** 물어 위원장이 결정한다. 목록을 통째로 내밀지 않는다 — 한 질문 = 한 결정, 답을 받고 다음으로. 메모리 [[explain-designs-in-plain-language]].
