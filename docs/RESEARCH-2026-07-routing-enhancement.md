...
| 순위 | 항목 | 비용 | 핵심 근거 |
|---|---|---|---|
| 1 | 택시 요금·통행료 표기 | 키 0, 필드 투영 | 카카오 응답 `summary.fare.taxi`/`fare.toll`, NCP `taxiFare`/`tollFare` 이미 수신 중 |
| 2 | ODsay 정류장별 리스트·배차간격 | 키 0, 필드 투영 | "N개 정거장 뒤 하차" 낭독. 같은 키·쿼터 내 |
| 3 | 도보 길찾기 신설 (Tmap 보행자) | 신규 키 1 | 유일한 한국어 완성 문장 턴바이턴. 최대 공백 해소 |
| 4 | 돌발상황 브리핑 (ITS 15040465) | 기보유 키+활용신청 | 사고·공사·통제를 자동차 미리 듣기에 합성 |
| 5 | 지하철 첫차/막차·시간표 | 기보유 키+활용신청 | "막차까지 N분" 텍스트 완결 정합 |
| 6 | 지하철 엘리베이터 위치·음성유도기 seed | 무료(파일+API) | 역 상세 접근성 시설 보강 |
...
- **택시 요금**: 카카오 자동차 응답 `summary.fare.taxi`(예상요금 원)·`fare.toll`(통행료), NCP `taxiFare`/`tollFare`(en 커버). `/api/route/car` 브리핑에 투영만. 조사 전체 최저 비용.
  - 장애인콜택시(서울시설공단)는 일별 통계 API만 존재(운행대수·평균대기시간 등, 최소 하루 전 데이터). 실시간 호출·대기 연계 불가. 참고 표기 이상은 불가.
- **ODsay 미활용 응답 필드**: subPath 정류장별 리스트(하차까지 정거장 수 낭독), 평균 배차간격. 그 외 카탈로그: 지하철역 전체 시간표(첫차/막차), 환승정보(환승 소요시간), 고속/시외버스·열차·항공 운행정보, 대중교통 접근성 영역(isochrone). 전부 기보유 키·일 1,000회 쿼터 공유.
