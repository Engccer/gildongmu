...
- **그 시트의 착지 대상은 상태 문장 행 하나다**(E38 위원장 판정 2026-09-12 — 무엇을 누르든, 하차 도착도 [다음 구간]이 아니라 문장에 앉는다). 남은 판정은 여부뿐이라 술어가 Bool(`phaseTransitionLands` ↔ 웹 `lands`)이고, 살아남은 다른 대상은 **자기 질문을 여는 화면**(역 선택·급행 확인·차량 선택 라벨)과 띠바 복귀·목적지 전환 상태 행뿐이다. ⚠ 대상이 하나면 착지 테스트의 검출력이 0이 된다 — **누르기 전에 그 컨트롤로 커서를 옮겨야** "착지했다"와 "거기 있었다"가 갈린다. → PATTERNS
