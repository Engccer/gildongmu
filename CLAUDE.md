...
- **안내 시트 상태 문장은 한 조립기(`arrivalStatusLine`)가 만들고 통지는 그 문장에 없는 것만 말한다**(E39·E41): 잔여+도착은 한 줄 두 조각(쉼표 결합, 결합용 키엔 마침표 없음), 언제나 참인 조각 생략(대기 `stateNotYetVisible`·추적 중), 도착 국면은 접근 정보 전부 생략, 서울버스는 `parseBusArrmsg` 구조로 조립(미지는 원문 병치), 수단 낱말은 `*Bus` 키로(라벨 합성 금지). 시작 통지는 목적지를 말한다(E40, 3수단). → PATTERNS
