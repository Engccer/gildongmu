...
- **결과 진동은 `ResultHaptic.fire(.success|.attention|.failure)` 한 창구다**(E30 확장, iOS 표준 3종만·스위치 `TrendHaptics.storageKey` 뒤). 통지를 게시하는 자리에 1회성 결과·전이에만 붙이고 반복 상태 통지에는 넣지 않는다. 3-state를 촉각에도 지킨다(있음·없음·실패가 다른 진동). 제너레이터 직접 생성은 기존 발원지 4곳뿐이고 `result-haptic-guard.test.ts`가 잠근다.
