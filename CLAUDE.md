...
- **iOS 채팅(탭·장소 sheet)은 헌장 §6 "대화형 UI"가 정본**(실측 근거·iOS 번역표는 `~/.claude/reference/accessibility.md`). repo 구현 정본: 산문 블록 분할 `parseChatMarkdownBlocks`(GildongmuKit)+블록별 `Text`(헤딩 `.isHeader`), 질문 말풍선 헤딩 trait, 포커스 계약=전송 시 보내기 버튼 유지·완료 시에만 질문 헤딩 이동(`questionRevision`/`answerRevision` 신호, VO 실행 중 완료 스크롤은 질문 상단으로 단일화+400ms 후 포커스+1회 재시도, `ios/Gildongmu/Chat/ChatConversationView.swift`, 계측은 `ChatFocusDiag.swift`), 마이크는 WhatsApp식 홀드(`HoldDictationButton.swift`, 2026-07-20 탭 토글 대체): 누른 동안(250ms 성립) 녹음·떼면 입력, 채팅만 위로 밀어 잠금·왼쪽으로 밀어 취소, 짧은 탭은 사용법 안내(청취 중이면 정지). **녹음 시작 순간 interrupting 무음 통지로 VO 낭독을 끊고 세션 중 접근성 라벨 불변(받아쓰기 중 VO 발화 0이 핵심 계약)**, 상태 신호는 소리·햅틱만. 라벨은 정적 "받아쓰기"("음성 입력"류 내용-라벨 충돌 금지 유지). 받아쓰기 완료=결과 원문 polite 통지(채팅은 보내기 버튼 포커스 이동 선행, 검색은 자동 검색+settled 포커스라 통지만 — `SpeechService`엔 자동 정지 경로가 없어 반환값 소비 유지), 취소는 종료 후 `voice.cancelled` polite 통지.
