...
- **서울버스 "곧 도착"(잔여 0)은 정차가 아니라 직전 정류소 출발이다**(A41 실호출 2026-09-12: `isArrive1` 0 고정 — 정차 신호가 API에 없다). 리듀서는 잔여 0을 임박 이벤트(`arrivingAtBoardStop`·`arrivingAtAlightStop`)로만 내고, 승차 승격은 "잔여 0 뒤 소실"(`boarded(departed)`), 하차는 소실 → 도착 추정뿐(확정은 선언). 잔여 0을 정차·확정 도착으로 다시 읽지 말 것. → INTEGRATIONS
...
- **안내 시트를 최소화하면 콘텐츠 뷰가 파괴되어 `@State`가 사라진다**(루트 `.sheet(item:)` 하나 + `presentedScreen = isMinimized ? nil : screen`; 종료 화면의 스와이프·VO escape가 곧 최소화라 일상 경로다). 화면만 어긋나는 표식은 종전대로 `@State`지만 **영속 값을 바꾸는 판정에 쓰이는 표식은 `@AppStorage`**이고, 소비는 도착 전이 `onChange`가 아니라 사용자가 누르는 버튼 핸들러에서 한다. 뷰 계층은 테스트 레인이 없어 배선을 소스 가드로 잠근다. → PATTERNS
- **iOS 목록 포커스 이동은 "가시화 → 지연 → 경합 해제 → 대입 → 검증 → 1회 재시도"가 정본**(`SearchView.landFirstRowFocus`; 동기 대입 한 줄은 실패). Bool 바인딩 다중 부착 금지·`scrollTo` 인자는 포커스 키와 다름·시뮬레이터 검출 불가. "내 주변"은 `nearbyFocusOnLoad`/`NearbyFocusLander` 공유. **대중교통 안내 시트만 3단 위**(A35, `TransitTrackingSheet.landControlFocus`): 실현 관측(onAppear 참조 카운트) → 실현될 때까지 반복 가시화(전 대상) → 100ms 단위 늦은 검증 3회 체증 → 실패 시 전 대상 폴백 통지 `.high`, 착지 트리거는 대상 뷰 `.task`가 아니라 상태 변화, 배경·모달(상세·검색 시트) 위에선 시도하지 않고 복귀·닫힘에 착지하며 최소화는 Task를 끊는다. `controlExists`는 `phaseControls` 바깥 분기(untrackable·지방버스)까지 본다. 착지 대상 부착은 `landingTarget` 한 자리(소스 가드 `transit-landing-guard.test.ts`). 다른 화면으로의 승격은 실승차 성공률이 정한다. → PATTERNS
- **그 시트의 착지 대상은 기본이 상태 문장 행이다**(E38 위원장 판정 2026-09-12 — 무엇을 누르든, 하차 도착도 [다음 구간]이 아니라 문장에 앉는다). 예외는 **자기 질문을 여는 화면**(역 선택·급행 확인 헤딩, 차량 선택 라벨 — →waiting 전이도 그 목록으로 가므로 라벨에 앉는다)과 띠바 복귀·목적지 전환 상태 행뿐. ⚠ 다음 행동 버튼은 "한 번 스와이프 아래"가 아니다(사이에 경유역 목록이 있다). ⚠ 대상이 하나로 몰리면 착지 테스트의 검출력이 0이 된다 — **누르기 전에 그 컨트롤로 커서를 옮겨야** "착지했다"와 "거기 있었다"가 갈리고, 중복 경로가 있는 자리는 결과 단언이 아니라 소스 가드로 잠근다. → PATTERNS
...
- **오디오 재생기는 둘(도보·대중교통)이고 미뤄진 원복은 최신 소유자에게 이전된다**(`.ownershipTransferred` — 인계에서 먼저 끝난 쪽의 `.ambient`가 새 세션 위에 떨어지지 않게). 활성화는 카테고리와 다른 축(`isActive`, 인터럽션 `.began`·`play()` 재확보), route 변경 `.categoryChange`는 **현재 세션 값 == 적용값** 대조로 자기 메아리만 거른다(`guideAudioRouteChangeEvent`) — 전면 필터 금지(채팅 TTS 탈취 회복 신호). → INTEGRATIONS
...
- **안내 시트 상태 문장은 한 조립기(`arrivalStatusLine`)가 만들고 통지는 그 문장에 없는 것만 말한다**(E39·E41): 잔여+도착은 한 줄 두 조각(쉼표 결합, 결합용 키엔 마침표 없음), 언제나 참인 조각 생략(대기 `stateNotYetVisible`·추적 중), 도착 국면은 접근 정보 전부 생략, 서울버스는 `parseBusArrmsg` 구조로 조립(미지는 원문 병치), 수단 낱말은 `*Bus` 키로(라벨 합성 금지). ⚠ 통지를 줄이기 전에 **그 전이의 착지 대상**을 확인한다(상태 문장이 아닌 전이가 둘 있다). 시작 통지는 목적지를 말한다(E40, 3수단). → PATTERNS
...
- **boarding 국면의 선언 버튼은 관측이 끝났을 때만 선다**(N3 ① — 승차 정류소 도착 관측이 riding 승격을 자동으로 한다. 조건은 순수 술어 `transitBoardingObservationLost` ↔ 웹 `boardingObservationLost`(`signalLost`·`upstreamFailed`)이고 앱 층에서 래치한다 — 회복에 버튼이 사라지면 포커스를 쥔 컨트롤이 폴 한 번에 제거된다). 그 버튼은 조용히 서므로 그 순간의 통지가 이름을 부른다. ⚠ **관측 승격 직후 즉폴을 넣지 말 것** — 그 왕복이 `boarded` 통지의 지연 슬롯을 latest-wins로 덮는다. 그 승격 전이는 **착지 대상도 아니다**(커서가 얹힌 상태 문장이 사라지지 않는다). → PATTERNS
