...
export function StationTimetable({ stationName }: { stationName: string }) {
  ...
  const load = useCallback(
    async (force: boolean, _source: "user" | "tool") => {
      ...
      setStatus((prev) => ({
        kind: "loading",
        gen,
        previous: force && prev.kind === "done" ? prev.timetable : undefined,
      }));
      try {
        ...
        setStatus((prev) =>
          prev.kind === "loading" && prev.gen === gen && prev.previous
            ? { kind: "done", gen, timetable: prev.previous, refreshError: true }
            : { kind: "error", gen },
        );
      }
    },
  ...
  useAxisSource("timetable", status, toSnapshot, loadForTool);
