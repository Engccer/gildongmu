 ...
 * 얼마나"를 담은 전문을 그 자리에 옮기면 들으면서 이미 모퉁이를 지난다. 두 층은
 * 역할이 다르다(정보 vs 타이밍). 옮긴 것은 **`ahead` 톤 하나**다 — 종전에는 40m
 * 전문에 붙어 있어서 소리가 "곧 뭔가 있다"만 말하고 "지금이다"는 못 말했다.
 */
export const IMMINENT_AHEAD_M = 10;
