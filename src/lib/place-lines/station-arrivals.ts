 ...
 * (`arrivalCode`)는 판정에도 값에도 쓰지 않는다 — 비시간형 행에도 `barvlDt`가 비0으로 오고(코퍼스 71행,
 * 00:30 `전역 출발`에 1200초), `[N]번째 전역`이 코드 1로도 온다(8호선 심야 24행).
 * ⚠ 불변식 I2: 역명을 싣는 것은 그 역이 열차 위치임이 실측으로 확인된 문법뿐이다(`전역 …` = 1정거장 전,
 * `[N]번째 전역 (X)` = N정거장 전, `N분 후 (X)`). `전전역 출발`의 `arvlMsg3`는 두 역 앞이 아니라 한 역
 * 앞을 가리키고(2/2) `{X} 전역출발`의 X는 조회 역 자신이라(12/12) 열차 위치가 아니다 — 이 둘은 역명을
 * 문장에도 꼬리에도 싣지 않는다. 못 알아보면 `null`(원문 경로) — 역을 지어내지 않는다(3-state).
 */
export function subwayArrivalProse(
  ...
  // ⚠ 이 두 문법은 **위치를 말하지 않는다**(I2). `전전역 출발`의 `arvlMsg3`는 두 역 앞이 아니라
  // 한 역 앞을 가리키고(코퍼스 2/2), `{X} 전역출발`의 X는 조회 역 자신이라(12/12) 열차 위치가 아니다.
  // 어긋나는 두 값을 한 줄에 담으면 거짓이 되므로 출발 사실만 싣는다(위원장 확정 2026-09-13).
