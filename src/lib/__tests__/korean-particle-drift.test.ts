 ...
 *
 * 조사가 갈리면 같은 목적지가 웹에서는 "성내로가", iOS에서는 "성내로이"로 낭독된다.
...
/** Swift 테스트의 `particleCases` 표에서 `(단어, 주격, 목적격)`을 뽑는다. */
function swiftCases(source: string): [string, string, string][] {
  const block = /let particleCases: \[\(String, String, String\)\] = \[([\s\S]*?)\n\]/.exec(
    source,
  );
  if (!block) {
...
const webCases: [string, string, string][] = PARTICLE_CASES.map((c) => [
  c.word,
  c.subject ?? "-",
  c.object ?? "-",
]);
