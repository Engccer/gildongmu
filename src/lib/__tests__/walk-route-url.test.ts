...
describe("walkRouteUrl", () => {
  it("lang=ko는 파라미터를 붙이지 않고 en만 붙인다(기존 캐시 키 유지)", () => {
    expect(
      walkRouteUrl({ origin: ORIGIN, dest: DEST, accessible: false, includeGeometry: false, via: null, lang: "en" }),
    ).toContain("&lang=en");
    expect(
      walkRouteUrl({ origin: ORIGIN, dest: DEST, accessible: false, includeGeometry: false, via: null, lang: "ko" }),
    ).not.toContain("lang=");
  });
  ...
  it("계단 회피만", () => {
    expect(
      walkRouteUrl({ origin: ORIGIN, dest: DEST, accessible: true, includeGeometry: false, via: null, lang: "ko" }),
    ).toBe("/api/route/walk?origin=37.5,127.1&dest=37.6,127.2&accessible=true");
  });

  it("기하만", () => {
    expect(
      walkRouteUrl({ origin: ORIGIN, dest: DEST, accessible: false, includeGeometry: true, via: null, lang: "ko" }),
    ).toBe("/api/route/walk?origin=37.5,127.1&dest=37.6,127.2&includeGeometry=1");
  });

  it("둘 다 — A4 수정 이전에는 존재하지 않던 조합이다", () => {
    expect(
      walkRouteUrl({ origin: ORIGIN, dest: DEST, accessible: true, includeGeometry: true, via: null, lang: "ko" }),
  ...
  it("경유지(N4): via는 '&via=lat,lng', null이면 부재(현행 URL 불변)", () => {
    expect(
      walkRouteUrl({ origin: ORIGIN, dest: DEST, accessible: false, includeGeometry: false, via: { lat: 37.55, lng: 127.15 }, lang: "ko" }),
