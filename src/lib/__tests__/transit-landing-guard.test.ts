...
describe("TransitTrackingSheet 착지 계약 (A35)", () => {
  it("focusedControl 바인딩은 landingTarget 헬퍼 안 한 자리에만 붙는다 — 가시화 키·실현 관측이 함께 달리는 유일한 경로", () => {
    ...
    const helper = SHEET.slice(SHEET.indexOf("private func landingTarget"), SHEET.indexOf("private static func controlId"));
  ...
  it("착지 트리거는 상태 변화다 — 대상 뷰의 .task/.onAppear가 landControlFocus를 부르지 않는다(트리거 결손 차단)", () => {
    ...
    for (let i = 0; i < lines.length; i += 1) {
      ...
      if (/landControlFocus\(\.(reboardPrompt|advance|changeBoarding|confirmBoarded|waitingLabel|boardAlready|expressPrompt|expressBlocked|destChangeStatus)/.test(window)) {
  ...
  it("계측 한 줄에 판정 축이 전부 있다 — attempts·elapsedMs·rendered·reason·vo, 배경 시도는 deferred로 기록", () => {
    for (const key of ["attempts=", "elapsedMs=", "rendered=", "reason=", "vo=", "deferred=true"]) {
      expect(SHEET, key).toContain(key);
    }
    // 배경에선 시도하지 않는다(L4) — 전경 게이트가 헬퍼 진입에 있다.
    expect(SHEET).toContain("guard model.isForeground else {");
...
describe("TransitTrackingSheet 지하철역 → 장소 상세 (E33)", () => {
  it("상태 문장은 언제나 같은 Text이고 역 언급은 로터 액션으로만 — 뷰 종류가 폴마다 갈리지 않는다(설계 리뷰 E2)", () => {
    ...
    expect(body).not.toMatch(/switch mentions/);
    expect(body).toContain(".accessibilityActions {");
    // 액션 라벨은 descriptor를 지난다(E1 — 한 라벨 안 언어 혼용 금지).
    expect(body).toContain("transitOpenStationLine(");
    expect(body).not.toContain("guard:allow");
  });

  it("장소 상세는 중첩 시트 하나(`.sheet(item: $detailPlace)`)를 지난다 — 목적지·경유역 공용", () => {
    expect(SHEET.match(/\.sheet\(item: \$detailPlace\)/g) ?? []).toHaveLength(1);
