...
export interface TransitRouteResult {
  ...
  /**
   * 사용자가 눌러 한 번 더 조회할 수 있는 수단 축(E50 판정 2). **강등 뒤 전체 후보**(운행 종료 포함)
   * 어디에도 그 수단만 타는 경로가 없을 때만 싣고, 없으면 필드 부재. ⚠ 표시 경로로 판정하지 말 것 — 운행
   * 종료라 빠진 경로를 과금 호출로 다시 받는다. 수단 재조회(`pathType`) 응답에는 싣지 않는다.
   */
