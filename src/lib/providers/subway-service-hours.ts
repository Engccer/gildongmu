...
/**
 * 한 요청이 조회할 구간 수 상한(방어). 넘긴 구간은 unknown으로 떨어진다
 * (조용한 절단이 아니라 정직한 미판정).
 *
 * ⚠ 근거가 2026-08-07에 바뀌었다. 종전 근거는 "상류가 경로를 3개로 자른다"였으나
 *   길찾기 재편이 강등을 **전체 후보**에 먼저 적용하도록 바꿔 그 전제가 사라졌다.
 *   새 근거는 실측이다: 9경로 응답에서도 지하철 유니크 키는 5개(수도권 길동→서울역)
 *   와 4개(부산 서면→해운대)뿐이다. 경로들이 같은 역·노선·방향을 공유하기 때문에
 *   경로 수가 늘어도 키는 그만큼 늘지 않는다. 상한 12는 2배 이상 여유가 있다.
 *   ⚠ 이 상한은 **지하철 전용**이다. spec §3.3의 "9경로 12개"는 버스까지 합친 수라
 *   이 상수와 직접 비교하면 안 된다(같은 12라 혼동하기 쉽다).
 */
const MAX_REFS = 12;
