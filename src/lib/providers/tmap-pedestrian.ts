 ...
 * (이미 완성된 한국어 안내문, 예 "158m 이동 후 우회전") — LineString은 지도
 ...
 *
 * 캐시: revalidate 3600(보행 경로는 준정적 — 같은 좌표쌍 캐시로 일 1,000건
 * 무료 쿼터를 보호한다).
 */
...
/**
 * 도보 경로 텍스트 브리핑 조회.
 * HTTP 비200이면 throw(라우트가 502로 전파) — "경로 없음"류 응답 코드는
...
export async function getWalkRouteBriefing(params: {
  ...
  const res = await fetch(ENDPOINT, {
    ...
    // 보행 경로는 준정적 — 같은 좌표쌍 캐시로 일 1,000건 무료 쿼터를 보호
