...
describe("annotateServiceStatus — 버스", () => {
  it("심야에 주간 노선을 outside로, 심야 노선을 running으로 판정한다", () => {
    ...
    expect(out[0].legs[0].serviceStatus).toBe("running");
    expect(out[1].legs[0].serviceStatus).toBe("outside");
  });

  it("첫차·막차 시각을 HH:MM 문자열로 싣는다", () => {
    const input = [route([leg("342", "A")])] as unknown as TransitRoute[];
    const out = annotateServiceStatus(input, HOURS, NO_SUBWAY, NIGHT);
    expect(out[0].legs[0].firstServiceTime).toBe("04:00");
    expect(out[0].legs[0].lastServiceTime).toBe("22:30");
  });

  it("조회 실패(Map에 없음)는 unknown이고 순위를 바꾸지 않는다", () => {
    const input = [
      route([leg("999", "MISSING")]),
      route([leg("342", "A")]),
    ] as unknown as TransitRoute[];
    const out = annotateServiceStatus(input, HOURS, NO_SUBWAY, NIGHT);
  ...
  it("환승 경로는 가장 나쁜 구간 상태를 경로 상태로 쓴다", () => {
    const input = [
      route([leg("N30", "B"), leg("342", "A")]), // running + outside
  ...
  it("같은 상태 안에서는 ODsay 추천순이 보존된다(안정 정렬)", () => {
    ...
    const out = annotateServiceStatus(input, HOURS, NO_SUBWAY, NIGHT);
    expect(out.map((a) => a.legs[0].lineName)).toEqual(["342", "370", "30-3"]);
  });
});
...
describe("annotateServiceStatus — 지하철", () => {
  it("막차 지난 지하철 전용 경로를 운행 중 버스 뒤로 강등한다", () => {
    ...
    // 01시에도 최상단에 남았다.
    const input = [
      route([subwayLeg("수도권 5호선", "길동", 1)]),
      route([leg("N30", "B")]), // running
    ] as unknown as TransitRoute[];
    const out = annotateServiceStatus(input, HOURS, SUBWAY_HOURS, NIGHT);
    expect(out[0].legs[0].lineName).toBe("N30");
    expect(out[1].legs[0].serviceStatus).toBe("outside");
    expect(out[1].legs[0].firstServiceTime).toBe("05:30");
    expect(out[1].legs[0].lastServiceTime).toBe("00:40");
  });
  ...
  it("운행 중인 지하철은 강등되지 않는다", () => {
    ...
    expect(out[0].legs[0].serviceStatus).toBe("running");
    expect(out[0].legs[0].lineName).toBe("수도권 5호선");
  });

  it("방향이 다르면 다른 시간표를 본다 — 조인 실패는 unknown", () => {
    // 하행(2) 시간표는 Map에 없다. 상행 것을 대신 쓰면 안 된다.
    const input = [route([subwayLeg("수도권 5호선", "길동", 2)])] as unknown as TransitRoute[];
    const out = annotateServiceStatus(input, HOURS, SUBWAY_HOURS, NIGHT);
    expect(out[0].legs[0].serviceStatus).toBe("unknown");
  ...
  it("버스·지하철 혼합 경로는 가장 나쁜 구간을 따른다", () => {
    const input = [
      route([leg("N30", "B"), subwayLeg("수도권 5호선", "길동", 1)]), // running + outside
      route([leg("N30", "B")]), // running
    ] as unknown as TransitRoute[];
    const out = annotateServiceStatus(input, HOURS, SUBWAY_HOURS, NIGHT);
    expect(out[0].legs).toHaveLength(1); // 순수 running 경로가 올라온다
    expect(out[0].legs[0].lineName).toBe("N30");
  });
  ...
  it("도보 전용 경로는 판정 대상이 없어 원순서를 보존한다", () => {
    ...
    const out = annotateServiceStatus(input, HOURS, SUBWAY_HOURS, NIGHT);
    expect(out[0].legs[0].mode).toBe("walk");
    expect(out[0].legs[0].serviceStatus).toBeUndefined();
  });
});
