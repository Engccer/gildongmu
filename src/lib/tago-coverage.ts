 ...
 *
 * ⚠ 2자리 코드가 **인접 도의 시를 물고 있어도**(코드 25 대전이 `계룡시`를 함께 든다)
 * 여기서 살릴 이유가 없다 — seed에 `34070 계룡시`(정류소 346)가 따로 있어 `34:계룡시`
 * 키는 그쪽에서 이미 생긴다. 예외 상수를 두면 **아무것도 바꾸지 않으면서 검증됐다는
 * 인상만** 남는다(리뷰가 실측으로 잡았다).
 */
