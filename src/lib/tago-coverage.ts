...
/**
 * TAGO 도시코드가 2자리인 지역(광역시 21~26·세종 12·제주 39)에서 **인접 도의 시**를
 * 물고 있는 이름. 그 시의 정류소는 실제로 제공되므로 미커버로 떨어뜨리면 안 된다.
 *
 * 지금은 계룡시(충남 34) 하나다 — TAGO가 계룡시 정류소를 대전(25) 코드 아래 둔다.
 */
const CARRIED_BY_METRO_CODE = ["34:계룡시"];

/**
 * `${도 접두}:${시군명}` 집합. 등록 정류소가 0인 도시(양양군)는 미커버로 친다.
 *
 * ⚠ **2자리 코드는 적재하지 않는다.** `Math.floor(code / 1000)`이 0이 되어
 * `"0:세종특별시"` 같은 **어떤 입력으로도 조회되지 않는 키** 9개를 만든다. 지금 무해한
 * 이유는 그 지역을 `WHOLE_PROVINCE_COVERED`·제주 분기가 **앞에서** 잡기 때문인데,
 * 그건 우연이라 분기 순서를 바꾸는 수정이 조용히 깨울 수 있다(백로그 D5).
 */
const COVERED = new Set([
  ...seed.cities
    .filter((c) => c.stops > 0 && c.code >= 1000)
    .flatMap((c) => c.names.map((n) => `${Math.floor(c.code / 1000)}:${n}`)),
  ...CARRIED_BY_METRO_CODE,
]);
