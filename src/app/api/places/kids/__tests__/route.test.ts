...
const mockFind = vi.mocked(findKidsPlacesNear);

/** 라우트는 항목 내부를 해석하지 않으므로 id만 있는 최소 fixture로 충분. */
const FIFTY = Array.from({ length: 50 }, (_, i) => ({ id: `k-${i}` }));

function makeRequest(query: string) {
  return new NextRequest(`http://x/api/places/kids${query}`);
}

describe("GET /api/places/kids (옵트인 limit 계약)", () => {
  beforeEach(() => {
  ...
  it("limit 미지정 → 기본 상한 8 + 절단 전 total", async () => {
    ...
    expect(body.kids.length).toBe(8);
    expect(body.kids[0].id).toBe("k-0");
  ...
  it("limit=50 → 50건 확장(옵트인)", async () => {
    ...
    const body = await res.json();
    expect(body.kids.length).toBe(50);
    expect(body.total).toBe(50);
  });
  ...
  it("키 없음 → { kids: [], total: 0 } (게이트 이중 방어)", async () => {
    mockHasKey.mockReturnValue(false);
    const res = await GET(makeRequest("?lat=37.5&lng=127.1&limit=50"));
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ kids: [], total: 0 });
    expect(mockFind).not.toHaveBeenCalled();
  });
