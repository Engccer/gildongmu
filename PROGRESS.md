...
| 부근 상황 재구성(M1, 웹) | 입구 기준 좌우·맞은편·건물 너머, 축 실패 시 방위 폴백 (⏳ 실보행·실기기 VO 판정, iOS 미이식) |
...
| 이탈 판정 방위 축(A6) | **iOS 실험판만**(보행 전용) · ⚠ 실사용 로그 판정(2026-08-09): 보행 속도에서 기기 방위 정확도 중위 83°라 축이 표를 못 넣는다 — 상수 튜닝이 아니라 설계 재론 필요(BACKLOG A6) |

### 플랫폼
| 기능 | 비고 |
|---|---|
| 웹 PWA | 수제 서비스워커, document network-first |
| iOS 앱 | SwiftUI + GildongmuKit, 4탭 |
