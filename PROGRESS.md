...
- **WebMCP 도구층**: W2 구현 완료(2026-08-29, main — 루트 상시 등록 7개·도구 자가 이동·단일 잠금·뷰 레지스트리·불투명 `ref`, spec `2026-08-29-webmcp-wave2-design.md`). 남은 것은 실기기 게이트 ⑧~⑪(`docs/FIELD-TEST.md` §8, 배포 차단 판정)·privacy `agent` 문안 확정·G4 제출물(위원장). 런타임 부재 브라우저에서는 등록이 침묵이라 일반 사용자에게 보이는 변화는 없다.
