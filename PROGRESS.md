...
| 따릉이 | 서울 |
| 도보 경로 | 카카오 기본 + Tmap 폴백, 계단 회피 모드, 서버 안내문 재작성, 경유지 1개(`via`, 웹·CLI·iOS — iOS 실시간 안내는 경유지 도착을 알리고 계속, ⏳ 실보행 판정) |
...
축 요약(2026-08-23 기준): **가장 큰 병목은 판정**이다 — 코드가 끝나고 위원장 실사용·실기기를 기다리는 항목이 §2 표(도보·자동차·대중교통)에 쌓여 있고, 그중 자동차·대중교통 판정이 G3 봉인 해제의 **유일한** 선행 조건이다. 대중교통에서 사람 없이 할 수 있는 것은 남지 않았다(실호출 게이트 5축 2026-08-16 종결). 결함은 A16(**L2·L3 종결, L1만 잔여** — 급행 정차역 데이터 선행)·A1(동결)·A18(코드 1·3선 종결 2026-08-17, 상수·2선은 실보행 판정 잔여) 셋이고(A7·A11·A13·A17은 종결), 갭은 B6·B7·B8이고(B9 웹 둘러보기는 2026-08-22 종결), 2026-08-22 묶음(M4·N1~N4)과 2026-08-23 묶음(K1·K2·K3·E15-1)은 코드 종결·판정 대기로 §2에 들어가 있다. ⚠ **건수를 여기 적지 않는다** — 표가 정본이고 수치는 사흘에 세 번 낡는다.
