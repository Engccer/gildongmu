...
| iOS | **1.13 `WAITING_FOR_REVIEW`**(빌드 21, 아카이브 커밋 `df229f0`, 제출 2026-08-27 05:41 KST) · 스토어 최신은 1.12 `READY_FOR_SALE`(빌드 20, 아카이브 `37b99db`) |
...
- 도보 안내의 도착 추정 자동 종료(잊힌 세션 정리)는 구현·출시 완료(1.10), **실보행 판정 대기**(`docs/BACKLOG.md` E13).
...
| 자동차 경로 | ko Tmap 기본 + 카카오모빌리티 폴백, en NCP, 경유지 1개(`via`, 웹·CLI·iOS) |
| 대중교통 경로 | ODsay, 운행 시간 밖(`outside`)만 강등(A21), 최단·최소환승 축, 경유지는 `unsupported` 정직 표시 |
...
| 음향신호기 BLE 진단(E20 계측기) | **iOS Experimental·Debug 전용**(정식판 없음, 웹 미러 없음). 「내 주변」 허브 목록 맨 아래(하위 화면 없음, 2026-08-20), 로그 `Documents/audio-signal-diag.log` (E20 2차 실측 0건·배제, 진단 UI는 유지) |
...
| 자동차·대중교통 안내 | 웹 prod · **iOS 봉인 유지**(`experimentalGuidanceEnabled`, ⏳ 실주행·실승차 판정). 자동차는 2026-08-23 K2로 도보 수준 완성 — 임박 시간 축(5초+fix 지연 1초 = 유효 6초/운전자 8초+1초 = 9초)·turnType 행동 분류·터널 따라잡기 안전·동승자/운전자 청취자 모드(운전자는 스피커 TTS)·정차 도착+도보 인계·경유지 삭제. ⏳ B1 실주행 판정(실험판) |
...
- **WebMCP 도구층**: spec 확정(`4240abb`, 도구 10개·정적 등록·`planId` 세대 토큰). 구현 대기(Codex `feat/webmcp`). 게이트 0·1 통과, 실기기 게이트 4항은 `docs/FIELD-TEST.md` §8.


**정본은 `docs/BACKLOG.md`다.** 여기에 목록을 복제하지 않는다 — 두 곳에 적으면 어느 쪽이 참인지 판정할 수 없게 된다.

`W1 WebMCP`는 임시 프로브 `4a3ee44`에서 게이트 0을 통과했다(2026-08-27). 인앱 브라우저 localhost 페이지 200·도구 2개 발견·DOM `<h4>` 포커스와 단일 polite live region 일치에 더해, 김헌용 위원장이 VoiceOver로 웹 콘텐츠 탐색·헤딩·폼 컨트롤 이동과 포커스 대상 5개의 커서 이동·낭독을 실사용 확인했다. 다음은 Tmap 별도 키 발급이고, 이후 spec과 설계 리뷰다.
...
축 요약(2026-08-23 기준): **가장 큰 병목은 판정**이다 — 코드가 끝나고 위원장 실사용·실기기를 기다리는 항목이 §2 표(도보·자동차·대중교통)에 쌓여 있고, 그중 자동차·대중교통 판정이 G3 봉인 해제의 **유일한** 선행 조건이다. 대중교통 게이트 6축은 **전부 통과**했다(마지막 `scripts/verify-odsay-express-lane.mjs`가 2026-08-24 00:01 KST 4/4 PASS — 도입 당일엔 ODsay 쿼터 소진으로 미실행이었다). 결함은 A16(**L2·L3 종결, L1은 지배적 경로 종결·좁은 경우 하나 잔여** — 2026-08-23 데이터원 판정 종결, 위치 추적안은 설계 리뷰로 기각)·A1(동결)·A18(코드 1·3선 종결 2026-08-17, 상수·2선은 실보행 판정 잔여) 셋이고(A7·A11·A13·A17은 종결), 갭은 B6·B7·B8이고(B9는 웹 둘러보기 2026-08-22 · 도보 대안 ① 2026-08-23 종결, ② 안내 전환만 잔여), 2026-08-22 묶음(M4·N1~N4)·2026-08-23 묶음(K1·K2·K3·E15-1)과 같은 날 백로그 전수 소화 웨이브(A19·B9 ①·E15-2·E8 / E16·E14·E19)는 코드 종결·판정 대기로 §2에 들어가 있다. ⚠ **건수를 여기 적지 않는다** — 표가 정본이고 수치는 사흘에 세 번 낡는다.
