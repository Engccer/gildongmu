...
| iOS | **1.11 `IN_REVIEW`**(빌드 18, 아카이브 커밋 `f312a39`, 제출 2026-08-22 07:11 KST) · **1.12 빌드 19 업로드·처리 완료**(아카이브 커밋 `c56752c`, 2026-08-23 12:37 KST) — ASC가 심사 중 버전이 있으면 새 버전 생성을 409로 거부하므로 **1.11 판정 뒤 `asc-submit --apply --submit`** 대기(위원장 판정: 철회 대신 승인 대기) · 스토어 최신은 1.10 `READY_FOR_SALE`(빌드 17) |
...
| 대중교통 경로 | ODsay, 운행 시간 밖 강등, 최단·최소환승 축, 경유지는 `unsupported` 정직 표시 |
...
| 기능 | 비고 |
|---|---|
| 채팅 | Gemini function calling 24도구(2026-08-23 K3: 첫차·막차·정위·한눈에 보기·무장애 상세 + `stationName`·`place`·`via` 인자) + Perplexity 웹 검색, 장소 앵커 |
...
| 기능 | 상태 |
|---|---|
| **도보 상세 안내** | 웹 prod · **iOS 정식판 출시**(1.7, 2026-08-15 플래그 졸업 + 백그라운드 모드 승격). ko 전용 |
...
| 직선거리 추적(구 "간략 안내") | **명칭은 UI에서 사라졌다**(2026-08-16, E16 축1 종결 — 전 로케일 스캔 가드가 재유입을 막는다). 지금은 상세가 불가능해진 지점에서만 켜지는 **이름 없는 내부 수단**이다. 웹은 아직 단독 진입점이 남아 있고, 비-ko에 상세를 주는 축이 열려야 그 진입점을 지울 수 있다(BACKLOG E16 축3 → 축2) |
...
대중교통 안내는 잠근 차량이 한 번도 관측되지 않으면 10분(잠정) 뒤 그 사실을 말하고, 탑승 변경이 지금 있는 역을 물어 재잠금 경로를 연다(A16 L2·L3, ⏳ 실승차 판정). "진행 상황"은 조망 모달을 연다(E15-1, 2026-08-23 — 구간·정차역·"현재 위치"·침묵 설명+탑승 변경·현재 위치 기준 다른 경로 전환, ⏳ 실승차 판정 `docs/FIELD-TEST.md` §5-4). 급행 경로도 실시간 안내가 열린다(2026-08-23 — 종전에는 ODsay `(급행)` 접미 때문에 급행 leg가 통째로 추적 불가였다). A16 L1의 데이터원 판정은 종결됐고(데이터는 있다) 남은 경우는 "완행 leg에 급행을 잠갔을 때" 하나다 — 그 판정은 데이터가 아니라 제품이며 `docs/BACKLOG.md` A16 L1이 정본이다. 시트에 "주변 확인"이 있고 기준은 확정 현재역 또는 하차역이다(E15-2, 2026-08-23, ⏳ 실승차 판정). "탑승" 버튼은 차량 선택이고 승차 정류소 도착을 관측해 앱이 `riding`으로 올린다(`boarding` 국면, N3 2026-08-22, ⏳ 실승차 판정 — A19 포커스 착지 포함).
...
축 요약(2026-08-23 기준): **가장 큰 병목은 판정**이다 — 코드가 끝나고 위원장 실사용·실기기를 기다리는 항목이 §2 표(도보·자동차·대중교통)에 쌓여 있고, 그중 자동차·대중교통 판정이 G3 봉인 해제의 **유일한** 선행 조건이다. 대중교통에서 사람 없이 할 수 있는 것은 남지 않았다(실호출 게이트 5축 2026-08-16 종결). 결함은 A16(**L2·L3 종결, L1은 지배적 경로 종결·좁은 경우 하나 잔여** — 2026-08-23 데이터원 판정 종결, 위치 추적안은 설계 리뷰로 기각)·A1(동결)·A18(코드 1·3선 종결 2026-08-17, 상수·2선은 실보행 판정 잔여) 셋이고(A7·A11·A13·A17은 종결), 갭은 B6·B7·B8이고(B9 웹 둘러보기는 2026-08-22 종결), 2026-08-22 묶음(M4·N1~N4)과 2026-08-23 묶음(K1·K2·K3·E15-1)은 코드 종결·판정 대기로 §2에 들어가 있다. ⚠ **건수를 여기 적지 않는다** — 표가 정본이고 수치는 사흘에 세 번 낡는다.
