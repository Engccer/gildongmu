...
| iOS | **1.10 `WAITING_FOR_REVIEW`**(빌드 17, 아카이브 커밋 `0013c52`, 제출 2026-08-19 07:31 KST) · 스토어 최신은 1.9 `READY_FOR_SALE`(빌드 16) · 1.7~1.8도 `READY_FOR_SALE` |
| iOS 최소 지원 | **18.0**(2026-08-19 하향, 심사 중인 1.10부터 적용 — 스토어 최신 1.9까지는 26.0) |
...
- 도보 안내의 도착 추정 자동 종료(잊힌 세션 정리)는 구현·배포 완료(도착 종 전경 한정 `63e0be7`만 미출시), **실보행 판정 대기**(`docs/BACKLOG.md` E13).
...
| 내 주변 둘러보기 | iOS: 위치 문장 + "한눈에 보기" 5불릿(`/api/nearby/overview`, 공통 반경 1km, 불릿별 3-state) + 주변 상황 자동 펼침(장소 상세 진입) + 카카오 10종 목록. 웹은 종전 구조(백로그 B9) |

### 접근성 정보
| 기능 | 비고 |
|---|---|
| 보행 인프라 | 음향신호기(서울 seed) + 횡단보도·점자블록(전국 OSM seed 79,575 노드, 연 1회 갱신) |
| 음향신호기 BLE 진단(E20 계측기) | **iOS Experimental·Debug 전용**(정식판 없음, 웹 미러 없음). 보행 인프라 화면 음향신호기 섹션 아래, 로그 `Documents/audio-signal-diag.log` (E20 2차 실측 0건·배제, 진단 UI는 유지) |
...
도보 종료 화면에 걸음·칼로리 요약이 들어갔다(iOS, 빌드 구성 무관 — 2026-08-17 — CMPedometer 실측 + 활동 칼로리 추정). **1.8로 출시됨(2026-08-18 `READY_FOR_SALE`), 1.9에서 문장형·음식 비유·체중 안내로 개정, 2026-08-19에 도착만이 아니라 중지·목적지 변경·권한 상실 종료까지 확장(1.10 심사 중, 50m 임계값).** ⏳ 실기기 판정(`docs/BACKLOG.md` §2 "건강 요약", 대본 `docs/FIELD-TEST.md` §3).

축 요약(2026-08-17 기준): **가장 큰 병목은 판정**이다 — 코드가 끝나고 위원장 실사용·실기기를 기다리는 항목이 §2 표(도보·자동차·대중교통)에 쌓여 있고, 그중 자동차·대중교통 판정이 G3 봉인 해제의 **유일한** 선행 조건이다. 대중교통에서 사람 없이 할 수 있는 것은 남지 않았다(실호출 게이트 5축 2026-08-16 종결). 결함은 A16(**L2·L3 종결, L1만 잔여** — 급행 정차역 데이터 선행)·A1(동결)·A18(코드 1·3선 종결 2026-08-17, 상수·2선은 실보행 판정 잔여) 셋이고(A7·A11·A13·A17은 종결), 갭은 B6·B7·B8이다. ⚠ **건수를 여기 적지 않는다** — 표가 정본이고 수치는 사흘에 세 번 낡는다.
