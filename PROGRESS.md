...
| 자동차·대중교통 안내 | 웹 prod · **iOS 봉인 유지**(`experimentalGuidanceEnabled`, ⏳ 실주행·실승차 판정). 자동차는 2026-08-23 K2로 도보 수준 완성 — 임박 시간 축(5초/운전자 8초)·turnType 행동 분류·터널 따라잡기 안전·동승자/운전자 청취자 모드(운전자는 스피커 TTS)·정차 도착+도보 인계·경유지 삭제. ⏳ B1 실주행 판정(실험판) |
...
대중교통 안내는 잠근 차량이 한 번도 관측되지 않으면 10분(잠정) 뒤 그 사실을 말하고, 탑승 변경이 지금 있는 역을 물어 재잠금 경로를 연다(A16 L2·L3, ⏳ 실승차 판정). 급행 정차역 데이터가 필요한 L1은 열려 있다. "탑승" 버튼은 차량 선택이고 승차 정류소 도착을 관측해 앱이 `riding`으로 올린다(`boarding` 국면, N3 2026-08-22, ⏳ 실승차 판정 — A19 포커스 착지 포함).
...
축 요약(2026-08-22 기준): **가장 큰 병목은 판정**이다 — 코드가 끝나고 위원장 실사용·실기기를 기다리는 항목이 §2 표(도보·자동차·대중교통)에 쌓여 있고, 그중 자동차·대중교통 판정이 G3 봉인 해제의 **유일한** 선행 조건이다. 대중교통에서 사람 없이 할 수 있는 것은 남지 않았다(실호출 게이트 5축 2026-08-16 종결). 결함은 A16(**L2·L3 종결, L1만 잔여** — 급행 정차역 데이터 선행)·A1(동결)·A18(코드 1·3선 종결 2026-08-17, 상수·2선은 실보행 판정 잔여) 셋이고(A7·A11·A13·A17은 종결), 갭은 B6·B7·B8이고(B9 웹 둘러보기는 2026-08-22 종결), 2026-08-22 묶음(M4·N1~N4)은 코드 종결·판정 대기로 §2에 들어가 있다. ⚠ **건수를 여기 적지 않는다** — 표가 정본이고 수치는 사흘에 세 번 낡는다.
