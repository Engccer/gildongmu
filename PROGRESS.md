...
- **WebMCP 도구층**: W2 구현 완료(2026-08-29, main — 루트 상시 등록 7개·도구 자가 이동·단일 잠금·뷰 레지스트리·불투명 `ref`, spec `2026-08-29-webmcp-wave2-design.md`). OpenAI WebMCP Challenge 제출 완료(2026-09-04, https://devpost.com/software/gildongmu). 남은 것은 실기기 게이트 ⑧~⑪(`docs/FIELD-TEST.md` §8, 배포 차단 판정). 런타임 부재 브라우저에서는 등록이 침묵이라 일반 사용자에게 보이는 변화는 없다.


**정본은 `docs/BACKLOG.md`다.** 여기에 목록을 복제하지 않는다 — 두 곳에 적으면 어느 쪽이 참인지 판정할 수 없게 된다.

대중교통 안내는 잠근 차량이 한 번도 관측되지 않으면 riding 조회 10회(잠정 — 2026-09-11 A36 ①부터 시계가 아니라 횟수) 뒤 그 사실을 말하고, 탑승 변경이 지금 있는 역을 물어 재잠금 경로를 연다(A16 L2·L3, ⏳ 실승차 판정). "진행 상황"은 조망 모달을 연다(E15-1, 2026-08-23 — 구간·정차역·"현재 위치"·침묵 설명+탑승 변경·현재 위치 기준 다른 경로 전환, ⏳ 실승차 판정 `docs/FIELD-TEST.md` §5-4). 급행 경로도 실시간 안내가 열린다(2026-08-23 — 종전에는 ODsay `(급행)` 접미 때문에 급행 leg가 통째로 추적 불가였다, ⏳ 실승차 판정 §2 A16 ⑥). A16 L1은 2026-09-02에 판정·구현 모두 종결됐다 — 급행 정차역 집합(`expressStops`, ODsay 런타임 조회 + 7일 캐시)으로 통과 급행의 활성화를 차단하고 결정적 문장을 내며, 근사 잠금은 급행 집합이 있는 노선에서 "급행인가요?"를 한 번 묻는다(⏳ 실승차 판정 §2 A16 ⑦⑧). 시트에 "주변 확인"이 있고 기준은 확정 현재역 또는 하차역이다(E15-2, 2026-08-23, ⏳ 실승차 판정). "탑승" 버튼은 차량 선택이고 승차 정류소 도착을 관측해 앱이 `riding`으로 올린다(`boarding` 국면, N3 2026-08-22, ⏳ 실승차 판정 — A19 포커스 착지 포함). 2026-09-11부터 역 선택의 하차역 행은 도착 선언이고(A37 ②), "이미 탑승했습니다"는 역부터 물어 그 역에 있는 열차를 고르게 하며 0건일 때만 비관측 잠금(폴·어림값 0, A34), 마지막 leg의 버튼은 "남은 도보 안내 시작" 하나다(E34) — ⏳ 실승차 판정 §2 A37·A34·E34 행. 같은 날 시트 컨트롤 착지를 실현 관측·반복 가시화·3회 체증 검증·전 대상 폴백 통지로 재설계했고(A35, 배경 무시도·전경 복귀 착지, 계측 `reason=`·`vo=`), 지하철 경유역 행·상태 문장 로터 액션이 중첩 시트 하나로 장소 상세를 연다(E33, 웹 미연결) — ⏳ 실기기 판정 §2 A35·E33 행.

도보 안내 경로 출발점은 정확한 fix(≤30m)를 최대 15초 기다린 뒤 정하고, 그 origin이 계측된다(A18 코드 종결 2026-08-17, ⏳ 실보행 판정 — 침묵 길이·간략 폴백 빈도).

도보 종료 화면에 걸음·칼로리 요약이 들어갔다(iOS, 빌드 구성 무관 — 2026-08-17 — CMPedometer 실측 + 활동 칼로리 추정). **1.8로 출시됨(2026-08-18 `READY_FOR_SALE`), 1.9에서 문장형·음식 비유·체중 안내로 개정, 2026-08-19에 도착만이 아니라 중지·목적지 변경·권한 상실 종료까지 확장(1.10 출시, 50m 임계값).** ⏳ 실기기 판정(`docs/BACKLOG.md` §2 "건강 요약", 대본 `docs/FIELD-TEST.md` §3). **2026-09-11(E31): 체중 입력 권유에 무시 상한 2회**(미출시, 실기기 판정 대기).
