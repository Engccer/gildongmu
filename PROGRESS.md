...
| iOS | **1.9 `READY_FOR_SALE`**(빌드 16, 아카이브 커밋 `01447d4`, 제출 2026-08-18 19:55 KST → 2026-08-19 승인) · 1.7~1.8도 `READY_FOR_SALE`. 1.9 아카이브 뒤 iOS 변경(최소 지원 18 하향·건강 요약 종료 확장·추정 도착 종 전경 한정)은 **미출시** |
| iOS 최소 지원 | **18.0**(2026-08-19 하향, 다음 제출부터 적용 — 스토어 최신 1.9까지는 26.0) |
...
도보 종료 화면에 걸음·칼로리 요약이 들어갔다(iOS, 빌드 구성 무관 — 2026-08-17 — CMPedometer 실측 + 활동 칼로리 추정). **1.8로 출시됨(2026-08-18 `READY_FOR_SALE`), 1.9에서 문장형·음식 비유·체중 안내로 개정, 2026-08-19에 도착만이 아니라 중지·목적지 변경·권한 상실 종료까지 확장(미출시, 50m 임계값).** ⏳ 실기기 판정(`docs/BACKLOG.md` §2 "건강 요약", 대본 `docs/FIELD-TEST.md` §3).
