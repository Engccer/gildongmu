...
- **WebMCP 도구층**: 구현 완료(main 통합 `fd7b44c`·`907cf67`, 2026-08-27 — 도구 10개 정적 등록·`planId` 세대 토큰·allowlist 출력, spec `4240abb`). 남은 것은 실기기 게이트 6항(`docs/FIELD-TEST.md` §8)·G4 제출물(위원장). 런타임 부재 브라우저에서는 등록이 침묵이라 일반 사용자에게 보이는 변화는 없다.
