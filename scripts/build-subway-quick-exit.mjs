 ...
 *
 * ⚠ **범위까지 좁힌다**(칸 1~10 · 문 1~4). 자릿수만 보는 `\d+-\d+`는 형태는 잡아도
 * 범위를 놓치는데, 선형 위치 모델이 `(칸−1)×4+문`이라 **`1-5`와 `2-1`이 둘 다 5**가 된다.
 * 오류가 아니라 조용한 충돌이라 어디서도 드러나지 않는다. 칸당 문이 4개가 아닌 전동차가
 * 들어오면 그건 값 하나가 낯선 게 아니라 **모델의 전제가 바뀐 것**이므로 빌드를 세우는
 * 것이 정답이다(사람이 `DOORS_PER_CAR`부터 다시 정해야 한다). 독립 리뷰 검출 2026-08-08.
 */
const CAR_DOOR = "(?:[1-9]|10)-[1-4]";
const DOOR = new RegExp(`^${CAR_DOOR}$`);
const BETWEEN = new RegExp(`^${CAR_DOOR},${CAR_DOOR} 사이$`);
