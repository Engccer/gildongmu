...
/// 파싱할 수 있게 하기 위해서다.
let particleCases: [(String, String, String)] = [
    ("성내로", "가", "를"),
    ("천호대로", "가", "를"),
    ("이마트", "가", "를"),
    ("명일로24길", "이", "을"),
...
@Suite struct KoreanParticleTests {
    @Test func 표대로_조사를_고른다() {
        for (word, subject, object) in particleCases {
            #expect(KoreanParticle.subject(word) ?? "-" == subject, "\(word) 주격")
            #expect(KoreanParticle.object(word) ?? "-" == object, "\(word) 목적격")
        }
    }

    @Test func 주격_조사를_붙인_어절() {
        #expect(KoreanParticle.withSubject("성내로") == "성내로가")
        #expect(KoreanParticle.withSubject("명일로24길") == "명일로24길이")
    }

    /// ⚠ 판정 불가에 빈 문자열이나 옵셔널 문자열 보간을 돌려주면 그대로 낭독된다.
    @Test func 판정_불가면_nil이라_호출자가_대체_문형을_고른다() {
        #expect(KoreanParticle.withSubject("GS25") == nil)
