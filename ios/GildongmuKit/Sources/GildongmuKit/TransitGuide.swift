...
/// ([선택한 열차에 탔어요])을 세운다 — 그 밖에는 승차 정류소 도착 관측이 riding
...
/// 폴링 항목(`/api/transit/track` 판별 union의 투영, §7). `message`는 **upstream 완성 문장 원문**이다
/// — 서버는 어느 국면에서도 다듬지 않는다(E39, 2026-09-12에 서울버스 승차 국면 재작성을 폐지했다).
/// 문장 조립은 `transitArrivalStatusLine`이 `parseBusArrmsgKind`로 구조를 읽어 한 자리에서 한다.
...
public enum TransitGuideInput: Sendable {
    ...
    /// boarding → riding 사용자 선언. 입력 자체는 불변이고 UI가 이 입력을 낼 수 있는 때만
    /// 좁혔다(N3 ① 2026-09-11 — 관측이 끝난 국면의 [선택한 열차에 탔어요]).
    case confirmBoarded
    ...
    case declareArrived
    /// "이미 탔어요" 흐름의 식별 잠금(A34 ②, 2026-09-11): 사용자가 지나는 역의 목록에서 고른 열차로
...
/// 웹 `seoulBusArrivedAfterSoon` 미러.
private func transitSeoulBusArrivedAfterSoon(_ state: TransitGuideState) -> Bool {
    state.phase == .arrived && !state.arrivedCertain
...
public func transitPollIntervalMs(_ state: TransitGuideState) -> Int {
    ...
    // A41(설계 리뷰 M2): riding 첫 조회 한 번만 15초 — 서울버스 승격이 "곧 도착 뒤 소실"(출발 20~40초 뒤)로
    // 밀려 첫 하차 정류소 폴이 60초 뒤면 짧은 구간의 하차 신호를 통째로 놓친다. 즉폴이 아닌 이유는 승격
    // 문장의 지연 슬롯을 침범하지 않기 위해서(N3 ① H1). 웹 미러.
...
private func handlePoll(
    ...
    // — 내리는 중에 "차량 신호를 찾지 못하고 있습니다"가 거짓이 된다(종전 확정 도착이 침묵하던 자리). 웹 미러.
    if transitSeoulBusArrivedAfterSoon(state) { return (next, event) }
...
/// `transitBoardingObservationLost`) — 탈출은 [선택한 열차에 탔어요] 또는 [다른 차량 선택]이다.
