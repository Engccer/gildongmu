...
public final class DeferredAnnouncer {
    ...
    /// 실제 게시 시도(억제 가드 → 전경 가드 → 게시). 반환 = 실제로 게시했는가.
    /// 지연은 타이밍만 바꾸고 실패 처리 계약은 바꾸지 않는다(§4-4) — 대기가 끝난
    /// 게시 시도는 "그 시점에 announce를 부른 것"과 완전히 같은 경로를 지난다.
    ...
    public init(
        ...
        post: @escaping (String, Bool) -> Bool
    ) {
        self.clock = clock
    ...
    public func announce(
        ...
        guard wait > 0 else {
            if !post(text, highPriority) { onDropped?() }
            return
        }
        nextToken += 1
        let token = nextToken
