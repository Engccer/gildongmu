...
final class LocationService: NSObject, CLLocationManagerDelegate {
    ...
    /// ⚠ 여기만 `softTTL`을 쓴다. 캐시를 오래 재사용해도 되는 이유는 이 값이 순위
    /// 가중용이기 때문이고, 같은 이유로 타임아웃도 짧다(검색어를 넣고 기다리는
    /// 자리라 8초는 침묵이다).
    ///
    /// ⚠ **정밀도 요구가 낮다고 reduced 좌표까지 받지는 않는다.** 소비자 중
    /// `loadCurrentAddressIfAuthorized`가 이 좌표를 역지오코딩해 "현재 위치,
    /// 〈주소〉"로 **표시**한다. km급 좌표로 주소를 만들면 화면으로 반증할 수 없는
    /// 거짓 위치 주장이 된다. 정밀 위치가 꺼진 세션에서는 검색이 좌표 없이 나가고
    /// (전국 정확도순), 주소 병기는 생략된다. 둘 다 정직한 축소다.
    /// **순위 가중용.** 낡거나 거친 좌표도 좌표 없음보다 낫다 — 결과 목록을 훑으면
    /// 순위가 이상하다는 것을 알 수 있으므로 반증 가능하다.
    ///
    /// ⚠ 정확도 바도 함께 연다(`storeCeiling`). TTL만 늘리고 바를 30m로 두면
    /// 실내·도심 협곡처럼 정확도가 계속 30m를 넘는 환경에서 캐시가 **한 번도
    /// 재사용되지 않아** 매 검색이 재측위한다. softTTL을 둔 근거가 정확히 그 근거가
    /// 필요한 환경에서 무효가 된다.
