...
- **정보의 정본은 리스트·텍스트다.** 지도 SDK는 캔버스라 스크린 리더가 읽을 수 없으므로, 지도에만 존재하는 정보가 있으면 그것은 버그다. 자동차 실주행 내비게이션은 딥링크로 네이티브 앱(`nmap://`·`kakaomap://`)에 위임하고, 자체 구현은 "출발 전 미리 듣기" 텍스트 브리핑과 **도보 실시간 안내**에 집중한다.
