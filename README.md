...
- **정보의 정본은 리스트·텍스트다.** 지도 SDK는 캔버스라 스크린 리더가 읽을 수 없으므로, 지도에만 존재하는 정보가 있으면 그것은 버그다. "출발 전 미리 듣기" 텍스트 브리핑과 **실시간 안내**(도보는 웹·iOS 정식판, 자동차·대중교통은 웹 정식판·iOS 실험판)를 자체 구현하고, 지도 앱 딥링크(`nmap://`·`kakaomap://`)는 장소 상세의 보조 출구로만 둔다.
