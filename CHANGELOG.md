...
**역명을 싣지 않기로 한 문법이 둘 있다**(불변식 I2). `전전역 출발`의 `arvlMsg3`는 두 역 앞이 아니라 한 역 앞을 가리키고(2/2), `{X} 전역출발`의 X는 조회 역 자신이다(12/12) — 실으면 열차가 없는 역을 "현재 위치"로 낭독한다. 문안 1판에 그 꼬리가 들어 있었고 위원장 확인으로 뺐다. 한 역 앞을 `전역`이 아니라 **`1정거장 전`**으로 부르는 것도 왕복에서 확정했다(버스 안내 상태 문장과 같은 말).
