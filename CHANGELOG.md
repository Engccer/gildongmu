...
- **비-ko 로케일 한국어 잔존 결함 묶음(A26, 병렬 세션 en-fix)**: ①횡단 유닛 판정을 ko 문장 "건너" 부분 문자열에서 서버 구조화 플래그 `WalkRouteStep.crossing`(카카오=재작성 횡단 행동문, Tmap=turnType 횡단 행동, `includeGeometry` 전용)으로 바꿔 en 도보 안내에서도 횡단 유닛이 선다(웹 `isCrossingStep` ↔ Kit 미러, 공유 fixture로 ko 동작 불변) ②iOS `RouteService.car`의 `lang`을 기본값 없는 필수 인자로, `BeaconModel` 자동차 안내·ETA 재조회에 `AppLanguage.dataLocale` 배선 ③`/api/route/car` 응답에 `guidanceLang`("ko"/"en") 추가 — en 요청의 ko 폴백(NCP 키 부재·경유지)을 정직화, 웹 `StepList`가 ko면 `<li lang="ko">`; 502 경로 탐색 실패엔 `code:"noRoute"` ④`CarRouteBriefing`·`TransitRouteBriefing` 오류 낭독을 서버 한국어 문자열 대신 status·code로 고른 `t()` 문장으로(6로케일 키 `route.briefing.{noRoute,rateLimited,notConfigured}`·`route.transit.notConfigured`) ⑤서버 합성 한국어에 구조화 원재료 병행(additive): 무장애 라벨 27종 `key`→`barrierFreeInfo.facility.*`(웹 `barrierFreeLines(d, t)`·iOS switch, 동조 가드 `barrier-free-labels-i18n.test.ts`), `SeoulMetroFacility.parts`(엘리베이터 위치 compass·meters·dong / 화장실 restroomType·wheelchairAccessible / 음성유도기 location·line), `TimetableLine.lineCore`("선" 접미), iOS "N정류장 전"→`ios.nearby.stopsBefore` ⑥`lang="ko"` 마크업: `PlaceCard` 이름·분류, `PlaceDetail` 분류, 대중교통 재승차역 버튼(판정 `hasHangul`). 건너뛴 자리(prose 템플릿 안 한국어 삽입·노선명)는 `docs/BACKLOG.md` A26 종결 기록 → E27·E28 재료. plan `docs/superpowers/plans/2026-08-31-en-locale-korean-cleanup-parallel-plan.md`.
