...
위원장 실사용 요청: 채팅 탭 답변 아래 장소 카드와 산문 안 장소명이 장소 상세로 이어지지 않았다(웹 카드는 이미 `requestOpenPlace`로 진입). 두 층으로 잇는다 — ① 장소 카드 활성화 = 장소 상세 **시트**(닫기 버튼, 닫으면 연 카드로 VO 포커스 복원. 기존 로터 액션 유지) ② 산문 블록은 그대로 한 객체로 두고, 그 블록에 카드 장소명이 등장하면 장소마다 로터 커스텀 액션 "○○ 상세 보기"를 단다(Kit `chatPlaceMentions` 결정론 매칭 — 인라인 링크로 단락을 쪼개는 안은 기각, 위원장 판정). 웹 산문 연결과 장소 채팅 시트(앵커 모드)의 산문 연결은 보류(`docs/BACKLOG.md`).
