...
대안은 축 이름이 붙는 경로만 싣는다(번호 채움 폐기). 축은 5개(가장 빠른·환승이 가장 적은·도보 거리가 가장 짧은·버스만 타는·지하철만 타는)이고 여럿이면 쉼표로 이은 한 이름이다. 전체 후보에 버스만·지하철만 경로가 없을 때만 길찾기 화면(웹·iOS)에 "…타는 경로 찾기" 버튼이 서고, 누르면 ODsay를 한 번 더 부른다(`/api/route/transit?pathType=`). 서버 응답은 additive이고 옛 앱은 `displayIndex`로 "대안 경로 N"을 유지한다. 이름 조립은 웹·Kit·`:kit`·CLI가 공유 fixture로 잠갔다. 실호출 게이트 13건(선정 뒤 대안 수 1~2, 재조회 제안 10쌍 중 1쌍). [spec](docs/superpowers/specs/2026-09-24-transit-alternatives-reasoned-design.md)

### N4 경유지 진행 표시 (iOS·웹 훅)

경유지 도보 안내가 경유지 50m 앞에서 "경유지 {이름}까지 {거리}"를 한 번 말하고, 도착 문장은 "경유지 {이름} 도착. 이제 목적지 {목적지}로 안내합니다"로 바뀌었다. 상세 시트·웹 패널의 남은 거리 행은 다음 목표 기준(도착 전 경유지, 뒤 목적지, 경유지 없는 세션은 종전 그대로). 리듀서 3벌(웹·Kit·`:kit`)에 접근 예고 이벤트·`guideNextTarget`·공유 fixture 12건, 웹 안내 훅·`DistanceBeacon`이 경유지를 받는다(안드로이드 앱 배선은 E43). [spec](docs/superpowers/specs/2026-09-24-waypoint-progress-design.md)

웹 길찾기 화면이 경유지 조회에서도 도보 안내 시작 버튼을 세우고 조회 시점의 경유지를 안내에 싣는다(자동차 버튼은 경유지 조회에서 여전히 없다). 경유지 경로를 못 찾으면 경유지 없이 안내한다고 화면 창구로 말한다(화면 통합 테스트 `DirectionsWalkVia.test.tsx`, 경유지 조회 실호출 1회).
...
제품 참조가 없어진 `route.pedestrian.stepFreeToggle`을 6로케일·생성물(iOS·안드로이드)에서 지웠다(E42 후속).
...
조회 화면 도보가 "최단 경로"(카카오 `SHORTEST`, 기본 펼침)와 "계단 회피 경로"(카카오 `ACCESSIBLE`, 없으면 "큰길 경로" `BROAD_FIRST`) 두 줄이 됐다(ko, 웹·iOS). 계단 회피 토글은 없어졌고 안내 시작 버튼은 줄 안으로 들어갔다(웹 B9 ② 흡수). 서버는 `lines=1` 줄 목록을 새로 싣고 `variant=shortest`의 출처를 카카오로 바꿨다(옛 `alternatives=1` 봉투는 그대로 — 배포된 iOS·안드로이드도 최단이 카카오가 된다). en은 추천·최단(Tmap) 그대로. 대중교통 승차 전 도보의 계단 회피도 토글과 함께 없어졌다(BACKLOG E42 남은 판정 ①). [spec](docs/superpowers/specs/2026-09-23-walk-two-lines-kakao-design.md)
...
고른 차량을 기다리는(boarding) 동안에도 keep-alive 위치 스트림을 켜서, 화면을 끄거나 다른 앱을 써도 폴이 돌고 도착 관측 승격이 기본 경로가 되게 했다(iOS 실험판). 관측이 끝난 뒤의 수동 진행 버튼은 [선택한 열차에 탔어요]·[선택한 버스에 탔어요], 대기 국면은 [이미 탔어요]로 바꾸고, 세 안내 문장(`vehiclePassed`·`boardingSignalLost`·`boardingUpstreamFailed`)에서 버튼 이름을 인용하던 꼬리를 뺐다(웹·iOS, 6로케일). [BACKLOG A46](docs/BACKLOG.md#a46-고른-열차를-기다리다-화면을-끄면-앱이-도착을-못-보고-돌아오면-도착-정보-없이를-묻는다--2026-09-23-위원장-실승차-피드백-ios-실험판---코드-종결2026-09-23-세션-a46-changelog-같은-날-실승차-판정-대기).
...
정식 기능 `debug`를 한소네 7에 설치하고 APK 해시 일치와 실행을 확인했다. 스크린리더 연결을 유지한 채 주소 경쟁 상태 2개와 길찾기 화면 검사 2개가 통과했다. 기기에서 Mac 경로를 찾던 테스트 fixture를 테스트 APK assets로 옮기고 화면 스크롤·비동기 선택 완료 대기를 보완했다. 같은 날 사용자가 한국어 사용 테스트의 정상 작동을 확인했다. 언어 전환 사용자 판정은 남아 있다. [검증 기록](docs/superpowers/plans/2026-09-20-android-directions-address-sync.md).

## 2026-09-20

### 안드로이드 길찾기 현재 주소 요청 동기화

측위 시작부터 주소 반영까지 같은 요청이 소유하도록 iOS 상태 관리를 이식했다. 취소·필드 변경·언어 변경 뒤 늦은 응답을 폐기하고, 이전 요청의 종료가 최신 로딩을 끄지 않도록 했다. Android 전체 게이트·웹 회귀 테스트·별도 리뷰 통과, 한소네 7 실기기 검증은 연결 대기다. [검증 기록](docs/superpowers/plans/2026-09-20-android-directions-address-sync.md).

### 안드로이드 개발 기준 확정·한소네 7 첫 실사용 확인

배포 전 개발과 iOS 기능 동기화는 정식판을 기준으로 하고, 실기기 테스트는 한소네 7로 진행한다. 운영 기준은 [android/README.md](android/README.md#배포-전-개발검증-기준).

도보 실시간 안내의 정식판 이동을 iOS 등가성 원칙에 따라 확정했다. 구현은 BACKLOG E43의 4순위이며, 한소네 7 정식 기능 구성 검증을 이동 작업의 완료 조건으로 둔다.

한소네 7 H732B(Android 15, API 35)의 USB ADB 승인 연결을 확인하고 정식 기능 구성의 `debug` 0.1.0(versionCode 1)을 빌드·설치·실행했다. 사용자가 설치와 내 주변 탭의 둘러보기, 길찾기 탭의 경로 조회, 조회 후 자동 초점 이동이 예상대로 동작함을 직접 확인했다. 화면 구조 조회 도구는 응답 대기로 중단했으며, 이번 확인은 마일스톤별 전체 접근성·실보행 검증 완료를 뜻하지 않는다.
...
### iOS 브리핑·최근 위치·상태 표시 보정 (E45, E28, E43)

빈 역명·공백 역명으로 승하차 문장이나 로터 액션을 만들지 않도록 부재 판정을 통일했다. 최근 장소·경로는 지정 당시의 라틴 표기를 보존하며, 길찾기 현재 주소는 측위 시작부터 최신 요청만 반영한다. 위치 취득 실패와 서버 실패, 권한 거부와 대략적 위치를 구분하고 경유 정류소 전용 통지와 유효한 지도 URL에만 서는 액션을 적용했다. [계획·검증](docs/superpowers/plans/2026-09-19-ios-backlog-parallel-plan.md).

### 브리핑 저장 응답의 무호출 재검증 (E47 ① 일부)
...
길찾기 탭의 대중교통 브리핑에서 지하철 구간 줄에 들린 역 이름마다 VoiceOver 로터에 "{역} 상세 보기"·"{역}에 전화 걸기"가 선다(역별 묶음, 등장 순). 줄은 전부 `Text`로 남고 역 개수는 액션 수만 정한다. 대상 역은 **이름 조인**으로 고른다 — 정차역 목록의 첫·마지막 항목은 서버가 무효 항목을 떨어뜨리면 조용히 중간역이 되므로 위치 인덱스를 쓰지 않고, 조인에 실패하면 진입점을 만들지 않는다. 진입점은 `TransitRouteRows` 옵트인이고 켜는 곳은 길찾기 탭 하나다(안내 조망 "다른 경로"엔 push 스택이 없어 액션이 무반응이 된다). 전화 액션은 저장소 상태와 무관하게 상시이고 라벨만 직통·대표번호로 갈리며, 번호 없음·찾는 중·조회 실패를 통지와 진동으로 가른다(모름은 `resolve` 킥오프 뒤 통지). 안내 시트 경유역 로터도 같은 창구로 통일했다(E44 §5.5 개정 — 그 화면은 3상태를 액션 부재로 뭉개고 있었다). 실호출 조인 성립률 95/95(spec §8.1). spec `docs/superpowers/specs/2026-09-18-briefing-station-entry-design.md`, 계획 `docs/superpowers/plans/2026-09-18-briefing-station-entry.md`.

## 2026-09-17

### iOS 역 장소 상세 개편 — 순서·시설 접기·역 전화번호·경유역 로터 (E44)

역 상세(검색 탭 포함, `stationLayoutKind`)가 역 정보(전화 맨 위) → 실시간 도착·첫차막차 → 교통약자 시설(종류별 접기, 접힘 줄에 운행 중지 수) → 무장애 → 길찾기 → 이 장소 주변(지하철 도착 제외) 순이 됐다. 대중교통 안내 시트 경유역은 카카오 검색으로 그 노선 역 전화번호를 찾아(같은 역·같은 노선·1km, 다른 노선으로 떨어지지 않음) 상세 전화 줄과 목록 로터 "전화 걸기"로 내고, 운영사 대표번호는 "대표번호"라고 밝힌다. 같은 역·같은 노선 역무실 항목에 번호가 있으면 그것을 먼저 쓴다(환승역 오번호, 판정 ⑦). 조회 실패는 문장으로 가른다. 역이 아닌 장소의 상세는 그대로. 실호출 게이트 불일치 0(spec §8). spec `docs/superpowers/specs/2026-09-17-station-detail-reorg-design.md`, 계획 `docs/superpowers/plans/2026-09-17-station-detail-reorg.md`.
...
내 주변 둘러보기 "한눈에 보기"가 `가장 가까운 곳은 봉래면옥으로 남쪽 40m, 김밥천국으로 동쪽 60m입니다`처럼 이름 뒤에 수치를 붙여 읽어 어색했다. 이제 `가장 가까운 곳은 남쪽 40m 지점에 있는 봉래면옥이고, 동쪽 60m 지점에 김밥천국이 있습니다`로 읽는다 — 첫 곳은 `nearestFirst`가 서술격 조사를 받고, 나머지는 `nearestItem` 나열이며 ko는 마지막 이름에만 주격 조사가 붙는다(판정 불가면 조사 자리를 비운다). 한 곳뿐이면 나열이 없어 `nearestOne`(`… 지점에 있는 GS25입니다`). 같은 불릿 앞머리의 지하철역 문장도 같은 어순으로 맞췄다(`서쪽 287m 지점에 있는 5호선 길동입니다`). 첫 곳에도 방향을 싣는다 — 위원장 판정 2026-09-13(수치만 남기면 가장 가까운 곳이 어느 쪽인지 알 수 없다).

부수 효과로 en의 단복수가 정확해졌다(`The nearest is` / `are` — 종전엔 한 곳이어도 `are`). 웹 `overview-lines.ts` ↔ Kit `LocationNarrative.swift` ↔ CLI `formatters.ts` 3벌을 함께 고쳤고, ko 어순이 iOS 위치 인자 ABI라 `ios/i18n/arg-order.json`을 3키 갱신했다(게이트가 정확히 그 3키를 잡았다). 실측: 길동 좌표 실호출로 변경 전후 문장 대조(불릿 6종·ko/en). 검증: 웹 4,253 · tsc 0 · lint 0 · CLI 235 · MCP 9 · Kit NearbyOverview 10 · 앱 타깃 빌드 성공.

### 지하철 도착 줄을 우리 문장으로 + 영문 줄이 잃던 현재역 (E37·A38, 웹+iOS)

내 주변 지하철·장소 상세의 도착 한 줄이 서울시 원문 표기를 그대로 낭독했다 — `[4]번째 전역 (하남검단산)`·`3분 후(2번째 전)`. 이제 그 문장을 **읽어 같은 뜻을 우리 문장으로 다시 쓴다**(`4정거장 전 하남검단산.`·`2정거장 전, 3분 후 도착. 현재 방배.`). 못 알아본 문장은 그 줄만 원문 + A32 꼬리라 실패 방향이 현행이다. 코퍼스 858행 전수에서 원문 폴백 0건·역명 불일치 0건. spec `docs/superpowers/specs/2026-09-13-subway-arrival-prose-design.md`.

**착수 게이트였던 심야 `barvlDt` 해석에서 백로그의 전제가 뒤집혔다.** "시간 문장이 아닌 행은 `barvlDt`가 전부 0"이라는 실측 표(20건 표본)가 틀렸고, 858행에서는 **65행**이 비0이다(최대 2026-09-12 00:29:50 `전역 출발`의 1200초). 즉 `CLAUDE.md`의 오발화 경고가 지하철에도 성립한다 → 불변식 I1: 게이트는 문장 모양이고 초 수도 문장에서 읽으며 `barvlDt`·`arvlCd`는 판정에도 값에도 쓰지 않는다.

**역명을 싣지 않기로 한 문법이 둘 있다**(불변식 I2). `{X} 전역출발`의 X는 조회 역 자신이고(12/12), `전전역 출발`은 관측이 얇아 `arvlMsg3`의 뜻을 확정하지 못했다(쓸 수 있는 1행이 문장과 어긋났고 나머지 1행은 20분 묵은 레코드) — 증거 없이 실으면 열차가 없는 역을 "현재 위치"로 낭독한다. 문안 1판에 그 꼬리가 들어 있었고 위원장 확인으로 뺐다. 한 역 앞을 `전역`이 아니라 **`1정거장 전`**으로 부르는 것도 왕복에서 확정했다(버스 안내 상태 문장과 같은 말).

A38: 영문 현재역 자리의 결측을 **영문 값 자신**으로 가른다(종전엔 ko 칸이 비었다는 이유로 `currentLocationEn`을 버려 en·ja·es·fr·it 사용자만 현재역을 통째로 잃었다). 문장형 경로는 같은 축으로 세워져 구조로 닫힌다.

검증: 웹 4,246 · tsc 0 · lint 0 · Kit 725 · 앱 타깃 빌드 성공. 설계 적대적 리뷰 **조건부 통과** 후 MAJOR 2건(릴리스에서만 줄이 사라지던 미매핑 키 폴백, `[N]번째 전역` 괄호 선택)과 권장 5건을 닫았다. ⚠ 채팅 **산문**은 미적용 — `router.ts`가 LLM에 원문을 넘겨 카드와 산문이 갈린다(서버 코드라 동결 대기, BACKLOG A42).
...
### 경로 브리핑에 지하철 출구 번호 (E25 낭독의 나머지 절반, 웹+iOS 정식판)

출구 번호가 **안내 세션에만** 있었다(2026-09-02 확정 도착 통지·경유역 목록 하차역 행). 출발 전에 듣는 길찾기 결과 브리핑에는 없어 "어느 출구로 들어가고 어느 출구로 나가는가"를 앱이 답하지 않았다(위원장 요청 2026-09-07 04:32 KST). 데이터는 2026-09-02부터 이미 웹·iOS 양쪽에 내려와 있어 순수 렌더 추가다. 판정 셋은 위원장이 실호출 시안을 보고 정했다 — ①탈 때·내릴 때 둘 다 ②내리는 줄은 문 위치 먼저·출구가 끝 ③승차 출구는 앞 도보 줄에.

승차 출구를 어느 줄이 싣는지는 **서버 문맥이 아니라 렌더되는 구간 배열의 직전 항목**으로 가른다: 직전이 도보면 그 줄, 아니면 탑승 줄 끝. 0m 도보 leg를 `odsay.ts`가 화면 목록에서 지우므로("버스에서 바로 갈아탄다"·"역에서 출발한다") 서버의 "역 밖 진입"과 "붙일 도보 줄이 있다"가 어긋나는데, 화면 구조로 가르면 겹침·누락이 둘 다 불가능해진다. 배타 술어 둘(`boardExitAfterWalk`·`boardExitOnBoardLine`)이 그 계약이고 불변식 테스트가 잠근다. 빠른하차가 없던 역은 종전에 하차 줄 자체가 없었으므로 하차역 이름으로 줄이 새로 선다(실호출 개화→중앙보훈병원이 그 경우). 출구 문구는 안내 세션과 **같은 키**(`transitGuide.exitBound`)를 재사용한다.

웹 `src/lib/transit-exit-lines.ts` ↔ Kit `TransitExitLines.swift` 미러. ⚠ Kit 판만 `exitBound`를 클로저로 받는다 — `transitGuide`가 `KIT_NAMESPACES` 밖이라 Kit 카탈로그에 그 키가 없고, kit-extra에 복제하면 두 벌이 되어 갈린다(문자열 해석만 앱 층, 형식 게이트와 분기는 Kit). 소비자 셋을 함께 고쳐 화면과 도구가 같은 문장을 낸다: 웹 브리핑 `TransitRouteBriefing` · WebMCP 도구 출력 `DirectionsView.buildToolPlan` · iOS 길찾기 행 `RouteBriefing`. 도보 줄의 키·인자 순서는 Kit `TransitWalkLegText.resolve`가 계속 소유한다(`boardExit` 인자 추가, `ios/i18n/arg-order.json`에 신규 4키 등록). 신규 문구 4개 × 6로케일.

검증: 웹 4,216 · Kit 719 통과, 신규 28건(웹 17 · Kit 11), **변이 주입 6종 × 두 플랫폼 12회 전부 검출**(겹침·값 바꿔치기·순서 뒤집기·새 줄 소실·게이트 제거·ABI 인자 순서), 키 린터 exit 0, iOS 시뮬 빌드 + 실기기 2구성 설치. ODsay 실호출 2경로를 실제 구현 함수로 렌더해 ko·en·fr 육안 대조(ja는 ODsay 429로 미실측, 재시도 안 함). 실사용 판정은 `docs/BACKLOG.md` §2 E25 행.
...
- 새 i18n 키 2개(6로케일, `ios-extra`), 정식판 문자열·동작 무변경. spec 없음(bounded). 판정은 `docs/BACKLOG.md` §2 E30 행, 대본 `docs/FIELD-TEST.md` §4-2.

## 2026-09-12

### 대중교통 안내 상태 문장을 문장형으로 + 시작 통지에 목적지 + 통지·착지 중복 제거 (E39·E40·E41, iOS + 웹)

위원장 문안 확정(TextEdit 왕복 3판). **문체는 그대로 두고** 겹치는 조각과 언제나 참인 조각을 빼고 대괄호만 풀었다. 실승차 실측 `갈월동에서 421 탑승 기다리는 중. 차량 접근 중. 갈월동에 4분후[1번째 전]. 마지막 갱신 21:03.` → `갈월동에서 421번 버스 탑승 기다리는 중. 1정거장 전, 약 4분 후 도착. 21:03 기준.`

- **E39**: 잔여와 도착 서술을 한 줄 두 조각으로 묶어 쉼표로 잇는 공유 조립기 `arrivalStatusLine`(웹) ↔ `transitArrivalStatusLine`(Kit)을 신설하고 상시 표시·이벤트 통지가 함께 지난다. 서울버스 완성 문장은 `parseBusArrmsg` 구조로 우리 문장을 고르고(미지 모양은 원문 병치) 수단 낱말은 `*Bus` i18n 키로 붙인다. 대기 국면 `차량 접근 대기.`와 추적 중 `추적 중.`·`차량 접근 중.`은 그 값이 언제나 참이라 생략하고, 도착 국면은 잔여·도착 문장·갱신 시각을 전부 버린다. `마지막 갱신 {시각}.` → `{시각} 기준.`(내 주변 도착 줄의 `asOf` 표기와 통일).
...
위원장 판정: 시트에서 **무엇을 누르든 커서는 상태 문장 행**(`SheetControl.status`)에 앉는다. 하차 도착도 [다음 구간] 버튼이 아니라 "하차 지점 도착. …" 문장이다. 시트 진입·하차 도착·차량 선택·고른 열차 탑승·역 선택 취소 복귀(두 흐름)·목적지 전환 확정/취소·조망 안 경로 전환이 모두 그 자리로 간다.

구현 중 위원장 2차 판정 둘: ①시트 진입은 상태 문장 그대로 두되 **안내 시작 통지가 목적지를 말하게 한다**(상태 문장엔 최종 목적지가 없다 — 도보·자동차 포함, 별도 항목 BACKLOG E40) ②**→waiting 전이만 예외**로 차량 선택 목록의 질문 라벨에 앉는다("이미 탑승" 흐름이 같은 목록에 다른 문으로 들어가면서 라벨에 착지하므로 두 문을 맞춘다).

종전 대상 `advance`·`changeBoarding`·`boardAlready`·`title`이 `SheetControl`과 웹 ref에서 사라졌다. 남은 예외는 자기 질문을 여는 화면(역 선택 헤딩·급행 확인 헤딩과 그 답 문장·차량 선택 라벨)과 띠바 복귀·목적지 전환 상태 행이다. A35 착지 기제(실현 관측 → 반복 가시화 → 늦은 검증 → 폴백 통지)는 불변.

⚠ 착지가 한 자리로 몰리면 착지 테스트의 검출력이 0이 된다(변이 주입 실측) — 웹 테스트는 누르기 전에 그 컨트롤로 커서를 옮겨(`clickFocused`) 실기기 경로를 재현하고 착지 지점 8개에서 검출을 확인했다. →waiting 전이만은 목록 포커스 소실 복귀가 같은 자리로 되돌리는 중복 경로라 결과 단언으로 잠기지 않아, 웹 컴포넌트도 소스 가드가 읽는다.

a11y 감사가 연 두 항목: 착지 낭독과 전이 통지의 중복(E41), 그리고 판정 전제였던 "버튼은 한 번 스와이프 아래"가 실제로는 경유역 목록을 지난 두 번이라는 사실 정정(문서·주석 4곳).
...
실호출(갈월동 `03012`, 차량 22대·원문 2,430행)로 `isArrive1`이 0 고정임을 확인해 정차 신호 후보 ①(같은 API 구조 필드)을 기각하고, "곧 도착"(잔여 0 = 직전 정류소 출발, 간선 90~211초 유지) 뒤 목록 소실을 승차 승격 신호로 삼았다. 리듀서(웹 `transit-guide.ts`·Kit `TransitGuide.swift`, 공유 fixture 14건 추가(ⓐ~ⓝ)·2건 개정 + 톤 fixture ⑭)에 이벤트 `arrivingAtBoardStop`("{노선} 곧 도착합니다")·`arrivingAtAlightStop`("이번 정류장에서 내리세요")과 승격 원인 `departed`를 더하고, 서울버스 riding의 확정 도착을 제거했다(소실 → 종전 도착 추정, 확정은 하차역 선언뿐). 설계 리뷰 반영 2건: "0을 본 뒤의 추정 도착"은 소실이 이어져도 `signalLost`·경고음을 내지 않고 폴 60초(종전 확정 도착이 침묵하던 자리의 회귀 차단), riding 첫 조회는 15초(승격이 출발 뒤로 밀린 만큼 짧은 구간의 하차 신호를 놓치지 않게). 지하철·근사·비관측 경로와 provider·라우트는 무변경, 새 i18n 키 2개(6로케일). spec `docs/superpowers/specs/2026-09-12-seoul-bus-stop-arrival-signal-design.md`, 조사 스크립트 `scripts/verify-seoul-bus-stop-arrival.mjs`. 실승차 판정은 `docs/BACKLOG.md` §2 N3 ②.
...
- **유휴 폴 정지를 한 문장으로 알린다(E36 ⑫ 종결, iOS 실험판, 세션 idle-notice)**: 화면을 켜 둔 채 한동안 버튼을 누르지 않으면 폴이 멈추는데(유휴 한계 max(30분, 2×구간 소요)) 화면이 그대로여서 멈춘 사실을 알 수단이 없었다. `enterIdleIfDue`가 새 키 `transitGuide.idlePaused`("한동안 조작이 없어 열차 조회를 잠시 멈추었습니다." 위원장 확정 문안, 6로케일)를 `.high`로 게시한다 — 재개 방법은 붙이지 않는다(위원장이 뺐다). 창구는 재개 문장(`announceNow`)과 달리 **자동 통지 창구 `announce`**다: 사용자 활성화의 응답이 아니라 타이머 판정이라 톤이 울리는 중이면 그 뒤에 말해야 한다. 백그라운드 정지는 `post`의 전경 게이트가 버리고, 전경 복귀가 곧 조작이라 재개 문장이 그 자리를 대신한다. 앱 타깃에 테스트 레인이 없어 배선은 소스 가드(`transit-background-guards.test.ts`), 변이 주입 2회(게시 제거 / `.high` 강등)로 검출력 실측. 실승차 판정 `docs/BACKLOG.md` §2 E36 ⑫(대본 `docs/FIELD-TEST.md` §5-2).
- **boarding 국면 [탑승했습니다] 제거 + 대기 국면 "이미 탔습니다"(N3 재설계 2건 종결, 웹+iOS 실험판, 세션 small-batch, spec `docs/superpowers/specs/2026-09-11-boarding-manual-advance-design.md`)**: 위원장이 두 세션에 걸쳐 보고한 혼선의 뿌리는 버튼 하나가 아니라 이름이 비슷한 둘이 번갈아 서는 것이었다("탑승했습니다" ↔ "이미 탑승했습니다"). ①차량을 고른 직후의 선언 버튼을 없애고 **승차 정류소 도착 관측**이 riding 승격을 하게 뒀다 — 수동 진행 수단은 관측이 끝났을 때만(`transitBoardingObservationLost` = `signalLost`·`upstreamFailed`, 앱 층 래치로 회복에도 사라지지 않는다) 다른 문구 [도착 정보 없이 탑승 진행]으로 선다. 그 버튼은 조용히 서므로 그 순간의 통지 셋(`boardingSignalLost`·**신설** `boardingUpstreamFailed`·`vehiclePassed`)이 이름을 부른다 — **키 신설은 판정 범위 밖 판단이고 근거는 헌장 §3이다**(조용히 나타나는 수단의 발견 경로가 그 통지뿐이라, 조회 실패 통지에도 `signalLost`와 같은 국면 분기를 뒀다. 코디네이터 수용 2026-09-11). 차량 선택 전이의 착지는 `.confirmBoarded`에서 상태 문장(`.status`)으로 — 그 국면엔 다음 행동이 없다. 반대로 **그 다음 전이(boarding → riding)의 자동 착지는 뺐다**(구현 리뷰 M1): 승격은 폴이 일으키고 커서가 얹힌 상태 문장은 사라지지 않으므로 착지가 포커스 강탈이 된다. 관측 승격 직후 하차역 첫 폴을 즉폴로 메우는 안은 **구현 리뷰(H1)가 코드로 기각했다** — 그 왕복(300~900ms)이 `boarded(observed)` 통지의 지연 슬롯(톤 1.33초 뒤 발화) 안에 들어와 "탑승. 하차: {역}."을 latest-wins로 버린다. 60초 공백 자체는 A33 문장이 이미 설명하고, 지하철 도착 목록은 근접 열차만 주므로 즉폴의 이득은 하차역 1~2정거장일 때로 한정된다(spec §4.5). ②대기 국면 버튼은 **"이미 탔습니다"**(ko만 — 다른 로케일의 쌍은 ①이 하나를 없애며 해소됐다). 지방버스 대기 국면의 같은 의도 버튼(`boardApprox`)도 6로케일 전부 그 값으로 동조했다 — 같은 국면·같은 의도가 수단에 따라 두 이름으로 들리는 것이 혼선의 모양 그대로였다(구현 리뷰 M7). 탈출구는 ②가 대신한다(버튼 없는 동안 타 버렸으면 [다른 차량 선택] → [이미 탔습니다]가 실제 탄 열차를 다시 지목해 더 정확한 잠금이 된다). 설계 리뷰 생략: 판정 확정이고 리듀서 입력·전이는 하나도 바뀌지 않는다(입력을 낼 수 있는 때만 좁혔다). 변이 주입 4회로 검출력 실측(술어 항상 참 / 래치 세우기 제거 / 래치 소거 제거 / A39 거절의 0 덮기). 구현 리뷰 3종(spec·code·a11y)에서 HIGH 1·MEDIUM 13·MINOR 19 중 채택 다수·유예 4·기각 4(판정은 spec §9). 실승차 판정 `docs/BACKLOG.md` §2 N3 행.
- **설정 체중 입력의 범위 밖 값은 직전 값을 유지하고 말한다(A39 종결, iOS, 세션 small-batch, spec `docs/superpowers/specs/2026-09-11-settings-weight-commit-design.md`)**: 20~300 밖을 `?? 0`으로 접어 저장하면서 필드에는 친 값이 남아 **스크린 리더 사용자는 저장된 줄 알았다**(3-state 위반). 판정 시점을 타자 한 글자마다에서 **편집 종료**로 옮기고(⚠ `.decimalPad`엔 Return이 없어 `onSubmit`이 오지 않는다 — 포커스 이탈·닫기 핸들러·`onDisappear` 셋이 종료 신호이고 커밋은 멱등), 판정을 Kit 순수 함수로 갈랐다(`WalkHealth.weightCommit` → store/clear/reject). 거절은 저장하지 않고 직전 값을 유지하며 필드를 되돌리고 `.high`로 1회 통지한다(직전 값 유무로 문장이 갈린다). 체중 Section 푸터에 허용 범위 + "입력한 정보는 서버에 저장되지 않아요"를 상시로 — E31 §5 잔여(권유가 사라진 뒤 그 문장이 앱 어디에도 없던 것)를 흡수했다. ⚠ 생명주기 훅은 **행이 아니라 화면 수명**에 건다(구현 리뷰 3종이 같은 자리를 짚었다 — `List`는 행을 지연 실현·해제하므로 행에 달면 VoiceOver 스와이프만으로 커밋이 돌아 타자 도중 되돌림이 난다). 뷰 레인이 없어 배선은 소스 가드(`settings-weight-commit.test.ts`), 변이 주입 1회로 검출력 실측. 설계 리뷰 생략(판정 확정·국소), 남은 실기기 확인 2건은 spec §5.
- **대중교통 안내 시트 컨트롤 착지 재설계(A35 코드 종결, iOS 실험판, 세션 transit-3, spec `docs/superpowers/specs/2026-09-11-transit-station-to-place-and-landing-design.md`)**: 09-05·09-09 로그(착지 1/6~5/6)를 전수 판독해 실패 기제를 셋으로 갈랐다 — ①오프스크린 AX 컬링(가설 — `actual=nil`은 "VO가 우리 대상 밖"과 구분되지 않는다) ②VoiceOver 자체 재배치(헤더로 되돌아간 실패 4/4가 전경 복귀·시트 등장 14초 안) ③트리거 결손(프롬프트 헤딩의 `.task`가 착지를 불러 헤딩이 실현되지 않으면 시도 자체가 없었다). 헬퍼를 "실현 관측(onAppear 참조 카운트) → 실현될 때까지 반복 가시화(전 대상 — 종전 5대상이 `default: break`) → 대입 → 100ms 단위 늦은 검증(600·900·1200ms 체증, 3회) → 실패 시 전 대상 공용 폴백 통지 `.high`"로 바꾸고, 착지 트리거를 상태 변화(`reboardPickerActive`·`aboardStep`)로 옮겼다. 배경·모달(장소 상세·목적지 검색 시트) 위에선 시도하지 않고 복귀·닫힘에 한 번(E36 배경 폴과의 경합, 진행 중 착지도 배경 전환이 끊고 이월). 계측 `controlFocus`에 `attempts·elapsedMs·rendered·reason·vo`(실제 VO 커서 라벨)를 실어 다음 로그 회수가 기제를 판정한다. 적대적 설계 리뷰(opus) MAJOR 8 전부 채택(L5 부분 — spec §9). 웹 무변경. 실기기 재판정 `docs/BACKLOG.md` §2 A35 행(A19 행 되살림).
- **안내 시트의 지하철 경유역·상태 문장 → 장소 상세(E33 iOS 코드 종결, 실험판, 세션 transit-3, 같은 spec)**: 표현 계층은 **현행 중첩 시트 하나**(`detailPlace`, 목적지·경유역 공용) — 착수 전제 "시트 위 시트 금지"는 N1 M3의 반대 방향 오독이었다(M3는 장소 상세가 떠 있을 때 루트 안내 시트를 올리는 것을 기각했고, 안내 시트 위 장소 상세는 2026-08-12 §2 채택 정본). 지하철 leg의 경유역 행은 행 전체가 버튼(단일 `Text` 라벨), 상태 문장은 뷰 불변 + 역 언급 수만큼 로터 액션 "{역} 상세 보기"(리뷰 E2: 폴마다 뷰 종류가 갈리면 포커스가 얹힌 줄이 15초마다 재생성된다 — 채팅 "1개면 블록 버튼" 갈래는 정적 산문 전제라 쓰지 않았다). 라벨은 descriptor 키 `openStation`(다른 descriptor와 같은 언어 규칙 — iOS 렌더러의 줄 언어 미태깅은 E28-① 정본), 언급 대응은 ko·en 라벨 둘 다(상태 문장은 조각별 언어가 갈린다). 구현 리뷰 3종(spec·code·a11y) MAJOR 5·MEDIUM 3 전부 반영 — 배경 전환·모달(상세·검색 시트) 위 착지는 이월, 최소화는 Task 취소, `controlExists`에 untrackable·지방버스 축(spec §10). 식별은 `viaStops` 좌표·`stationId`(`transitStopPlace` — `name`은 ko 조인 키). **웹 미연결**: 웹 세션은 길찾기 뷰 수명이라 `requestOpenPlace`가 뷰를 언마운트해 안내가 조용히 끝난다(BACKLOG E33 잔여). 실기기 판정 §2 E33 행(시트 닫힘 뒤 커서 복원이 블로킹 항목).

- **`neverSeen` 상한을 벽시계가 아니라 riding 조회 횟수로(A36 ① 종결, 웹+iOS 실험판, 세션 transit-2, spec `docs/superpowers/specs/2026-09-11-transit-background-poll-design.md`)**: 09-05 로그에서 주머니에 넣어 둔 9분이 "차량 확인 안 됨"의 근거로 세어져 복귀 첫 폴에 `neverSeen`이 났다. 판정 축을 `ridingSince`(10분)에서 `ridingPolls`(10회 = 미등장 주기 60초 등가)로 바꿨다 — riding에서 결과(ok·empty)를 받은 조회만 세고 실패 폴·boarding 폴·비관측 잠금은 세지 않으며, riding 진입(탑승 변경 취소 복귀 포함)에서 0으로 리셋한다(리뷰 B1: `restoreBoarding`은 `enterRiding`을 지난다). Kit ↔ 웹 미러, 공유 fixture 4 개정·3 신규, 하네스 `ridingPolls` 단언 키. 벽시계 백스톱은 위원장 판정("죽어 있던 구간을 시계로 세는 것은 틀렸다")과 충돌해 두지 않았다(리뷰 M1 기각). 실승차 판정 `docs/BACKLOG.md` §2 A36 행.
- **대중교통 안내 백그라운드 폴 지속 + 첫 관측 소리 + 유휴 폴 정지 + keep-alive 위치 스트림(E36 코드 종결, iOS 실험판, 세션 transit-2, 같은 spec)**: `pausedInBackground`를 폐지해 폴이 국면·주기·유휴만 보게 했다. 그런데 코드 실측으로 위원장 판정의 전제("도보와 같은 방식")가 절반만 참이었다 — `TransitGuideModel`은 오디오 승격(`beginSession`)을 한 번도 하지 않아 톤이 전부 `.ambient`(백그라운드 무음)였고, `audio` 백그라운드 모드는 소리를 내는 동안만 앱을 살린다(소리 없는 승차 국면은 30초 안에 재워진다). 그래서 ①대중교통도 세션 중 `.playback` 승격(무음 스위치 무시는 도보와 같은 트레이드오프) ②백그라운드 톤은 `trackingStarted` 하나(`playTone(_:allowedInBackground:)` 기본값 없음, 소스 가드가 허용 자리 1개를 센다) ③음성은 `post` 전경 게이트로 0, 복귀 시 버린 통지가 있을 때만 현재 상태 한 문장(`neverSeen`이면 탈출구 행동 문장) ④**riding 동안 저정밀 keep-alive 위치 스트림**(위원장 판정 ⓐ 2026-09-11: 상태바 위치 표시·약간의 배터리 수용, 좌표 미소비, 권한 없으면 열지 않음) ⑤**유휴 폴 정지**: 마지막 조작 이후 max(30분, 2×구간 소요)면 폴·keep-alive를 멈추고 세션은 유지, 어떤 조작·전경 복귀든 "안내를 재개합니다. {상태}"로 재개(리뷰 B3 — 종전엔 잊힌 세션의 폴을 `pausedInBackground`가 우연히 막았고 `transitSessionPollCap`은 상한이 아니라 감속 문턱이다). `LocationService`는 세 스트림 프로파일을 한 함수에 모으고 단발 취득이 keep-alive를 끊거나 Best를 남기던 구멍을 닫았다(리뷰 M2·M3). 적대적 설계 리뷰(opus) BLOCKER 3·MAJOR 7 중 M1만 기각(spec §9). 실승차·배터리·쿼터 판정 §2 E36 행(대본 `docs/FIELD-TEST.md` §5-2).
- **오디오 세션 소유권 세 구멍(PORTS 행 종결, iOS 정식·실험 공통, 세션 transit-2, 같은 spec §4.3)**: dodo 관찰 셋을 gildongmu에서 재현·판정했다. ①"카테고리와 활성화를 한 값에" — 재조정 단락 분기는 원래 없어 미재현이나 **같은 증상의 다른 경로**(인터럽션 `.began` 미처리 + `.ended` 유실 + `play()`가 카테고리만 보고 재확보 안 함)가 있어 활성화 축 `isActive`·`.interruptionBegan`을 더했다 ②"원복 자격 저장" — 불변식 `didPromote == (마지막 적용 == .playback)`이 리듀서 층에서 성립함을 이벤트 9종 길이 ≤5 전수 열(59,049)로 못 박았다(미재현) ③`routeChangeNotification` `.categoryChange` 메아리 미필터 재현 — 세션 시작마다 플레이어 재생성이 한 번 더 돌았다. 전면 필터는 채팅 TTS `duckOthers` 탈취에서 회복하는 유일한 신호까지 없애므로(리뷰 M5) 시간 창이 아니라 **현재 세션 값 == 적용값** 대조로 메아리만 거르고 남의 변경은 `.categoryTakenOver`(재생성 없이 재적용). 부수: 재생기가 둘(도보·대중교통)이라 인계에서 먼저 끝난 쪽의 미뤄진 `.ambient`가 새 소유자의 `.playback` 위에 떨어지던 경합(리뷰 B2, 대중교통 승격 전엔 증상 없음)을 전역 최신 소유자 토큰 + `.ownershipTransferred`로 닫았다. 변이 주입 3종 각각 검출 확인. 실기기 판정 §2 PORTS 행.
...
- **체중 입력 권유를 두 번 무시하면 그만 띄운다(E31, iOS)**: 도보 종료 화면에서 권유가 떠 있는 채 [체중 입력하기]를 누르지 않고 [닫기]를 누른 것만 "무시"로 세고(시트 최소화·스와이프·30분 만료 소거·앱 종료는 세지 않는다), 두 번이면 권유 문장·버튼·"서버에 저장되지 않아요" 고지가 사라진다. 그 뒤로는 기준 체중이 칼로리 문장 안으로 들어가 줄 수가 늘지 않는다("이번 구간에서 1,240걸음 걸으셨어요. 65kg 기준으로 약 39kcal를 태우셨어요."). 체중을 입력한 사용자의 문장은 종전 그대로다(두 벌 키 `ios.beacon.healthSummary`·`healthSummaryWithWeight`, 6로케일). 판정은 Kit 순수 함수 둘 + 뷰 배선 소스 가드(`weight-prompt-wiring.test.ts`)다. spec `docs/superpowers/specs/2026-09-11-weight-notice-dismissal-design.md`.
- **승차 뒤 상태 문장 3키를 열차/버스 수단별로(A33 종결, 웹+iOS 실험판, 세션 transit-1)**: "차량 위치 확인 중."이 앱이 내 위치를 못 잡는 것으로 들렸다(대중교통 추적은 GPS를 쓰지 않는다). 위원장 확정 문장 "하차역에 가까워지면 열차 위치가 표시됩니다."로 바꾸고 형제 둘을 같은 어투로 3-state로 갈랐다(잠시 끊김 / 끝내 확인 못 함). 같은 키가 버스 승차에도 쓰여 `*Bus` 3키 신설(하차 정류장·버스), 6로케일. 소비 3곳(상시 표시·복귀 통지·조망 침묵 행)이 열차/버스 축을 기본값 없는 인자로 받는다. 잔여 조각(노선·하차역·약 N개 정거장·마지막 갱신)은 불변.
- **하차역 선언·탑승 열차 재확인·도보 인계 단일 버튼(A37 ②·A34 ②+①·E34 코드 종결, 웹+iOS 실험판, 세션 transit-1, spec `docs/superpowers/specs/2026-09-11-transit-reboard-and-handoff-design.md`)**: 리듀서 입력 둘을 새로 열었다 — `declareArrived`(역 선택에서 하차역을 고르면 그 leg를 **확정 도착**으로 끝낸다, waiting·riding에서. 대기 국면으로 되돌리던 A37의 막다른 길이 닫힌다)와 `boardAboard`(사용자가 지나는 역의 도착 목록에서 고른 열차로 boarding을 지나지 않고 riding 직행). [이미 탑승했습니다]는 이제 **역부터 묻고**(전용 질문 "지금 어느 역을 지나고 계신가요?", 역 선택 화면 재사용) 그 역에 **있는** 열차(arvlCd 0~5, 두 정거장 밖 99 제외 — 리뷰 B2가 요구한 N3급 교차검증)만 목록에 세운다. 목록이 빌 때만 [열차 정보 없이 계속]으로 근사 잠금이고 그 잠금은 **비관측**(`transitLockIsUnobserved` — 지방버스 외 근사): 폴 0·매칭 0·어림값 표시 0, 상태줄은 "탑승 열차가 확인되지 않아 열차 위치를 표시하지 않습니다."뿐(급행 우선 매칭 분기는 유일한 소비자가 사라져 은퇴, 급행 확인은 통과 급행 거절 + 상시 문장으로 남는다). 마지막 leg의 버튼은 하나 — 라벨이 처음부터 "남은 도보 안내 시작"이고 한 번 누르면 leg 종료 + 도보 시작(`advanceIntoWalkHandoff` → `acceptWalkHandoff`, 진입점 계수 불변). 도착 통지는 그 버튼 이름과 도보 분을 담고(`arrivedWalkNext`), 인계 제안 화면(도달 경로 0)은 삭제. 확정 도착·비관측 riding은 폴 주기 0이고 **즉폴 게이트**가 함께 바뀌었다(종전엔 즉폴이 주기 0을 무시했다 — 리뷰 B1). 적대적 설계 리뷰(opus) BLOCKER 2·MAJOR 7·MINOR 6 전부 채택(spec §9). 공유 fixture 재작성 3·relock 1·신규 6, 변이 주입 1회(비관측 조기 반환 제거 → 빨간불 확인). 실승차 판정은 `docs/BACKLOG.md` §2 A37·A34·E34 행(대본 `docs/FIELD-TEST.md` §5-3).
...
- **마감 표기 정정**: 챌린지 마감이 OpenAI 측 장애로 12시간 연장돼 **2026-09-04 17:00 KST**(09-04 01:00 PT)가 됐다. 규정 페이지 산문은 원 마감(09-03 13:00 PT)을 그대로 들고 있어 어긋나므로 정본은 Devpost 마감 필드다. `docs/BACKLOG.md` 7곳·research·워크스페이스 `PORTS.md`를 갱신했고, 재발을 막으려 W1 절 제목에서 시각을 빼 앵커가 시각에 묶이지 않게 했다.
...
- **백로그 E30 햅틱 모드 등재**(위원장 제안 2026-09-03, 정식 포함 빌드가 2.0): 진동만으로 안내를 따라가는 설정 토글. 현행(톤 10/13종 진동 동반·항상 켬·백그라운드 불가)과 양자택일(화면 유지 vs Apple Watch)을 `docs/BACKLOG.md` E30에 적었다.

## 2026-09-02
- **문서 축소 — `CLAUDE.md` 175KB → 68KB, 상세 계약을 `docs/INTEGRATIONS.md`·신설 `docs/PATTERNS.md`로 이관.** 매 세션·매 리뷰어가 전량 읽는 파일이 두 달 만에 6.7배로 불어 세션 시작 비용의 59%를 차지했다(2026-09-02 웨이브 6세션·리뷰어 20여 개 × 약 10만 토큰). 91개 항목을 원문 그대로 옮기고(스크립트 이관, 본문 바이트 보존 검증) `CLAUDE.md`에는 규칙 한두 줄 + `→ INTEGRATIONS`/`→ PATTERNS` 참조만 남겼다. 재팽창 가드 `src/lib/__tests__/claude-md-budget.test.ts`(80KB 상한). doc-audit 스킬 감사 대상에 `docs/PATTERNS.md` 추가.

- **급행 정차역 집합 `expressStops` + 출구 번호 `exit` 서버 투영(A16 L1 데이터층·E25, 세션 transit-data)**: `/api/route/transit` `includeStops=1` 응답의 지하철 leg에 additive 두 필드가 실린다. `expressStops`는 그 노선 급행 정차역 이름 전체 집합(ODsay 원문, 노선 순서, 같은 순서의 `stationID`가 `expressStopIds`로 함께)으로 **표 노선(현재 9호선)의 완행·급행 leg 둘 다**에 붙고, ODsay 노선 전 구간 **정·역방향 2콜**을 수락 판정 7조건(전 구간 커버·양 끝·stationID 강단조·역순 일치·완행 진부분집합 등)에 태워 통과한 집합만 7일 `unstable_cache`에 굳는다 — 실패·미지 노선은 필드 부재(빈 배열은 절대 오지 않는다), 재시도는 노선별 단일 실행 + 실패 종류별 쿨다운(판정 부재 6h·429 1h·일시 10m). `exit`는 `startExitNo`/`endExitNo`를 **경로 문맥**(역 밖 진입 승차·역 밖 하차)에서만 투영한다. 웹 `TransitLeg`·Kit `TransitRouteLeg`(`TransitLegExit`) 옵셔널 디코딩. 부산물: 빠른환승 문 `"0-0"`(같은 승강장 완행→급행)이 문으로 통과되던 결함을 긍정 정규식으로 닫았다. codex 적대적 설계 리뷰 17건 → 채택 15·기각 2(spec §9; 코드 골든 seed는 위원장 판정과 충돌해 실호출 게이트 골든으로 대체). 실호출 게이트 `scripts/verify-odsay-express-stops.mjs`(16역 골든 전수·출구 기대값, 5콜)는 첫 실행 25/26(게이트 단언 결함 1건 수정), 수정본 재실행은 같은 날 ODsay 429로 미실측(A22 루틴, 2026-09-03 이후). 판정 계층(급행 결정적 차단 문장·출구 낭독)은 transit-guide 세션 몫. spec `docs/superpowers/specs/2026-09-02-express-stops-data-design.md`.
- **근사 잠금 급행 확인 프롬프트(A16 L1 잔여, 위원장 판정 2026-09-02, 세션 transit-guide, 웹+iOS 실험판)**: "이미 탑승했습니다"가 급행 집합이 있는 노선(9호선)에서 "급행인가요?"(헤딩 착지, 답 "급행 열차"/"일반 열차")를 한 번 묻는다. 급행 답은 `declaredExpressVerdict`(§4 판정을 후보 없이 재사용)로 통과 급행이면 잠그지 않고 "이 급행은 {하차역}에 서지 않습니다"(iOS `.high` 즉시 + 상시 문장 행 착지, 웹 live region + 상시 문장), 정차·판정 불가면 `TransitLock.express = true`로 잠가 하차역 목록에서 **급행 항목만** 추적한다(급행이 없으면 미등장 — 완행 폴백 없음. 공유 fixture "근사 잠금 급행 선언" — 더 가까운 완행이 있어도 급행 잔여). 집합 없는 노선은 프롬프트 없이 종전. 문장 키 3종 6로케일(`expressPrompt`·`expressYes`·`expressNo`). spec §6. 실승차 판정 `docs/BACKLOG.md` §2 A16 ⑧.
- **급행 결정적 미도달 게이트 + 출구 번호 낭독(A16 L1 판정 계층·E25 낭독, 세션 transit-guide, 웹+iOS 실험판)**: ①승차 후보 판정에 `expressVerdict`(웹 `transit-guide.ts` ↔ Kit `TransitGuide.swift`) — **ID 판정 1순위**(`expressStopIds` ∧ `alightStop.stationId`, 정규화 없음), 이름 판정은 폴백이고 자격 둘(하차역이 `viaStops`와 조인·집합이 `viaStops`와 이름 공유)이 붙는다. 결과는 `skips`(활성화 차단 + "이 급행은 {하차역}에 서지 않습니다") / `stops`("급행, {하차역} 정차") / `unknown`(집합 부재·자격 미달 — 종전 "정차 여부 확인 필요", 차단 금지). 종전 `terminatesEarly: boolean`은 **단일 차단 술어 `unreachable`**(종착 앞 / 급행 통과, 겹치면 종착)로 대체했고 선택 진입점(iOS `board(item:)`·웹 `boardCandidate`)도 같은 술어로 재판정한다. ②확정 도착 통지 끝과 경유역 목록 하차역 행에 "{N}번 출구 방면"(`transitGuide.exitBound`, 6로케일) — 추정 도착엔 붙이지 않는다(전 역에서 신호를 잃은 추정에 확정형 출구 안내는 잘못 내리게 한다). 서버가 형식·문맥(역 밖 하차만)을 걸러 실은 값에 소비자 형식 게이트(`validExitNo` ↔ `transitValidExitNo`)를 이중으로 둔다. 문장 키 4종(통지용 `exitBoundSentence`는 마침표 있는 문장형 — 공백 연결 채널에서 "방면 다음:"으로 이어지지 않게)·descriptor 2종·공유 fixture 9케이스·웹 4분기/Kit 미러 단위 테스트 + 웹 컴포넌트 테스트(통과 급행 행 버튼 부재). 차단 행은 급행 조각을 빼고 사유 줄만 결정 문장으로 둔다(종착 앞 + 급행 정차가 함께 붙으면 모순 낭독). 부수 수정: Kit `withoutPrewalk`가 en 이름 3종(`lineNameEn`·`boardNameEn`·`alightNameEn`)을 버리던 결함(en 세션의 승차 전 도보 뒤 첫 구간 이름이 한국어로 떨어졌다)을 필드 전량 복사로 닫았다. 실승차 판정 `docs/BACKLOG.md` §2 A16 ⑦·E5·B2 ⑫. spec 같은 문서 §4·§5.
- **대중교통 승차 추세 톤 + riding 계측 + 톤 뒤 발화(E15 ②·A16 미확정 ①②, 세션 transit-guide, iOS 실험판)**: ①**추세 톤 계층 신설** — 승차 중 잔여 정거장이 한 정거장 줄 때마다 `closer`, 늘면 `farther`, 신호 소실·조회 실패가 이어지면 60초마다 `unreliable`(잠정). 판정은 Kit `TransitGuideTone.swift` ↔ 웹 `transit-guide-tone.ts` 순수 미러(공유 fixture `transit-guide-tone-scenarios.json` 9시나리오·86스텝은 **리듀서 입력 단위**라 `transitGuideStep → transitToneStep`을 이어 돈다), 배선은 `TransitGuideModel.dispatch` 한 곳. 도보 `toneLayerStep`과 배타 계층·앵커 비교만 공유하고 축은 정수 정거장 수(데드밴드 1, 정지 축 없음). ⚠ 순서가 도보와 다르다 — **이벤트 소유가 신뢰 불가보다 앞**(신호 전이는 항상 이벤트와 함께 오므로 한 폴에 경고음 둘이 나던 설계 결함, 리뷰 #1). 앵커는 "마지막으로 전달된 잔여"라 이벤트가 말한 값으로 옮긴다(사다리 직후 중복 진행음 차단). 확정 도착만 억제(추정 도착은 경고 유지). 신규 소리 0(기존 3종 재사용). ②**계측**(`TransitGuideDiag`): `pollStart`/`pollEnd` 1쌍/폴(단조 시각 차·예정 주기·종료 상태, `defer`로 정확히 1회) · `scene` 전이 · boarding·riding·arrived `ridePoll`(리듀서와 같은 `transitFindLockedItem`로 매칭 여부·잔여·코드·현재역·나이·스냅숏) · `tone` — 08-16 로그가 못 가르던 "empty vs 매칭 실패"와 "폴 간격"이 다음 실승차에서 답을 얻는다. ③**대중교통 통지도 톤 뒤 발화**: `TransitGuideModel.announce`가 `AccessibilityNotification` 직접 게시에서 Kit `DeferredAnnouncer`로(도보 2026-08-14 계약) — 임박 큐·도착 종이 문장 앞머리를 잘라 먹던 자리. 사용자 직접 응답(진행 상황·새로고침·재조회·경로 교체)은 `announceNow`. ④이벤트 톤이 억제(검색 시트 받아쓰기) 중에도 나던 기존 결함을 `playTone` 단일 창구로 닫았다(리뷰 #4). codex 적대적 설계 리뷰 2회(1차 17건 → 수용 13·부분 1·기각 3, 2차 7건 → 수용 5·기각 2·위원장 판정 1) — spec `docs/superpowers/specs/2026-09-02-transit-trend-tone-and-express-gate-design.md` §8. §4(급행 미도달 게이트)·§5(출구 번호 낭독)는 transit-data 통합 뒤 착수. 실기기 미배포(위원장 지시 — 기기 미연결), 게이트는 Kit 테스트·시뮬 빌드. 실승차 판정 `docs/BACKLOG.md` §2 E15 ② 행.
- **장소 상세 영업시간 한 줄 정식판 승격(E24 종결, 위원장 판정 2026-09-02, 웹+iOS)**: iOS `experimentalPlaceHoursEnabled` 검사를 지워(플래그 졸업) 정식판 장소 상세에도 "오늘 영업시간 10:00~22:00 (Google Maps)" 한 줄이 전화 줄 앞에 선다. 웹 `PlaceDetail`에 같은 줄(`PlaceHoursLine.tsx` — 3초 상한, 어떤 실패도 침묵, jsdom 테스트 4축)을 달았고, 문구 4종은 `ios-extra`에서 `messages/*.json` `placeHours.*`로 옮겨 웹·iOS가 한 벌을 쓴다. privacy 문안 6로케일에서 "iOS 실험판" 낱말 삭제. TTS 격리 allowlist에 웹 소비자 2파일 추가(패턴 `placeHours\.`). 서버·약관·쿼터 계약은 불변(spec `2026-08-30-place-hours-google-design.md` 머리 개정 표기). ⚠ 상세 조회 무료분 33회/일을 웹·iOS가 나눠 쓴다 — 초과는 침묵(`docs/BACKLOG.md` E24 관측 항목).
- **이탈 시 재조회 확인 프롬프트 폐기 → 자동 채택(E10ⓑ 개정, 위원장 판정 2026-09-02, iOS)**: 이탈 확정 시 자동 조회한 새 경로를 "준비됨" 상태로 붙들고 "현재 위치부터 다시 안내 시작" 라벨로 수락을 기다리던 흐름을 지웠다 — 누르지 않을 이유가 없는 확인은 군더더기. 조회가 성공하면 즉시 채택하고 "새 경로로 다시 안내합니다. 안내 N개, 총 {distance}. {첫 안내}"(`guide.autoReroute`, `.high`)로 고지만 한다. 채택 시점 신선도(30m/120초 + 최근 fix 15초)는 §5.6 도로 중앙 실사고를 막는 유일한 안전망으로 남았고, 미달·실패·상한 도달이면 "경로 다시 조회" 버튼이 예비 출구다. 만료 능동 전이·워치독 시간 만료·`hasPreparedProposal`·`proposalAdopt` 6로케일은 삭제. 리뷰 반영: 자동 채택과 수동 재조회·전환의 이중 커밋을 양방향(토큰 폐기 + `rerouteInFlight` 가드)으로 차단, 자동 채택으로 재조회 버튼이 사라지면 시트가 제목 행에 착지(`offRouteEndedByReroute`). 웹은 자동 조회 자체가 없어 불변. spec `2026-08-12-walk-route-alternatives-design.md` §6 개정 표기, `docs/FIELD-TEST.md` E10 행 갱신.
- **간략 세션 도착 종료 + 종료 잔재 정리(A31, 위원장 실사용 2026-09-01)**: ①**간략 안내 세션이 목적지 근처에서 멈추면 도착 추정으로 스스로 끝난다** — 도착 추정(`presumedArrivalStep`)의 국면 게이트를 "도착 창"(최종 접근 국면 ∨ 간략 근처 창)으로 넓혔다. 판정 함수·임계(제자리 5분·두절 3분)는 불변이고, 간략 창의 자격은 새 Kit 리듀서 `briefArrivalWindowStep`(비콘 `nearby` 래치 ∧ 정확도 ≤30m — 래치는 정확도로 스케일돼 100m fix에서 200m까지 켜져 그대로는 종료 권한이 될 수 없다, 설계 리뷰 BLOCKER)이 정한다. 대상은 `tooClose` 인계만이 아니라 간략으로 도는 모든 세션(경로 조회 실패 강등 포함 — 종전엔 확정 도착 경로 자체가 없었다). 창 상태는 `resetArrivalWindow()` 한 곳이 지운다(경로 커밋·재획득·stop·간략 인계·강등·래치 초기화). 09-01 등굣길 로그 리플레이(`brief-window-replay.test.ts`): 건물 진입 직후 정확도 40→106m fix 10건이 창을 닫고 회복 뒤 재진입, 인계 395초 뒤 stationary 발동 — 실제 안전망 종료(20분)보다 13분 앞. ②**30분 지난 종료 화면은 백그라운드를 거친 복귀에서 걷어낸다**(`isEndScreenStale`, `ContinuousClock` 종료 시각 — systemUptime은 잠자기 동안 멈추고 벽시계는 앞으로 교정될 수 있다). `wasBackgrounded`를 `.active` 맨 앞에서 소비해 종전 비추적 상태의 영구 잔류(제어센터 왕복이 복귀로 오인)를 함께 고쳤고, 앱 루트는 유휴 리셋보다 먼저 세션에 전경 전환을 전달한다. 미뤄진 종료 통지는 함께 버리되 실패 문장은 남아 읽힌다. ③**`clearArrival`의 판별선을 `endKind != .stopped`에서 `status.isFailure`(denied·unavailable)로** — 안전망·사용자 중지 종료의 문장이 길찾기 탭 선두에 영영 남던 갈래(2026-08-17 `.arrived` 수정의 나머지). 소스 가드 9건(주석·문자열 제거 본문 기준). codex 적대적 설계 리뷰 BLOCKER 1·MAJOR 5 → 수용 5·부분 1(spec §8). 실보행 판정은 `docs/FIELD-TEST.md` §3(A31). spec `docs/superpowers/specs/2026-09-02-brief-session-arrival-end-design.md`.
...
- **CLI `route car --lang en` 한국어 폴백 표기 + `lang` 검증 통일(E29 종결, 세션 small-batch, plan `docs/superpowers/plans/2026-09-02-backlog-sweep-2-parallel-plan.md`)**: CLI 포매터 계약이 `FORMATTERS[name](body, ctx)`로 바뀌어 `runEndpoint`·`search`·`station info`가 요청 `lang`을 내린다. `route car`는 요청 en ∧ 응답 `guidanceLang: "ko"`일 때 요약 줄 다음에 `한국어 안내(영문 미제공)` 한 줄(웹 `lang="ko"`·iOS `CarRouteRows`와 같은 사실, 부재는 침묵). `/api/route/car`의 `lang`은 `langParam()`(미지 값 400), `/api/chat`의 `locale`은 지원 6로케일(미지 값 400 `invalid_locale`) — 종전 두 라우트만 오타를 조용히 한국어로 취급했다. 소비자(웹 `dataLocale`·iOS `AppLanguage`·CLI 원값) 전수 통과 확인. CLI README·`--help`·`packages/cli/CHANGELOG.md` 미출시 항목 갱신(발행은 코디네이터). 편승: `catalog-lang-drift`에 `/api/transit/track` 예외 — 2026-09-01부터 red였는데 루트 `test:run` include 밖이라 무관측이었다.
- **C5 빈 도구 응답 케이스 + systemInstruction 3-state 최소판 + 하네스 실호출(위원장 승인 1회+재실행 1회, 세션 small-batch)**: `src/__ab__/cases.ts`에 41~43(소아 진료 0건·근접 지하철 0건·따릉이 서울 밖, `safety`, `Case.location`으로 실데이터가 비는 좌표 지정) 추가, `system-instruction.ts`의 "실패하거나 빈 결과면" 한 문장을 빈 결과·정보 없음·실패를 각각 "없다·알 수 없다·조회하지 못했다"로 부르는 긍정 트리거로 재작성(금지절 치환, 순증 0). 3.6-flash 뒤집힌 케이스 0, 42·43 pass^3 통과. 실측이 드러낸 채점기 사각 6종(3자리 HHMM 진료시간·예시 괄호 역명·범주어·미터 반올림·`번길`·공공 창구 이름)을 수정하고 결과 파일에 `toolOutputs`를 남기게 했다. 41 `search_places` 보강은 같은 날 위원장 판정으로 **허용** — 41·37 forbid에서 제외, 41 `fromTools` 확장(재채점은 결과 파일에 도구 원문이 없어 다음 실행 몫, `docs/BACKLOG.md` C5). 리포트 `docs/evals/2026-09-02-model-ab-c5-empty-tool{,-rerun}.md`(이 저장소의 첫 커밋된 eval 결과 — 종전 08-25 결과는 `.ab-out/`에만 있어 기준선이 없었다).
- **CLI/MCP 0.10.0 발행(`cli-v0.10.0`, 위원장 승인 2026-09-02, 세션 doc-audit)**: 0.9.0 이후 묶음 — `lang` 배선(E26)·포매터 `(body, ctx)` 계약과 `route car` en→ko 폴백 표기(E29)·`station_metro_facilities` `lang`·`route transit` 응답의 `expressStops`/`expressStopIds`/`exit` additive·환승 빠른환승 문(A20). 버전 4곳 + CHANGELOG 2곳 동조, 발행 워크플로 성공·`npm view` 両패키지 0.10.0. 릴리스 노트 정본 `packages/cli/CHANGELOG.md`·`packages/mcp/CHANGELOG.md` §0.10.0.
- **grounding 채점기 단위 흡수(D27 종결)**: `NUMBER_RE`에 dodo 이식본의 `도`·`유로`·`€`를 정본이 흡수해 두 이식본이 갈리지 않게 했다(gildongmu 도메인 채점 무변화, 오탐 0).
- **iOS 품질 4건(E16 잔여 2건 · §8 ko 인자 순서 게이트 · E19 ③, 병렬 세션 ios-quality)**: ①Kit `RouteService.walk`·`walkAlternatives`의 `lang`이 열린 `String`에서 `DataLocale` enum(`ko`·`en`, `WalkRouteVariant` 대칭)이 됐다 — 앱은 `AppLanguage.dataLocaleValue`가 타입 정본이고 문자열 `dataLocale`은 그 투영(다른 `lang: String` 서비스는 종전 계약). ②Kit의 ko 문장 분류기 `walkStepAction`과 `GuideActionSource`(`.text`/`.step`)·`stepActionFor`·`GuideTuning.actionSource`를 지웠다 — 도보·자동차 모두 서버 투영 `step.action`만 보므로(E16 축3·K2) 검사 삭제로 졸업, `buildDisplayUnits`는 `source` 인자를 잃었다. 웹 `walk-action.ts`는 서버 정본이라 그대로이고, 공유 fixture `guide-live-rows-scenarios.json`의 도보 스텝에 서버 투영과 같은 `action`을 명시했다(웹 테스트는 종전대로 문장 분류라 결과 불변). ③**ko 문장 인자 순서 게이트**: `ios/i18n/arg-order.json`(키 → ko 플레이스홀더 등장 순서, 295키)을 `messages-to-xcstrings.mjs`가 생성·대조한다 — 기존 키의 순서가 바뀌면 exit 1(`--update-arg-order`로만 통과), 신규 키 자동 등록·사라진 키 자동 제거. 게이트 테스트 `xcstrings-arg-order.test.ts`가 저장본 동조와 **임시 저장소 사본에서의 변이 주입**(ko 한 문장의 플레이스홀더 둘 맞바꿈 → exit 1)을 프로세스 수준으로 단언한다. ④iOS 0건 문구 3곳이 웹 한정어를 되찾았다(6로케일, `ios-extra`): 소아 "야간·휴일" · 아이 놀 곳 "(키즈카페·놀이터·어린이공원)" · 무장애 "등록된". 실기기 미배포(위원장 지시 — 기기 연결 불가), 시뮬 빌드·Kit 664 테스트로 게이트 종결.
- **비-ko 로케일 잔여 5건(E27 잔여 ⓐ + E28 후속 4건, 병렬 세션 en-ios, 위원장 판정 2026-09-02)**: ①**지하철 시설 노선명의 iOS 미러** — `/api/station/metro-facilities`가 `lang`(`langParam()`, 미지 값 400)을 받고 en이면 음성유도기 `parts.lineEn`(E27 표 `subwayLineNameEn`, 예 "Line 5")을 additive로 싣는다(표 미스는 부재). iOS `StationSections.facilityName`이 `lineEn`을 우선하고 부재만 종전 앱 언어 접미 조립 — es에서 같은 화면의 역 메타 줄과 "Línea 5"/"Line 5"로 갈리던 자리. Kit `StationService.metroFacilities(lang:)`은 기본값 없는 필수 인자. 웹은 표를 직접 타므로 무변화(라우트 테스트가 ko byte-identical 단언) ②**진료 종별** `NightClinic.kind` "의원"/"병원" → `clinicNearby.kind.{clinic,hospital}` 6로케일(웹 `clinic-kind.ts` ↔ Kit `ClinicKind.swift`, ko는 같은 낱말). 그 밖(종합병원 등)은 원문 + 줄 `lang="ko"` ③**둘러보기 도로명** `SceneItem.roadRoman`(`romanAddressOf`, `road`가 있을 때만) — 웹·iOS 장면 문장이 비-ko에서 "on Myeongil-ro"로 읽는다. 도로명엔 괄호 병기를 두지 않는다(문장 안 절이라 `KoTail`을 줄 가운데 둘 수 없다 — 이름 괄호만 줄 끝) ④**길찾기 화면 병기** — 웹 `DirectionsView` 출발·도착 후보(장소 `nameRoman`, 주소 juso `engAddr`), iOS `DirectionsEndpointSearchView` 후보 행, `DirectionsTabView` 현재 위치 필드의 역지오코딩 주소(`reverseGeocode(lang:)`의 `addressEn ?? addressRoman`). 확정 라벨·최근 목록은 원명 그대로(표시 층만) ⑤**iOS 수동 위치 라틴 표기** — `DirectionsEndpoint.place(labelRoman:)`(기본 nil, 지정 화면이 장소 `nameRoman`·주소 `engAddr`를 싣는다) → `ManualLocation.labelRoman`(additive, 옛 저장값은 nil) → `manualLocationLabel(store, accessible:)`이 시각 `Roman (한글)`/낭독 로마자 두 변종을 내고 표시줄·길찾기 출발지 필드가 같은 함수를 지난다(검증 가능/불가 판정선 한 곳 유지, 웹 `useManualLocationBilingual` 동형). 패턴 매치 호출부 14곳은 arity만 확장(`BeaconTrackingSheet`·`TransitTrackingSheet`·`GildongmuApp`·`DirectionsTabView`·`LocationBarView`). 게이트: 웹 4,008 테스트·tsc·lint, Kit 668 테스트, 시뮬 빌드. **실기기 미배포(위원장 지시 2026-09-02 — 기기 연결 불가)**, 판정 항목은 `docs/BACKLOG.md` §2 E28-⑥. plan `docs/superpowers/plans/2026-09-02-backlog-sweep-2-parallel-plan.md`.
...
- **영어 잔여 소품 5건(E27 잔여 ③~⑤ + A30, 병렬 세션 en-polish)**: ①지하철 시설 노선 라벨이 E27 노선명 표(`subwayLineNameEn`)를 탄다 — A26이 임시로 `subway.lineNumber`를 로케일별로 조립하던 자리라 같은 화면의 역 메타 줄(`linesEn`)과 es에서 "Línea 5" vs "Line 5"로 갈렸다. 헤딩 문장도 `metroHeadingLine`으로 place-lines에 모아 표를 태우고(화면·도구 공용) 시설 줄과 같은 `hasHangul` 조건부 `lang="ko"`를 줄 통째로 준다. 표 미스·ko는 종전 접미 조립 폴백 ②조각 구분자 가운뎃점을 쉼표로(A30): `bike.availability`·`airQuality.station` 6로케일 — 판정 축은 **양옆 공백**이라 한국어 합성어(`관광·명소`)는 대상 밖이고, `i18n-messages.test.ts`가 그 축으로 스캔한다 ③`/api/geocode/reverse`의 `lang`을 `langParam()`으로(미지 값 400) — 소비자 전수(웹 2·iOS 1·CLI 0) 확인 후, 조용한 ko 강등 폐기 ④수동 위치 라벨 병기: `ManualLocation.labelRoman` additive에 지정 시점 라틴 표기를 저장(장소=`Place.nameRoman`, 주소=juso `engAddr`)하고 표시줄이 `useManualLocationBilingual`로 1순위 낭독, 한글은 `KoTail` 괄호 — 비-ko가 "Set location, 강동구청"으로 남던 자리 ⑤ODsay `lang=1` 게이트 4번째 경로(서울역→동탄) 실호출 46/46 PASS. GTX-A를 급행과 같은 **필수 표본**으로 승격했다 — 선정 5개 밖이라 종전 형태로는 그 노선을 한 번도 안 본 채 초록이었다. 관측: ODsay `nameKor`가 `"수도권 GTX-A"`로 지역 접두를 붙여 `isKorLine`의 한글 분기에서 통과한다. plan `docs/superpowers/plans/2026-09-01-en-locale-residual-parallel-plan.md`.
- **실시간 대중교통 안내 en 게이트 해제(E27 잔여 ①, 병렬 세션 transit-en-gate, 위원장 판정 2026-09-01)**: 비-ko 로케일에서 대중교통 실시간 안내를 시작할 수 있다(웹 정식판·iOS 실험판). ①`/api/transit/track`에 `lang`(`langParam()`, 미지 값 400 — `mode`/`phase` 분기 **앞에서** 한 번 파싱) + 실시간 영문 조각 4종(`messageEn`·`directionEn`·`destinationNameEn`·`currentLocationEn`). 지하철은 E27 `enrichArrivalEn`을 그대로 태우고 종착역만 seed 영문 직접 조회, 서울버스는 신규 `bus-arrival-en.ts`(`parseBusArrmsg` + `busArrivalMessageEn`, 국면별 어순 `In 6 min 47 sec`/`6 min 47 sec left`), TAGO는 `messageEn: ""`(ko에도 완성 문장이 없는 자리라 부재가 아니라 자리 표시 — 빼면 en 줄이 전부 ko로 폴백). `lang` 부재 응답은 키 집합이 종전과 정확히 동일 ②웹 `trackTargetUrl` 4갈래·Kit `TransitTrackService` 4메서드에 `lang` **기본값 없는 필수 인자** ③`TransitGuideLeg`·`TransitPrewalkTarget`·상태·이벤트 6종이 영문 조각을 additive로 나른다(같은 관측의 ko·en이 같은 이벤트에 — 소비자가 "마지막 항목"을 따로 기억해 짝지으면 늦은 폴·국면 전이에서 조용히 어긋난다) ④**좁은 표시 투영** `TransitDisplayLeg`·`TransitDisplayItem`(웹 `transit-display.ts` ↔ Kit `TransitDisplayProjection.swift`, 공유 fixture): 조인 필드가 **타입에 없어** 표시 계층이 조회 쿼리를 만들 수 없다 ⑤**공유 문장 descriptor** `transit-guide-text.ts` ↔ Kit `TransitGuideText.swift`(키·인자 순서·줄 언어 판정, 공유 fixture 34케이스). 웹은 어댑터 표 `TRANSIT_TEXT_ARG_NAMES`로 next-intl named 인자로 옮기고, iOS는 앱 리터럴 switch(`TransitGuideTextRenderer`) — 앱 타깃에 테스트 레인이 없어 판정을 Kit에 둔 것이 요지다 ⑥`boardOverride`가 이름에서 **인덱스**로(동명 역이 둘이면 이름 역조회가 다른 역의 영문명을 오류 없이 고른다) ⑦웹 live region·상시 표시가 `lang`을 함께 나른다(한국어 폴백이 섞이면 `"ko"`, 영어 줄은 무태그) — 영어 엔진이 한글을 만나면 그 이름이 통째로 침묵하기 때문 ⑧게이트 해제는 대중교통 4자리뿐이고 계단 회피·자동차·도보·출입구 승격은 불변(반대 방향 단언으로 증명). 가드 3층: 발화 sentinel(자리마다 다른 토큰 + 정확한 결과 단언 + 조각별 제거 역방향), 소스 가드(조인 값이 문구 조회에 들어가는 자리 — 실제 누수 3건 검출: 빠른하차 문구·대안 경로 시트 2곳), 게이트 조건 가드. 실호출 게이트 `scripts/verify-transit-track-lang.mjs` 8/8 PASS(TAGO는 upstream 장애로 미실측 — 0건·장애를 통과로 세지 않는다). codex 설계 리뷰 20건 판정(채택 16·부분 3·전제 오류 기각 1) spec §9. 실승차 판정은 W2 마감 뒤(`docs/FIELD-TEST.md` §5-6). ⑨웨이브 종료 뒤 회귀 수정 1건(a11y 재감사): 승차 전 도보 세션의 목적지 라벨만 한국어로 남아 en 한 흐름이 `Walk to Cheonho` → `천호까지 200m` → `Arrived at Cheonho`로 들렸다 — `destinationLabel`은 표시 전용(조인 소비자 0)이라 웹·iOS 모두 대중교통 문구와 같은 이름을 넘기고, iOS는 `transitPrewalkLabel`이 Kit 빈 문자열 정규화기까지 봉한다. spec `docs/superpowers/specs/2026-09-01-transit-guide-en-gate-design.md`.

## 2026-08-31

- **카카오 분류 경로 영문화(A28, 병렬 세션 category-en)**: en 장소 카드·상세의 "교육,학문 > 학교 > 중학교"가 `Education & Academia > School > Middle School`로. ①세그먼트 사전 `src/lib/data/kakao-category-en.json` 947항목(카카오 직접 스윕 4,000회 → 장소 59,535·고유 경로 1,176·세그먼트 1,246 코퍼스, 번역은 세션이 직접 작성, 브랜드는 공식 영문 표기만 — 지역 소규모 브랜드 잎사귀는 의도적 미등재) ②순수 계층 `kakao-category.ts`: 세그먼트 **전부** 등재일 때만 영문 경로, 하나라도 미등재·빈 세그먼트·제어 문자는 부재(fail-closed) ③서버 additive `categoryEn`(키 부재 계약, 로케일 무관 — `nameRoman` 동형): `kakao-local`·`kids-places`·`surroundings`·`surroundings-scene` → `Place`·`KidsPlace`·`SurroundingPlace`·`SceneItem`, `nearby-place` 3종 전달 ④표시: 웹 `PlaceCard`·`PlaceDetail`, iOS `PlaceRow`·`PlaceDetailView`·`AroundNearbyView`가 `pickCategory`(비-ko: `categoryEn` ?? 원문+`lang="ko"`, 웹 ↔ Kit 공유 fixture), Kit 모델 4종 디코딩. 판정 축(`isStation`·`categoryOf`·키즈 화이트리스트·채팅)은 원문 유지 + 소스 가드 ⑤게이트: 코퍼스 커버리지 장소 97.7%, 실호출 `scripts/verify-kakao-category-en.mjs`(4투영 경로·7지역) 카드 96.8%·고유 경로 88.0% PASS, 변이 주입(가드 제거 → 3건 검출), Kit 646 테스트, 시뮬 빌드. codex 설계 리뷰 15건 판정(반영 8·실측 반증 기각 7 — 다부모 세그먼트 0건) spec §10. spec `docs/superpowers/specs/2026-08-31-kakao-category-en-design.md`.
- **수량 문구 복수형(A29, 병렬 세션 plurals, 위원장 웹 실사용 "1 places")**: 웹 `messages/{en,es,fr,it}` count류 27키를 ICU `{count, plural, one {…} other {…}}`로(ko·ja 바이트 불변, `device(s)`·`cambio/i`·`is/are` 회피 표기 제거, fr은 CLDR대로 0도 단수). iOS는 xcstrings 네이티브 `variations.plural` 대신 **ICU 블록을 카탈로그 문자열 그대로** 싣고(`{N, plural, one {…} other {…}}`, lproj 컴파일 시 `.stringsdict`가 되어 명시 언어 조회가 못 읽는 것을 실험으로 확정) Kit `Localization.swift`의 순수 해석기(`pluralCategory`·`resolvePluralBlocks`·`formatLocalized`)가 `appLocalized`·`kitLocalized` 공통 경로에서 분기를 고른다. ios-extra 12키 복수형 + `ios.nearby.announce{Places,Bikes,Stops,Stations,Events}`(단위 낱말 합성 `announceCount`+`unit*` 폐기, ko "주변 곳 3개" → "주변 장소 3곳"), Swift 호출부 수량 인자 `Int` 통일. 게이트: `i18n-messages` 토큰 파서 ICU 인자화, `i18n-plurals`(피연산자 허용 목록·en count=1 골든 표·공유 fixture `plural-category-cases.json`), `xcstrings-plural`(변환·`xcstringstool` 컴파일 산출물·Swift 정적 가드), Kit `LocalizationTests`. 변환 스크립트는 스킵이 있으면 exit 1. spec `docs/superpowers/specs/2026-08-31-plural-forms-design.md`(codex 설계 리뷰 23건 처리 §10).
- **대중교통 영문화(E27, 병렬 세션 transit-en)**: ①`/api/route/transit`·`/api/station/{subway-arrival,subway-arrival/nearby,meta,timetable}`에 `lang`(ko|en, 미지 값 400). en이면 ODsay `lang=1` 영문을 **additive `*En`**(`lineNameEn`·`fromNameEn`·`toNameEn`·`stops[].nameEn`·`departNameEn`·`arriveNameEn`)에 싣고 **한국어 필드는 그대로**(조인 경로 무변경). `*Kor` 하나라도 빠진 영문 응답은 버리고 ko로 재조회(fail-closed). ko·CLI/MCP는 byte-identical ②노선명 영문 표 `subway-line-names.ts`(seed 47종·실시간·ODsay·TAGO 생산자 5축 drift 테스트, 급행 `Line 9 Express` 보존, 미지=부재) — 도착 `lineEn`·내 주변/역 메타 `linesEn`·시간표 `lineNameEn` ③ODsay 영문 정류소명 정규화 `transit-name-en.ts`(`ㆍ`·`. `→쉼표, `Stn.`→`Station`, 약어·이니셜·숫자 뒤 보존) ④실시간 도착 영어 문장 `subway-arrival-en.ts`(서버, `arvlCd`×문장 정확 행렬 + seed 영문 역명 — `Arrived at Gangnam`·`In 2 min 30 sec`·`3 stations away`·`To Seongsu via Yeoksam`·`Outer Circle`, 어긋나면 부재) ⑤웹 `pickLine`·iOS `TransitDisplay.pickLine`로 **줄 단위 원자성**(영문 조각이 하나라도 없으면 줄 전체 한국어 + `lang="ko"`, 비-en 로케일의 순수 영어 줄은 `lang="en"`), 역명 병기 `Gangnam (강남)`(E28 `bilingualName`, 괄호는 `aria-hidden`·iOS `accessibilityLabel` 영문만). 웹 6곳·iOS 5곳 표시, Kit `RouteService.transit`·`StationService`·`NearbyService` `lang` 필수 인자 ⑥실호출 게이트 `scripts/verify-odsay-lang.mjs` 35/35(서울역 도착에서 공항철도↔인천국제공항선 노선 동치 결함 검출·수정). 설계 리뷰(codex) 17건 반영 §9. spec `docs/superpowers/specs/2026-08-31-transit-english-design.md`.
- **지하철 승차 중 상태줄 탑승자 시점 문장(A27, 위원장 실승차 피드백 2026-08-29 "충정로까지 전역 도착")**: 승차 국면 지하철은 `arvlCd`로 문장을 고른다 — 3·4·5 "다음 역 {stop}." · 0 "{stop} 진입 중." · 1 "{stop} 도착." · 2 "{stop} 출발." · 99 생략(잔여 수가 말한다) · 미지 코드 원문 그대로(까지 틀 없이). 순수 함수 `subwayRidingMessage` 웹 ↔ Kit 미러(공유 fixture), 상태 `lastArrivalCode`·이벤트 `arrivalCode` additive, 6로케일 키 `transitGuide.subway*`. 버스 승차 국면·대기 후보·내 주변 도착 목록은 불변. ⚠ ko 상태줄 문구가 바뀐다(근거 = A27). spec 같은 문서 §3.9.

- **비-ko 로케일 한국어 잔존 결함 묶음(A26, 병렬 세션 en-fix)**: ①횡단 유닛 판정을 ko 문장 "건너" 부분 문자열에서 서버 구조화 플래그 `WalkRouteStep.crossing`(카카오=재작성 횡단 행동문, Tmap=turnType 횡단 행동, `includeGeometry` 전용)으로 바꿔 en 도보 안내에서도 횡단 유닛이 선다(웹 `isCrossingStep` ↔ Kit 미러, 공유 fixture로 ko 카카오 기본 경로 byte-불변 — ko+Tmap 폴백은 종전에 "건너" 부재로 한 번도 서지 않던 횡단 유닛이 이제 선다, 의도된 수정) ②iOS `RouteService.car`의 `lang`을 기본값 없는 필수 인자로, `BeaconModel` 자동차 안내·ETA 재조회에 `AppLanguage.dataLocale` 배선 ③`/api/route/car` 응답에 `guidanceLang`("ko"/"en") 추가 — en 요청의 ko 폴백(NCP 키 부재·경유지·**기하 요청** — 리뷰 검출: 기하 요청까지 NCP로 보내면 iOS en 자동차 안내가 간략으로 강등된다)을 정직화, 웹 `StepList`가 ko면 `<li lang="ko">`; 502 경로 탐색 실패엔 `code:"noRoute"` ④`CarRouteBriefing`·`TransitRouteBriefing` 오류 낭독을 서버 한국어 문자열 대신 status·code로 고른 `t()` 문장으로(6로케일 키 `route.briefing.{noRoute,rateLimited,notConfigured}`·`route.transit.notConfigured`) ⑤서버 합성 한국어에 구조화 원재료 병행(additive): 무장애 라벨 27종 `key`→`barrierFreeInfo.facility.*`(웹 `barrierFreeLines(d, t)`·iOS switch, 동조 가드 `barrier-free-labels-i18n.test.ts`), `SeoulMetroFacility.parts`(엘리베이터 위치 compass·meters·dong / 화장실 restroomType·wheelchairAccessible / 음성유도기 location·line), `TimetableLine.lineCore`("선" 접미), iOS "N정류장 전"→`ios.nearby.stopsBefore`; 엘리베이터 위치 서버 문장의 거리도 `formatDistance`로(1km 이상 "1.2km", 클라 조립과 동일) ⑥`lang="ko"` 마크업: `PlaceCard` 이름·분류, `PlaceDetail` 분류, 대중교통 재승차역 버튼(판정 `hasHangul`). 건너뛴 자리(prose 템플릿 안 한국어 삽입·노선명)는 `docs/BACKLOG.md` A26 종결 기록 → E27·E28 재료. plan `docs/superpowers/plans/2026-08-31-en-locale-korean-cleanup-parallel-plan.md`.

- **장소명 영문 병기(E28, 병렬 세션 place-names)**: 영문 원천이 없는 이름(카카오·네이버 장소, 병원, TAGO/TOPIS 정류소, 문화행사 제목·장소, 따릉이 대여소, 혼잡도 영역, 공기질 측정소, 둘러보기·한눈에 보기 항목, 현재 위치 주소)에 서버가 국어원 로마자 `nameRoman`류를 additive로 싣고(`src/lib/romanize.ts` 자체 규칙 — `@romanize/korean`은 94어 표에서 30어 오답이라 기각), 비-ko 로케일이 `Roman (한글)` 한 줄 괄호로 병기한다. 접근 가능한 이름은 괄호 앞만: 웹은 괄호 span `aria-hidden`+`lang="ko"`를 접근성 객체의 **마지막 노드**에(가운데 두면 텍스트 노드가 갈린다 — Chrome AX 실측), iOS는 `.accessibilityLabel`. 조립 규칙은 `bilingualName`(웹 ↔ Kit, 공유 fixture)이고 역지오코딩은 `lang=en`에서 juso 공식 영문(`addressEn`)→로마자(`addressRoman`). 웹 주소 결과의 한글 보조 블록(버튼 이름에 한글이 낭독되던 자리)과 지번 줄 분절도 함께 정리. ko 화면 byte-identical. spec `docs/superpowers/specs/2026-08-31-place-name-bilingual-design.md`(codex 설계 리뷰 34건 판정 §10, 실결함 1건 검출).
