...
내 주변 지하철·장소 상세의 도착 한 줄이 서울시 원문 표기를 그대로 낭독했다 — `[4]번째 전역 (하남검단산)`·`3분 후(2번째 전)`. 이제 그 문장을 **읽어 같은 뜻을 우리 문장으로 다시 쓴다**(`4정거장 전 하남검단산에 있습니다.`·`2정거장 전, 3분 후 도착. 현재 방배.`). 못 알아본 문장은 그 줄만 원문 + A32 꼬리라 실패 방향이 현행이다. 코퍼스 858행 전수에서 원문 폴백 0건·역명 불일치 0건. spec `docs/superpowers/specs/2026-09-13-subway-arrival-prose-design.md`.
