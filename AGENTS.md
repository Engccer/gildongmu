...
- **definition list(`<dl>/<dt>/<dd>`)는 단순 라벨-값에 금지.** SR이 항목마다 "용어/정의" 역할과 콜론을 별도 낭독해 "표처럼" 읽힘. 평문 `<p><span class="font-medium">라벨</span> 값</p>`로(콜론 제거, 라벨 볼드 시각구분만). `lang="ko"`는 `<span>`으로 이전.
