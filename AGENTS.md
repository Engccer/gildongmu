...
| `docs/INTEGRATIONS.md` | 외부 통합(provider·route·envelope·좌표·영문 응답·실시간 안내 세션·API 키)의 **상세 계약**. 이 파일의 `→ INTEGRATIONS` 항목 본문 | 통합과 무관한 내부 계약(→ PATTERNS) |
...
- **답변 복사·듣기는 평문 한 벌과 통지 창구 하나를 쓴다**(웹 B12): 평문은 `markdownToPlainText`(Kit와 공유 fixture. 이 파일엔 JS 약칭 클래스·`m` 플래그 금지, ICU와 줄 경계·공백 집합이 다르다), 통지는 진행 창구 재사용, 듣기는 `speechSynthesis` 우선·`/api/tts`는 보이스 없을 때만. → PATTERNS
