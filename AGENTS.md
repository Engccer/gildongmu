...
- **결정 지점 안내는 두 층이고 거리가 다르다**(2026-08-09): 40m `announceSteps`(전문, **무톤**)가 *무엇을* 할지, 10m `imminent`(짧은 명령형 + `ahead` 톤 + 햅틱)가 *지금이다*를 알린다. 문구 선택은 `walkStepAction`(웹 ↔ Kit `WalkAction.swift` 미러, 서버 재작성 문장에 부분 문자열 판정, 건널목을 회전보다 먼저 — "좌측 횡단보도"의 "좌측"은 회전이 아니다). ⚠ **`imminentUpTo < announcedUpTo`가 발화 조건이고 그래서 이 블록이 6b보다 앞이다** — 전문 없이 명령만 내보내지 않으면서, 짧은 유닛에서 6b에 매번 밀려 모퉁이를 지나 울리는 것도 막는다. ⚠ **행동 없는 경계는 발화만 건너뛰고 래치는 전진**시킨다(세워 두면 다음 회전을 영영 못 알린다). ⚠ **walk 전용**(`imminentAheadM`, car는 null — 15m/s에서 10m는 0.7초다). ⚠ **햅틱은 백그라운드 미지원**이라 어떤 신호도 진동에만 싣지 않는다. 설계는 spec `2026-08-09-walk-imminent-cue-design.md`.
