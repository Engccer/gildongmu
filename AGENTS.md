...
- **19개 도구**: 장소2(검색·주소)·내주변6(소아진료·아이놀곳·둘러보기·지하철·무장애관광지·문화행사 `get_nearby_events` — 카드 없이 산문 정본)·좌표5(버스·따릉이·공기질·날씨 `get_weather` 기상청 — 카드 없이 산문 정본·보행인프라 `get_walk_infrastructure` — 무키라 게이트 없음, 서비스 계층만 호출)·역명2·길찾기3(자동차·대중교통·도보 `get_walk_route`)·웹검색1(`search_web` Perplexity). 각 게이트. `get_bus_route`는 V1 제외. 공기질·날씨 declaration은 긍정 트리거("직접 질문·야외 활동 적합성일 때만")로 과잉 조회 억제하되, 공기질 호출 시 get_weather 동반(연관 도메인 페어링 — 미세먼지 답변에 날씨 접지, 2026-07-28. 날씨 쪽 트리거 문구만으론 동반이 성립하지 않아 공기질 declaration에 co-call 지시가 필요했다 — 실측).
...
| `SEOUL_OPEN_DATA_KEY` | `hasSeoulOpenDataKey` | 서울 열린데이터(따릉이·문화행사). ⚠ 실시간 지하철은 별도 키 |
